...
`packages/presentation/design-system` 是 SciForge 的语义主题 token 与低层 React primitives 包。它刻意保持小而稳定：页面可以保留自己的科研领域布局，重复控件、交互状态和基础视觉语言由本包提供。
...
- 面向 artifact 的复杂展示应放在 `packages/presentation/components` 或长期别名 `packages/presentation/interactive-views`。
