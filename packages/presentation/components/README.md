...
- 长期能力名称是 interactive artifact views/renderers；`packages/presentation/components` 继续作为稳定 registry 兼容层。
...
`packages/presentation/components` 的长期职责是把结构化 artifact 渲染成可读、可操作、可引用的 interactive surface：
...
`packages/presentation/interactive-views` 是本包的非破坏性语义别名。当前 registry 真相源是 `packages/presentation/components`，并继续保留 `@sciforge-ui/components` package name、manifest、componentId、alias 和 renderer 兼容导出。
