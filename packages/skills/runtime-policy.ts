...
export function agentServerGeneratedTaskPromptPolicyLines() {
  return [
    ...
    'Capability discovery contract: if the compact capability brief is insufficient, ambiguous, or a provider/verification/repair path needs alternatives, call capability_discovery.search/expand/plan/explain first; do not infer completion from discovery recommendations. After discovery, actual work must still call invoke_capability through the Gateway.',
...
export function agentServerCapabilityRoutingPromptPolicyLines() {
  return [
    ...
    'When capabilityBrokerBrief or selectedToolIds includes local.vision-sense/observe.vision, treat it only as the optional screenshot/grounding/verifier sense provider for the Computer Use action provider. Do not make generated tasks emit click/type_text/press_key/scroll/wait commands; invoke the Computer Use provider through its host adapter and preserve only compact screenshot refs/grounding/execution/pixel-diff summaries across turns. Do not read DOM or accessibility tree for that vision path, and fail closed for send/delete/pay/authorize/publish actions unless upstream confirmation is explicit.',
