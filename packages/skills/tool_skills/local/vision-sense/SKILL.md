...
metadata:
  ...
  requiredConfig: shared-model-router-config, model-router-vision-grounding, trace-output-dir
  tags: vision, modality:vision, grounding, text-output, computer-use-input, model-router
---
...
- 适合：解释截图/图像、生成视觉目标描述、输出 Model Router grounding translator 坐标证据、text-only observation 和 file-ref-only trace memory。
