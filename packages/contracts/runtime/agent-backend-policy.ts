...
export interface RuntimeAcceptanceFailureLike {
  code: string;
  detail: string;
}
...
export function runtimeAgentBackendFailureCategories(text: string, httpStatus?: number): RuntimeAgentBackendFailureKind[] {
  ...
  if (/responseTooManyFailedAttempts|too[\s-]?many[\s-]?failed[\s-]?attempts/i.test(text)) categories.push('too-many-failed-attempts', 'retry-budget');
  if (/exceeded retry(?: limit)?|retry[\s_-]?budget|too many retries|max retries/i.test(text)) categories.push('retry-budget');
...
export function runtimeAcceptanceDiagnostic(input: { failures: RuntimeAcceptanceFailureLike[] }): RuntimeAcceptanceDiagnostic {
  const rawDetails = input.failures
    .map((failure) => `${failure.code}: ${failure.detail}`)
    .join('\n\n');
  const haystack = rawDetails.toLowerCase();
  const secondary = input.failures
    .filter((failure) => !/execution-failed|backend-repair-failed/i.test(failure.code))
    .map((failure) => readableRuntimeAcceptanceFailure(failure.code));
  const categories = runtimeAgentBackendFailureCategories(rawDetails);
  if (categories.some(runtimeAgentBackendIsRateLimitKind)) {
    return {
  ...
  return {
    title: '任务未通过验收',
    summary: input.failures.map((failure) => failure.detail).join('；'),
    recoverActions: [],
    secondary,
    rawDetails,
  };
...
export function readableRuntimeAcceptanceFailure(code: string) {
