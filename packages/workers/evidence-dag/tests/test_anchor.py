...
PARSED = {
    "nodes": [
        {"tmp_id": "s", "type": "source_assertion", "trace_ref": "step-1",
...
class TestAnchorRepair(unittest.TestCase):
    def test_invalid_refs_repaired_to_real_ids(self):
        ...
        self.assertEqual({n.trace_refs[0] for n in g.nodes.values()}, {"step-1", "step-2"})
        repaired = resolve_trace_refs(g, TRACE)
        self.assertEqual(repaired, 2)
        refs = {n.type: n.trace_refs[0] for n in g.nodes.values()}
        self.assertEqual(refs[NodeType.SOURCE_ASSERTION], "item_a1")
        self.assertEqual(refs[NodeType.CLAIM], "item_b2")

    def test_valid_refs_left_untouched(self):
        parsed = {"nodes": [{"tmp_id": "s", "type": "source_assertion", "trace_ref": "item_a1",
                             "content": "totally unrelated text xyz"}], "edges": []}
        g = build_graph(parsed, "t")
        self.assertEqual(resolve_trace_refs(g, TRACE), 0)  # already valid -> no change
        self.assertEqual(next(iter(g.nodes.values())).trace_refs, ["item_a1"])
    ...
    def test_no_false_anchor_when_no_overlap(self):
        ...
        self.assertEqual(next(iter(g.nodes.values())).trace_refs, ["bogus"])
