...
Each upstream role needs only a base URL, model name, and API key. There is no provider or wire-protocol setting. The router derives a compatibility profile from both the model id and endpoint, chooses the provider's documented primary wire, and caches the working protocol by normalized base URL and model. OpenAI and Qwen 3.7 can use Responses; DeepSeek, Moonshot/Kimi, and Zhipu/GLM use Chat Completions; official Google OpenAI-compatibility endpoints use Chat Completions; other gateways retain negotiated fallback behavior. Before dispatch, the router removes Chat-only fields from Responses history, preserves provider reasoning state where its Chat dialect requires it, maps legacy reasoning and token-limit fields, and normalizes function-tool JSON Schema without renaming semantic property, `required`, or enum values.

The router retries another protocol only after a definitive endpoint or request-schema rejection; authentication, quota, rate-limit, timeout, ambiguous server, and ordinary validation failures are returned without retrying a potentially billable request. A 2xx response whose body or terminal stream frame does not match the selected wire schema remains terminal. The only 2xx fallback exception is an explicit structured upstream error event, such as a gateway rejecting a model on its Responses converter before any model output is produced.
