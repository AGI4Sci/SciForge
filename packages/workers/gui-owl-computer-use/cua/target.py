...
_LOCATOR_FIELDS = {
    ...
    TargetKind.WINDOWS_UIA: {"processId", "nativeWindowHandle", "automationId"},
    TargetKind.ISOLATED_DESKTOP: {"isolatedEnvironmentId"},
    TargetKind.HOST_DESKTOP: {"monitorId"},
    TargetKind.STATIC_IMAGE: {"imageRef"},
}
...
def parse_target_descriptor(value: object, *, generate_id: bool = True) -> TargetDescriptor:
    ...
    if kind is TargetKind.WINDOWS_UIA and "nativeWindowHandle" in locator and generation is None:
        raise ValueError("windows-uia nativeWindowHandle requires target.generation")
...
def host_desktop_target() -> TargetDescriptor:
    return TargetDescriptor(
        target_id="host-desktop:default",
        kind=TargetKind.HOST_DESKTOP,
        locator=MappingProxyType({"monitorId": "1"}),
    )
...
def _validate_locator(kind: TargetKind, value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if kind is TargetKind.WINDOWS_UIA and not any(
        key in result for key in ("processId", "nativeWindowHandle", "automationId")
    ):
        raise ValueError("windows-uia target requires a processId, nativeWindowHandle or automationId")
    if kind is TargetKind.WINDOWS_UIA and "nativeWindowHandle" in result and "processId" not in result:
        raise ValueError("windows-uia nativeWindowHandle requires processId")
    ...
    if kind is TargetKind.ISOLATED_DESKTOP and "isolatedEnvironmentId" not in result:
        raise ValueError("isolated-desktop target requires isolatedEnvironmentId")
    if kind is TargetKind.STATIC_IMAGE and "imageRef" not in result:
        raise ValueError("static-image target requires imageRef")
    return result
...
def _redact_locator(kind: TargetKind, locator: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if kind is TargetKind.STATIC_IMAGE and "imageRef" in result:
        result["imageRef"] = "<redacted>"
    return result
