...
test('validates Markdown image destinations and required local artifacts', async (t) => {
  const tempRoot = await mkdtemp(join(tmpdir(), 'write-assist-markdown-images-'))
  t.after(async () => {
    await rm(tempRoot, { recursive: true, force: true })
  })
  const workspaceRoot = join(tempRoot, 'workspace')
  await mkdir(join(workspaceRoot, 'reports', 'assets'), { recursive: true })
  await writeFile(join(workspaceRoot, 'reports', 'assets', 'figure.png'), Buffer.from('png'))
  await writeFile(join(workspaceRoot, 'reports', 'valid.md'), '![Method](assets/figure.png)\n', 'utf8')
  await writeFile(join(workspaceRoot, 'reports', 'invalid.md'), [
    '![Empty]()',
    '![Missing](assets/missing.png)'
  ].join('\n'), 'utf8')
  await writeFile(join(workspaceRoot, 'reports', 'unreferenced.md'), '# No image reference\n', 'utf8')
  const outsideImage = join(tempRoot, 'outside.png')
  await writeFile(outsideImage, Buffer.from('png'))
  await symlink(outsideImage, join(workspaceRoot, 'reports', 'assets', 'escape.png'))
  await writeFile(
    join(workspaceRoot, 'reports', 'escaped.md'),
    '![Escape](assets/escape.png)\n',
    'utf8'
  )

  const service = createWriteAssistService()
  const valid = await service.validateMarkdownImages({
    workspaceRoot,
    path: 'reports/valid.md',
    minimumImages: 1,
    minimumLocalImages: 1,
    expectedLocalImages: ['reports/assets/figure.png']
  })
  assert.equal(valid.ok, true)
  if (!valid.ok) return
  assert.equal(valid.valid, true)
  assert.equal(valid.localImageCount, 1)
  assert.deepEqual(valid.matchedExpectedLocalImages, ['reports/assets/figure.png'])

  const invalid = await service.validateMarkdownImages({
    workspaceRoot,
    path: 'reports/invalid.md',
    minimumLocalImages: 1,
    expectedLocalImages: ['reports/assets/missing.png']
  })
  ...
  assert.deepEqual(invalid.matchedExpectedLocalImages, [])
  assert.equal(invalid.issues[0]?.line, 1)
  assert.equal(invalid.issues[1]?.line, 2)

  const unreferenced = await service.validateMarkdownImages({
    workspaceRoot,
    path: 'reports/unreferenced.md',
    expectedLocalImages: ['reports/assets/figure.png']
  })
  assert.equal(unreferenced.ok, true)
  if (!unreferenced.ok) return
  assert.equal(unreferenced.valid, false)
  assert.deepEqual(unreferenced.matchedExpectedLocalImages, [])
  assert.deepEqual(unreferenced.issues.map((issue) => issue.code), ['expected_local_image_missing'])

  const escaped = await service.validateMarkdownImages({
    workspaceRoot,
    path: 'reports/escaped.md',
    expectedLocalImages: ['reports/assets/escape.png']
  })
  assert.equal(escaped.ok, true)
  if (!escaped.ok) return
  assert.equal(escaped.valid, false)
  assert.deepEqual(escaped.matchedExpectedLocalImages, [])
  assert.deepEqual(escaped.issues.map((issue) => issue.code), [
    'outside_workspace',
    'expected_local_image_missing'
  ])
})
