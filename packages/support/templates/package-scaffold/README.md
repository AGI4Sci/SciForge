...
5. If it renders artifacts or exposes interactive object refs/events, place it under `packages/presentation/components` today and optionally re-export from `packages/presentation/interactive-views`.
6. If it defines stable cross-package types, place it in `packages/contracts/runtime` or a package-owned public export.
...
- Shared types move upward into `packages/contracts/runtime` or a package public export before multiple owners depend on them.
