...
class RuntimeActors:
    """One SQLite connection owner for each concurrently executing actor.

    ``sqlite3.Connection`` transaction state belongs to a connection, not to a
    Python thread. Sharing an Engine between the threaded HTTP server and a
    worker therefore lets one actor commit or nest another actor's transaction.
    Each long-lived actor gets its own Engine/Store/connection instead.
    """
    ...
    @classmethod
    def create(cls, db_path: str, session_dir: str, *, llm: Any = None) -> "RuntimeActors":
        engines: list[Engine] = []
        try:
            for _ in range(3):
                engines.append(Engine(db_path, session_dir, llm=llm))
        except Exception:
            for engine in engines:
                engine.store.close()
            raise
        return cls(api=engines[0], update=engines[1], audit=engines[2])
...
class Handler(BaseHTTPRequestHandler):
    ...
    # ThreadingHTTPServer may enter one Handler per request concurrently. The
    # API actor owns one connection, so all access to that connection is
    # serialized here. Background actors use independent connections.
    ...
    def _dispatch(self, method: str, parts: list[str], query: dict) -> Any:
        ...
        if method == "GET" and len(parts) == 3 and parts[0] == "updates" \
                and parts[2] == "status":
            version = query.get("acceptedRequestVersion")
            fingerprint = query.get("desiredFingerprint")
...
def main() -> None:
    ...
    if os.environ.get("EDAG_MODEL_ROUTER_BASE_URL"):
        ...
        llm = ModelRouterLLM()
    actors = RuntimeActors.create(db_path, session_dir, llm=llm)
    Handler.engine = actors.api
    Handler.api_token = os.environ.get(API_TOKEN_ENV, "")
    stop = threading.Event()
    update_thread = threading.Thread(
        target=_update_worker, args=(actors.update, stop), daemon=True)
    audit_thread = threading.Thread(
        target=_audit_worker, args=(actors.audit, stop), daemon=True)
    update_thread.start()
    audit_thread.start()
    ...
    try:
        ...
        actors.api.store.close()
        if not update_thread.is_alive():
            actors.update.store.close()
        if not audit_thread.is_alive():
            actors.audit.store.close()
