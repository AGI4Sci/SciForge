...
def _is_restricted_policy(policy: Any) -> bool:
    ...
    # Canonical access policy owned by Artifact Versions. A workspace-scoped
    # Project reader may consume ``workspace`` and ``public`` records;
    # ``allowExport`` is independent from read access. Unknown or restricted
    # shapes remain fail-closed below.
    if set(policy) == {"visibility", "principals", "allowExport"}:
        visibility = policy.get("visibility")
        principals = policy.get("principals")
        allow_export = policy.get("allowExport")
        if (
