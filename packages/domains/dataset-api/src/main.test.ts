...
test('publishes the complete Dataset API surface through governed capabilities', () => {
  ...
  assert.ok(definitions
    ...
    ].includes(definition.id as never))
  ...
  for (const id of [
    ...
    const registration = findDefinition(definitions, id)
    assert.deepEqual(registration.audiences, ['ui'])
    assert.equal(registration.effect, 'external-write')
    assert.equal(registration.approval, 'confirmation')
  }
  assert.equal(findDefinition(definitions, DATASET_API_CAPABILITY_IDS.confirmPlan).approval, 'confirmation')
})
...
test('binds plan confirmation to the broker approval invocation', async () => {
  const calls: Array<Record<string, unknown>> = []
  const definitions = capabilityDefinitions({
    processing: {
      confirmPlan: async (input: Record<string, unknown>, approval: Record<string, unknown>) => {
        calls.push({ input, approval })
        return { planId: input.planId, status: 'confirmed' }
      }
    }
  })
  const capability = findDefinition(definitions, DATASET_API_CAPABILITY_IDS.confirmPlan)
  const input = { planId: 'plan-0123456789abcdef' }

  await assert.rejects(
    capability.handler(input, { caller: { workspaceId: '/workspace/project', approvals: [] } }),
    /exact broker approval receipt/
  )
  await capability.handler(input, {
    caller: {
      workspaceId: '/workspace/project',
      approvals: [{
        actionId: DATASET_API_CAPABILITY_IDS.confirmPlan,
        invocationId: 'agent_inv_exact',
        mode: 'confirmation'
      }]
    }
  })
  assert.deepEqual(calls, [{
    input: { ...input, workspaceRoot: '/workspace/project' },
    approval: { invocationId: 'agent_inv_exact' }
  }])
})
...
type Definition = {
  ...
  handler: (
    ...
    context: { caller: { workspaceId?: string; approvals?: Array<Record<string, unknown>> } }
