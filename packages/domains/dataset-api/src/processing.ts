...
export function createDatasetProcessingService(options: {
  ...
  return {
    ...
    async confirmPlan(
      raw: DatasetConfirmPlanInput,
      approval: Readonly<{ invocationId: string; approvedAt?: string }>
    ) {
      const input = datasetConfirmPlanInputSchema.parse(raw)
      const workspaceRoot = await resolveWorkspaceRoot(input.workspaceRoot, defaultWorkspaceRoot)
      const planPath = join(workspaceRoot, '.sciforge', 'datasets', 'plans', `${safeId(input.planId)}.json`)
      ...
      if (draft.planId !== input.planId || draft.status !== 'draft') {
        throw new Error(`Preparation plan '${input.planId}' is not a reviewable draft.`)
      }
      const draftSha256 = hash(planBytes)
      const existing = await readPlanConfirmation(workspaceRoot, input.planId)
      const confirmation = existing?.draftPath === planPath && existing.draftSha256 === draftSha256
        ? existing
        : {
            version: 1 as const,
            planId: input.planId,
            status: 'confirmed' as const,
            approvalId: approval.invocationId,
            approvalSource: 'host-capability-broker' as const,
            draftPath: planPath,
            draftSha256,
            confirmedAt: approval.approvedAt ?? new Date().toISOString()
          }
      await writeIdempotentJson(planConfirmationPath(workspaceRoot, input.planId), confirmation)
      const artifact = await describeStandaloneArtifact(
        planConfirmationPath(workspaceRoot, input.planId),
        'plan',
        'dataset_confirm_plan',
        { planId: input.planId, approvalId: confirmation.approvalId },
      ...
      return {
        ...
        artifact,
        pipelineArtifact,
        plan: { ...draft, status: 'confirmed' as const, confirmation }
      }
    },
    ...
    async publish(raw: DatasetPublishInput) {
      ...
      const uncoveredArtifacts = validationCoverageGaps(artifacts)
      if (quality.validationReportCount > 0 && uncoveredArtifacts.length > 0) {
        throw new Error(
          'Dataset publication validation reports do not cover the exact artifact path and SHA-256: ' +
          uncoveredArtifacts.map((artifact) => artifact.sourcePath).join(', ')
        )
      }
...
async function requireConfirmedPlan(
  ...
  const confirmedPlan = { ...plan, status: 'confirmed' as const, confirmation: recordedConfirmation }
...
async function readPlanConfirmation(workspaceRoot: string, planId: string): Promise<{
  ...
  status: string
  approvalId: string
  approvalSource: 'host-capability-broker'
  draftPath: string
  draftSha256: string
  confirmedAt: string
} | null> {
  try {
    ...
    if (confirmation.planId !== planId || confirmation.status !== 'confirmed' ||
        typeof confirmation.approvalId !== 'string' || confirmation.approvalSource !== 'host-capability-broker' ||
...
function validationCoverageGaps(artifacts: Array<{
  sourcePath: string
  sha256: string
  operation?: string
  format?: string
  parentArtifacts?: Array<{ path: string; sha256: string }>
}>): Array<{ sourcePath: string; sha256: string }> {
  const validationReports = artifacts.filter((artifact) => (
    artifact.operation === 'dataset_validate' || artifact.operation === 'dataset_structure_validate'
  ))
  const covered = new Set(validationReports.flatMap((report) => (
    report.parentArtifacts ?? []
  )).map((parent) => `${resolve(parent.path)}\u0000${parent.sha256}`))
  return artifacts
    .filter(isPublishedValidationTarget)
    .filter((artifact) => !covered.has(`${resolve(artifact.sourcePath)}\u0000${artifact.sha256}`))
    .map(({ sourcePath, sha256 }) => ({ sourcePath, sha256 }))
}

function isPublishedValidationTarget(artifact: {
  operation?: string
  format?: string
}): boolean {
  if (artifact.format === 'report') return false
  return ![
    'dataset_filter_excluded',
    'dataset_deduplicate_removed',
    'dataset_id_map_ambiguous',
    'dataset_id_map_unmatched',
    'dataset_join_unmatched_left',
    'dataset_join_unmatched_right',
    'dataset_graph_invalid'
