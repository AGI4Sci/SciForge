...
import type { ScientificJobScenario } from './contract.js'

export type { ScientificJobScenario } from './contract.js'

export const SCIENTIFIC_COMPUTE_TRACE_EVENT_TYPES = Object.freeze({
  submitted: 'scientific-compute.job-submitted',
  started: 'scientific-compute.job-started',
  finished: 'scientific-compute.job-finished',
  failed: 'scientific-compute.job-failed',
  cancelled: 'scientific-compute.job-cancelled',
  resumed: 'scientific-compute.job-resumed'
} as const)
...
export function validateScientificJobBaselineTrace(
  ...
  return validateScientificTraceClosure(trace.events)
}
...
function createScenarioEvents(options: ScenarioEventOptions): ScientificTraceEvent[] {
  ...
  const traceStarted = eventId(options.jobId, 'trace-started')
  ...
  events.push(
    baseEvent(options, {
      eventId: traceStarted,
      type: 'TRACE_STARTED',
      actor: { type: 'system', id: 'scientific-compute' },
      payload: {
        jobId: options.jobId,
        scenario: options.scenario,
    ...
    baseEvent(options, {
      eventId: submitted,
      type: SCIENTIFIC_COMPUTE_TRACE_EVENT_TYPES.submitted,
      parentEventId: plan,
    ...
    baseEvent(options, {
      eventId: started,
      type: SCIENTIFIC_COMPUTE_TRACE_EVENT_TYPES.started,
  ...
  events.push(
    baseEvent(options, {
      eventId: failed,
      type: SCIENTIFIC_COMPUTE_TRACE_EVENT_TYPES.failed,
    ...
    baseEvent(options, {
      eventId: cancelled,
      type: SCIENTIFIC_COMPUTE_TRACE_EVENT_TYPES.cancelled,
  ...
  events.push(
    baseEvent(options, {
      eventId: resumed,
      type: SCIENTIFIC_COMPUTE_TRACE_EVENT_TYPES.resumed,
...
function createHumanInteractionScenarioEvents(options: ScenarioEventOptions): ScientificTraceEvent[] {
  const events: ScientificTraceEventInput[] = []
  const traceStarted = eventId(options.jobId, 'hci-trace-started')
  ...
  events.push(
    ...
    baseEvent(options, {
      eventId: finished,
      type: SCIENTIFIC_COMPUTE_TRACE_EVENT_TYPES.finished,
      parentEventId: monitored,
      actor: { type: 'scheduler', id: options.scheduler },
      payload: {
    ...
    traceTerminalEvent(options, eventId(options.jobId, 'hci-trace-completed'), acceptance, 'TRACE_COMPLETED', {
      status: 'completed'
    })
  )
...
function appendFinishedAttempt(
  ...
  const completed = eventId(options.jobId, metadata.attempt === 0 ? 'trace-completed' : `trace-completed-${metadata.attempt}`)
  ...
  events.push(
    ...
    traceTerminalEvent(options, completed, review, 'TRACE_COMPLETED', {
      status: 'completed',
      attempt: metadata.attempt
    })
  )
...
function appendDiagnosticClosure(
  ...
  const failed = eventId(options.jobId, 'trace-failed')
  ...
  events.push(
    ...
    traceTerminalEvent(options, failed, review, 'TRACE_FAILED', {
      status: 'blocked',
...
function traceTerminalEvent(
  options: ScenarioEventOptions,
  eventIdValue: string,
  parentEventId: string,
  type: 'TRACE_COMPLETED' | 'TRACE_FAILED' | 'TRACE_CANCELLED',
...
function defaultActorFor(type: ScientificTraceEventInput['type']): ScientificTraceActor {
  if (type.startsWith('scientific-compute.job-')) return { type: 'scheduler', id: 'local-fixture' }
