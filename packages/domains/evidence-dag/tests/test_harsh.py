...
from evidence_dag.extractor import (  # noqa: E402
...
from evidence_dag.llm import LLMCallError, StubLLM  # noqa: E402
...
class TestParseJsonRobustness(unittest.TestCase):
    ...
    def test_garbage_raises(self):
        with self.assertRaises(ExtractionOutputError) as raised:
            _parse_json("not json at all, no braces")
        self.assertEqual(raised.exception.code, "extractor_invalid_json")

    def test_empty_is_typed_not_json_decode_error(self):
        with self.assertRaises(ExtractionOutputError) as raised:
            _parse_json("")
        self.assertEqual(raised.exception.code, "extractor_empty_output")
        self.assertNotIsInstance(raised.exception.__cause__, json.JSONDecodeError)

    def test_non_object_is_typed(self):
        with self.assertRaises(ExtractionOutputError) as raised:
            _parse_json("[]")
        self.assertEqual(raised.exception.code, "extractor_invalid_shape")

    def test_non_text_is_typed(self):
        with self.assertRaises(ExtractionOutputError) as raised:
            _parse_json(None)
        self.assertEqual(raised.exception.code, "extractor_invalid_output_type")

    def test_extract_retries_malformed_output_once_with_repair_prompt(self):
        class RepairingLLM:
            def __init__(self):
                self.calls = []
        ...
        llm = RepairingLLM()
        graph = extract_dag(
            [{"id": "trace-1", "type": "message", "content": "No evidence."}],
            llm,
            "runtime:test",
        )
        ...
        self.assertEqual(llm.calls[0]["max_tokens"], 8192)
        self.assertEqual(llm.calls[1]["max_tokens"], 16384)
        self.assertIn(
            "extractor_invalid_json",
            llm.calls[1]["messages"][0]["content"],
        )

    def test_extract_stops_after_one_repair_attempt(self):
        class BrokenLLM:
            def __init__(self):
                self.calls = 0
...
class TestServerRobustness(unittest.TestCase):
    ...
    def _update_body(thread_id, *, watermark="w1"):
        return {
            ...
            "workspaceRoot": os.getcwd(), "projectRoot": os.getcwd(),
    ...
    def test_typed_llm_error_survives_service_result_and_status(self):
        ...
        thread_id = "typed-http-error"
        first_status, first = self._req("POST", "/updates", self._update_body(thread_id))
        self.assertEqual(first_status, 200)
        committed_digest = first["data"]["snapshot"]["digest"]
        engine.llm = FailedResponseLLM()
        try:
            failed_body = self._update_body(thread_id, watermark="w2")
            failed_body["trace"].append({
                "id": "step-3",
                "type": "message",
                "content": "New claim.",
            })
            failed_status, failed = self._req("POST", "/updates", failed_body)
        finally:
        ...
        status_code, status = self._req(
            "GET",
            f"/updates/status?threadId={thread_id}",
        )
        self.assertEqual(status_code, 200)
        self.assertEqual(status["data"]["error"], failed["error"])
        self.assertEqual(status["data"]["snapshot"]["digest"], committed_digest)

    def test_typed_extraction_error_survives_service_result(self):
        ...
        engine.llm = llm
        try:
            failed_status, failed = self._req(
                "POST",
                "/updates",
                self._update_body("typed-extraction-error"),
            )
        finally:
            engine.llm = original_llm
        ...
        self.assertEqual(failed_status, 503)
        self.assertEqual(llm.calls, 2)
        self.assertEqual(failed["error"]["code"], "extractor_empty_output")
        self.assertEqual(failed["error"]["type"], "ExtractionOutputError")
        self.assertEqual(failed["error"]["attempts"], 2)
        self.assertFalse(failed["error"]["retryable"])
        self.assertIsNone(failed["error"]["incompleteReason"])
        self.assertIsNone(failed["error"]["responseStatus"])
