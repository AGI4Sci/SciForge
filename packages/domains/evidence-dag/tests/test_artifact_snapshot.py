...
class TestArtifactRegistry(unittest.TestCase):
    def test_workspace_scope_key_is_internal_and_rejects_project_root_escape(self):
        with tempfile.TemporaryDirectory() as workspace, tempfile.TemporaryDirectory() as outside:
            storage = os.path.join(workspace, ".edag")
            engine = Engine(storage_dir=storage)
            first = engine._registry(workspace_root=workspace, project_root=workspace)
            second = Engine(storage_dir=storage)._registry(
                workspace_root=workspace, project_root=workspace,
            )
            self.assertEqual(first.path, second.path)
            with self.assertRaisesRegex(ValueError, "contained by workspaceRoot"):
                engine._registry(workspace_root=workspace, project_root=outside)
    ...
    def test_snapshots_are_immutable_idempotent_and_historical_audit_is_read_only(self):
        with tempfile.TemporaryDirectory() as workspace:
            ...
            repeated = engine.update(
                ...
                trace=None, workspace_root=workspace, project_root=workspace,
            )
    ...
    def test_content_change_keeps_assertion_identity_and_marks_old_path_stale(self):
        with tempfile.TemporaryDirectory() as workspace:
            ...
            resolved = engine.resolve_artifact(
                source_before.artifact_id or "", workspace_root=workspace,
