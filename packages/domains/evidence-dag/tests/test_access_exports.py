...
class EngineAccessExportTests(unittest.TestCase):
    ...
    def test_restricted_artifact_registry_commands_never_return_locations_or_acl(self):
        with tempfile.TemporaryDirectory() as workspace:
            engine = Engine()
            candidate = os.path.join(workspace, "CANARY-candidate.txt")
            with open(candidate, "w", encoding="utf-8") as handle:
                handle.write("private bytes")
            digest = "sha256:" + hashlib.sha256(b"private bytes").hexdigest()
            registered = engine.register_artifact(
                workspace_root=workspace,
                project_root=workspace,
                payload={
                    "kind": "dataset",
                    "locator": "CANARY-missing.txt",
                    "contentDigest": digest,
                    "version": "CANARY private registry version",
                    "mediaType": "CANARY/private-media-type",
                    "selector": {
                        "type": "text", "quote": "CANARY private selector",
                    },
                    "accessPolicy": {"read": True, "tenantAcl": ["alice"]},
                },
            )
            self.assert_clean(registered)
            self.assertTrue(registered["accessRestricted"])
            artifact_id = registered["artifact"]["artifactId"]
            resolved = engine.resolve_artifact(
                artifact_id,
                workspace_root=workspace,
                project_root=workspace,
                candidate_locators=["CANARY-candidate.txt"],
            )
            self.assert_clean(resolved)
            self.assertNotIn("accessPolicy", json.dumps(resolved))
            self.assertNotIn("locator", json.dumps(resolved).lower())
            scanned = engine.resolve_artifacts(
                workspace_root=workspace, project_root=workspace,
            )
            self.assert_clean(scanned)
            self.assertNotIn("workspaceRoot", scanned)
            self.assertNotIn("scope", scanned)
            self.assert_clean(engine.events(event_types=["ArtifactMoved"]))

            registry = engine._registry(
                workspace_root=workspace, project_root=workspace,
            )
            current = registry.versions[registry.artifacts[artifact_id].current_version_id]
            current.locator = "CANARY-old.txt"
            current.rebind_candidates = ["CANARY-candidate.txt"]
            rebound = engine.confirm_artifact_rebind(
                artifact_id,
                workspace_root=workspace,
                project_root=workspace,
                locator="CANARY-candidate.txt",
            )
            self.assert_clean(rebound)
            self.assertNotIn("locator", json.dumps(rebound).lower())
...
class HttpAccessExportTests(unittest.TestCase):
    ...
    def test_restricted_artifact_http_commands_project_registry_records(self):
        with tempfile.TemporaryDirectory() as workspace:
            candidate = os.path.join(workspace, "CANARY-http-candidate.txt")
            with open(candidate, "w", encoding="utf-8") as handle:
                handle.write("http private bytes")
            digest = "sha256:" + hashlib.sha256(b"http private bytes").hexdigest()
            status, registered = self.request("POST", "/artifacts", {
                "workspaceRoot": workspace,
                "projectRoot": workspace,
                "kind": "dataset",
                "locator": "CANARY-http-missing.txt",
                "contentDigest": digest,
                "version": "CANARY private HTTP registry version",
                "mediaType": "CANARY/private-http-media-type",
                "selector": {
                    "type": "text", "quote": "CANARY private HTTP selector",
                },
                "accessPolicy": {"allowedPrincipals": ["alice"]},
            })
            self.assertEqual(status, 200)
            self.assert_clean(registered)
            artifact_id = registered["data"]["artifact"]["artifactId"]
            status, resolved = self.request(
                "POST", f"/artifacts/{quote(artifact_id, safe='')}/resolve", {
                    "workspaceRoot": workspace,
                    "projectRoot": workspace,
                    "candidateLocators": ["CANARY-http-candidate.txt"],
                },
            )
            self.assertEqual(status, 200)
            self.assert_clean(resolved)
            self.assertNotIn("locator", json.dumps(resolved).lower())
            status, scanned = self.request("POST", "/artifacts/resolve", {
                "workspaceRoot": workspace, "projectRoot": workspace,
            })
            self.assertEqual(status, 200)
            self.assert_clean(scanned)
            self.assertNotIn("workspaceRoot", json.dumps(scanned))

            registry = self.engine._registry(
                workspace_root=workspace, project_root=workspace,
            )
            current = registry.versions[registry.artifacts[artifact_id].current_version_id]
            current.locator = "CANARY-http-old.txt"
            current.rebind_candidates = ["CANARY-http-candidate.txt"]
            status, rebound = self.request(
                "POST", f"/artifacts/{quote(artifact_id, safe='')}/confirm-rebind", {
                    "workspaceRoot": workspace,
                    "projectRoot": workspace,
                    "locator": "CANARY-http-candidate.txt",
                },
            )
            self.assertEqual(status, 200)
            self.assert_clean(rebound)
