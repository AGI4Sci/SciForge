...
function parseWatermark(raw: string): ParsedWatermark | null {
  ...
  // Artifact lifecycle receipts are independent durable obligations. A later
  // ordinary turn watermark must never make an unprocessed lifecycle page look
  // covered; only batches of the exact same receipt are comparable.
  if (base.includes(':artifact-lifecycle')) {
    return {
      family: `artifact-lifecycle:${base}`,
      sequence: 0n,
      discriminator: '',
