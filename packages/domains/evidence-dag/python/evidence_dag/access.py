...
_PUBLIC_LEVELS = {"public", "open", "unrestricted", "workspace"}
...
def policy_restricted(value: Any) -> bool:
    ...
    if not isinstance(value, dict):
        return True
    # The shared Artifact Versions domain owns this exact policy contract.
    # ``workspace`` and ``public`` are readable by a workspace-scoped Evidence
    # service; ``allowExport`` is an export permission and must not be confused
    # with read access. Keep malformed and restricted contracts fail-closed.
    if set(value) == {"visibility", "principals", "allowExport"}:
        visibility = value.get("visibility")
        principals = value.get("principals")
        allow_export = value.get("allowExport")
        if (
            visibility not in {"workspace", "restricted", "public"}
            or not isinstance(principals, list)
            or any(not isinstance(item, str) or not item.strip() for item in principals)
            or not isinstance(allow_export, bool)
        ):
            return True
