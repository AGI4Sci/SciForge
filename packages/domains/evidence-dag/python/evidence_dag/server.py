...
from .extractor import ExtractionOutputError
from .llm import LLMCallError, ModelRouterLLM
from .service import Engine, ReviewDecisionConflict, evidence_update_error
...
def err(code: str, message: str, *, retryable: bool = False,
        details: Optional[dict[str, Any]] = None, operation: str = "",
        request_id: str = "", started: str = "") -> dict:
    error = {"code": code, "message": message, "retryable": retryable}
    if details:
        error.update({
            key: details[key]
            for key in ("type", "attempts", "incompleteReason", "responseStatus")
            if key in details
        })
        ...
        try:
            ...
            if artifact_match:
                ...
                if artifact_match.group(2) == "resolve":
                    ...
                    result = self.engine.confirm_artifact_rebind(
                        artifact_id,
                        workspace_root=str(body.get("workspaceRoot") or ""),
                        project_root=body.get("projectRoot"),
            ...
            failure = evidence_update_error(exc)
            return self._send(503, err(
                failure["code"],
                failure["message"],
                retryable=failure["retryable"],
                details=failure,
                operation=action or "post",
                request_id=rid,
                started=started,
            ))
