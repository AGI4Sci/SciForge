...
class Engine:
                ...
                if incremental_trace:
                    if _is_canonical_execution_bundle(incremental_trace):
                        ...
                        lineage_delta = ingest_trace_lineage(
                            graph, incremental_trace, registry,
    ...
    def register_artifact(
        ...
        supplied_policy = payload.get("accessPolicy")
        if supplied_policy is not None and not isinstance(supplied_policy, dict):
            raise ValueError("accessPolicy must be an object when supplied")
    ...
    def resolve_artifact(
        ...
        return project_registry_result(result)

    def confirm_artifact_rebind(
        self, artifact_id: str, *, workspace_root: str,
        project_root: Optional[str], locator: str,
    ) -> dict[str, Any]:
        registry = self._registry(
            workspace_root=workspace_root, project_root=project_root,
        )
        result = registry.confirm_rebind(artifact_id, locator)
        return project_registry_result(result)
    ...
    def resolve_artifacts(
        ...
        restricted = any(
            self._registry_artifact_restricted(registry, artifact_id)
            for artifact_id in registry.artifacts
        )
        if not restricted:
            restricted = any(
                (graph := self.get(str(item.get("threadId") or ""))) is None
                or graph_restricted(graph)
                for item in affected
            )
        return project_registry_result(result, restricted=restricted)

    @staticmethod
    def _registry_artifact_restricted(
        registry: ArtifactRegistry, artifact_id: str,
    ) -> bool:
        artifact = registry.artifacts.get(artifact_id)
        if artifact is None:
            return True
        if policy_restricted(artifact.access_policy):
            return True
        if any(
        ...
        return any(
            ...
            for anchor in registry.anchors.values()
        )
        ...
        if isinstance(artifact_id, str) and artifact_id:
            ...
            for registry in self._registries.values():
                ...
                if artifact is None:
                    continue
                found = True
                if policy_restricted(artifact.access_policy) or any(
                    version.artifact_id == artifact_id
                    and availability_restricted(version.availability)
                    for version in registry.versions.values()
                ):
