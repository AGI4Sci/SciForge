...
const TEXT_BYTES = 2_000_000
const BINARY_BYTES = 4 * 1024 * 1024
...
async function readTextPrefix(file: WorkspacePreviewFileState) {
  const binary = await readBinaryPrefix(file, TEXT_BYTES)
...
> {
  ...
  if (fileInfo.size > BINARY_BYTES) {
    return { ok: false, reason: 'too-large', message: `File is ${fileInfo.size} bytes; lightweight worker observation limit is ${BINARY_BYTES} bytes.` }
  }
  const binary = await readBinaryPrefix(file, BINARY_BYTES)
  return { ok: true, bytes: binary.bytes, bytesRead: binary.bytesRead }
}

async function readBinaryPrefix(file: WorkspacePreviewFileState, maxBytes = BINARY_BYTES) {
  const fileInfo = await stat(file.path)
