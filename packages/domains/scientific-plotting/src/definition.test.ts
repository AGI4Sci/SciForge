...
test('manifest publishes governed Scientific Plotting provenance renderer contributions', () => {
  ...
  assert.deepEqual(SCIENTIFIC_PLOTTING_RENDERER_RESOURCE_NAVIGATION_CONTRACT, {
    resourceKinds: ['scientific-plot', 'scientific-plot-render-manifest'],
    target: {
      surface: 'right-panel',
      contributionId: 'scientific-plotting.workbench-right-panel'
    }
  })
})
