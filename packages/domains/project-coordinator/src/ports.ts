      ...
      try {
        generated = await agentExecution.run({
          ...
          prompt: [
            ...
            'Treat the Budget values as hard Cloud upper bounds, never as a requested task count; do not fill the budget. For an expert-team/design-review request, use explicitly listed Worker roles or sub-questions as the starting point for a human-reviewable draft decomposition; preferably recommend one focused Task per role or sub-question, and keep supporting deliverables, metrics, risks, actions, phases, and report sections in completionCriteria or rationale when they do not need separate ownership. This is a draft recommendation, not a fixed expert or task count: do not impose a cardinality limit or a one-role/one-Task invariant. The Human Coordinator reviews the Plan Draft and decides the final decomposition before submission, including any add, remove, combine, split, reorder, reassign, or boundary edits. If no roles or sub-questions are listed, propose a bounded draft decomposition for Human review instead of inventing experts; otherwise retain normal decomposition behavior.',
            'For a design-analysis-only text collaboration Task, include the literal marker design-analysis-only in its objective, say that the Worker analyzes only the assigned sub-question and does not execute or claim an experiment. Its completionCriteria must require Expert/Role and Sub-question, Conclusion, Evidence or basis with [expert:<role>] or [source:<label>] attribution, and Recommendation or next action; an optional uncertainty note is useful. The Coordinator final summary, not each Worker report, must consolidate the overall design, first Agent landing task, quantitative success metrics, hard constraints, risks, human confirmation points, and next actions. If the brief asks for a design/validation iteration, describe the proposed design sequence and label it unexecuted; never present simulation, experiment, or feedback as completed. If no file input or artifact is requested, set fileIntent to null.',
