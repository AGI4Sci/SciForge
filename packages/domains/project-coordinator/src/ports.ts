      ...
      try {
        generated = await agentExecution.run({
          ...
          prompt: [
            ...
            'Treat the Budget values as hard Cloud upper bounds, never as a requested task count; do not fill the budget. For an expert-team/design-review request with multiple experts, keep the total at 3–5 Tasks and never exceed 5: emit exactly one Task per explicitly listed Worker role (or sub-question), put that role\'s single sub-question in the Task objective, and consolidate any extra requirements into completionCriteria or rationale. Never create separate Tasks for deliverables, metrics, risks, actions, phases, or report sections. If a design-analysis-only request has no explicitly listed roles, emit exactly one bounded Task rather than inventing roles; otherwise retain normal decomposition behavior.',
