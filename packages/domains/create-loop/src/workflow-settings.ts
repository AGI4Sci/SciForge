...
function normalizeRunMode(value: unknown): 'agent' | 'plan' { return value === 'plan' ? 'plan' : 'agent' }
function normalizeScheduleReasoningEffort(value: unknown): 'off' | 'low' | 'medium' | 'high' | 'max' {
  return value === 'off' || value === 'low' || value === 'high' || value === 'max' ? value : 'medium'
}
function normalizeStatus(value: unknown): 'idle' | 'running' | 'success' | 'error' {
