...
Otherwise:
1. Discover every distinct plan-gated capability named by processingRecipe exactly once with includeSchema=true, plus dataset-api.prepare-plan and dataset-api.execute-plan exactly once each. Never infer operationRef values. Do not invoke the individual processing capabilities; their discovered input schemas are mandatory construction contracts for the immutable plan.
...
5. Invoke dataset-api.prepare-plan exactly once with confirmedByUser=true. The confirmed Loop specification authorizes this bounded preparation plan.
6. Invoke dataset-api.execute-plan exactly once with only the returned planId. Do not call individual plan-gated operations directly and do not retry a failed execution.
7. Require execution.status=succeeded, the execution step count to equal the number of plan-gated recipe steps, and every step status=succeeded. Any step that reports outputRecords=0 is a failed preparation outcome even if its status says succeeded; a join must also report at least one matchedLeftRecords. After execute-plan returns, call no more tools.
...
2. Convert incoming outputSchema deterministically into validationRules: one rule per field with field, type, and required, preserving the designed field order. Invoke dataset-api.prepare-plan with confirmedByUser=true. Build its operations and outputs from the exact blueprint below. Replace only the three angle-bracket values with incoming acceptedSamples, parentArtifacts, and validationRules. Do not add planId, placeholders, extra parameters, or alternate artifact names anywhere in the plan.
