...
- A **lab** groups targets, selects its isolated environment provider and configuration, and sets a
  lab concurrency limit. The management UI discovers registered VirtualBox machines through the
  package-owned read capability, presents their name and runtime state, and saves the selected
  canonical UUID. It also generates a stable gateway SSH alias from the VM name and UUID; raw UUID
  and alias editing remain available only under advanced settings. Docker labs record a container
  image.
...
1. Create the laboratory and select a discovered VirtualBox VM. UUID lookup and the default gateway
   alias are automatic; Docker and raw locator editing are explicitly labeled advanced options.
