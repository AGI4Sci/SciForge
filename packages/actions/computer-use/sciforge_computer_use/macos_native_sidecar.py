...
def _capabilities(payload: Mapping[str, Any], output_dir: Path) -> dict[str, Any]:
    ...
    return _result(
        ...
        refs=[ref],
        value={"capabilitiesRef": ref, "capabilities": capabilities, "bindingKind": "external-command"},
    )
...
def _discover(payload: Mapping[str, Any], output_dir: Path) -> dict[str, Any]:
    ...
    if not displays:
        return _blocked("discover", "macos_display_discovery_unavailable", output_dir, run_id)
    screens = _virtual_screens(displays, run_id, display_group_id, output_dir)
    windows = _windows_for_virtual_screens(_live_windows(), screens, run_id, output_dir)
    ...
    return _result(
        ...
        refs=[ref, *[str(screen.get("screenRef")) for screen in screens if screen.get("screenRef")], *[str(window.get("windowRef")) for window in windows if window.get("windowRef")]],
        value={"discoveryRef": ref, "discovery": discovery, "bindingKind": "external-command"},
    )
...
def _preflight(payload: Mapping[str, Any], output_dir: Path) -> dict[str, Any]:
    ...
    if platform.system() != "Darwin":
        blocked_reasons.append("macOS native sidecar requires Darwin.")
    if not shutil.which("screencapture"):
        blocked_reasons.append("screencapture is unavailable.")
    if _mapping(payload.get("inputModalityPolicy")).get("sharedSystemInputAllowed") is not False:
        blocked_reasons.append("shared system input must be disabled for M6.")
    if not allowed_window_refs:
        blocked_reasons.append("allowedWindowRefs must be discovery-backed and non-empty.")
    ...
    return _result(
        ...
        refs=[preflight_ref, isolation_ref],
        value={"permissionPreflightRef": preflight_ref, "isolationReportRef": isolation_ref, "bindingKind": "external-command"},
...
def _capture(payload: Mapping[str, Any], output_dir: Path) -> dict[str, Any]:
    ...
    if completed.returncode != 0 or not screenshot_path.is_file() or screenshot_path.stat().st_size <= 0:
        return _blocked("capture", "macos_screencapture_failed", output_dir, run_id, detail=(completed.stderr or completed.stdout).strip())
...
def _execute(payload: Mapping[str, Any], output_dir: Path) -> dict[str, Any]:
    ...
    event = {
        ...
        "permissionPreflightRef": payload.get("permissionPreflightRef"),
        "beforeEvidenceRefs": payload.get("beforeEvidenceRefs") or [],
        "groundingRefs": payload.get("groundingRefs") or [],
        "actionKind": action_kind,
        "target": target,
...
def _blocked(tool: str, reason: str, output_dir: Path, run_id: str, *, detail: str | None = None) -> dict[str, Any]:
    blocked_ref = _write_json(output_dir / "blocked" / f"{run_id}-{tool}-{_digest({'reason': reason, 'detail': detail})}.json", {
        "schemaVersion": "sciforge.computer-use.macos-native-sidecar-blocked.v1",
        "tool": tool,
        "runId": run_id,
        "reason": reason,
        "detail": detail,
    ...
    return _result(tool, "blocked", reason, refs=[blocked_ref], value={"blockedDiagnosticRef": blocked_ref}, diagnostic_only=True)
