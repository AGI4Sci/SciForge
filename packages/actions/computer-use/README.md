...
当前 active product backlog 以 `PROJECT_CU.md` 的 VirtualAppScreen 路线为准：VirtualAppScreen product gate、adapter-first background app control、annotation-to-proposal、background native window capture/action、research workflow/live acceptance matrix。BrowserRuntime 的 DOM/AX refs 只能作为 observation、target hint、freshness check、adapter source 或 verifier context；DOM/AX/Playwright 不得绕过 Computer Use 的 lease、before/after evidence 和 validator 单独证明完成。Docker/noVNC/RDP/M6 multi-screen 只保留为 legacy diagnostic、historical evidence、backend packaging 或 sidecar/ref 回归，不再写成 active product gate、并发模型或产品验收 owner。
