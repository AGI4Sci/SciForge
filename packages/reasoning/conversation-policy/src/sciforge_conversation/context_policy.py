"""Python compatibility bridge for runtime-owned turn context policy."""
...
def _repo_root() -> Path:
    for parent in Path(__file__).resolve().parents:
        if (parent / "package.json").exists() and (
            parent / "src/runtime/gateway/conversation-context-policy.ts"
        ).exists():
            return parent
    return Path.cwd()
...
import {{ {export_name} }} from './src/runtime/gateway/conversation-context-policy.ts';
...
"""
    ...
    if completed.returncode != 0:
        ...
        raise RuntimeError(f"conversation context policy bridge failed: {reason}")
...
def build_context_policy(request: Mapping[str, Any] | Any) -> JsonMap:
    result = _from_gateway(_payload(request), "buildConversationContextPolicy")
    if not isinstance(result, dict):
        raise RuntimeError("conversation context policy bridge returned a non-object payload")
    return result


def should_isolate_history(request: Mapping[str, Any] | Any) -> bool:
    return bool(_from_gateway(_payload(request), "shouldIsolateHistory"))


__all__ = ["build_context_policy", "should_isolate_history"]
