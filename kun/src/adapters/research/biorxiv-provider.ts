...
export class BiorxivResearchProvider implements ResearchSearchProvider {
  ...
  async search(request: ResearchSearchRequest): Promise<ResearchSearchProviderResult> {
    try {
      ...
      const collection = await this.loadIntervalPapers(interval, request)
      const papers = collection.filter((paper) => paperMatchesQuery(paper, request.query))
      return {
        papers: papers.slice(0, request.maxResults),
        webResults: [],
        diagnostics: [{
          id: this.id,
          enabled: true,
          available: true,
          resultCount: Math.min(papers.length, request.maxResults)
        }]
      }
    } catch (error) {
      return {
  ...
  private async loadIntervalPapers(
    interval: string,
    ...
    for (let cursor = 0; cursor <= MAX_CURSOR; cursor += PAGE_SIZE) {
      ...
      papers.push(...parseBiorxivPapers(json))
    ...
    this.intervalCache.set(interval, papers)
    return papers
  }
}
