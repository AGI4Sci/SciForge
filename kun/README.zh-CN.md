...
SciForge Runtime 是 SciForge 的本地 HTTP/SSE 代理运行时。它为 GUI 提供稳定、类型化且 GUI 友好的代理循环合约：
...
在 SciForge 里，Runtime 不是把模型回复包一层 UI，而是一条本地 agent loop：
它可以长期携带项目上下文、稳定调用工具、恢复会话，并在桌面、写作、手机连接
和定时任务之间复用同一套执行链路。

SciForge Runtime 的核心目标是提高每一个 token 的 ROI。它会尽量让 token 花在用户需求、
...
| `--model-router-base-url` | 本地 Model Router `/v1` base URL | `http://127.0.0.1:3892/v1` |
...
SciForge Runtime 也可以在无 GUI 的情况下独立运行：
...
SciForge Runtime 使用 JSON 配置文件管理运行时行为，避免重建后重配或硬编码参数。
...
`serve.apiKey` 是本地 Model Router runtime key，不是上游供应商凭据。
...
- `serve.runtimeTuning.executionGovernance` 会抑制同一回合内重复的相同工具调用，阻止无意义 tool loop 继续烧 token。
...
Skill 目录必须包含显式 `skill.json`（包含 `id`、`description`、prompt 与 tool 白名单等），以便提高加载和诊断确定性。旧目录如果只有 `SKILL.md`，需要补 manifest 后才会加载。

推荐迁移步骤：

1. 保留现有 `SKILL.md`。
2. 在同目录新增等价的 `skill.json`。
3. 重启 SciForge Runtime 或刷新诊断。
4. 当 `/v1/runtime/tools` 正常报告该 Skill 且没有 validation error 后，即完成 manifest-only 迁移。
...
- `kun run`/`kun chat`/`kun exec` 报错：核对 `--config`、`--data-dir`、`--api-key`、`--model-router-base-url`、`--runtime-token` 一致；先用 `kun exec --list-tools --json` 检查工具注册表。
- 功能显示为 disabled：通常表示配置标志为 false；显示为 unavailable 通常表示标志为 true，但 provider/存储/模型未就绪或初始化失败。

## GUI 集成

GUI 主流程由 `local-runtime-process.ts` 启动 SciForge Runtime，主进程通过中性 `AgentRuntime` adapter 持有 bearer token 并访问活动 runtime URL，渲染层只使用 `window.sciforge.agentRuntime`。`agents.sciforge` 下保存 `binaryPath`、`port`、`autoStart`、`runtimeToken`、`dataDir`、`model`、`approvalPolicy`、`sandboxMode`、`insecure`；上游供应商凭据与成员 base URL 保存在 Model Router 设置中。
...
历史 provider 专属设置不再属于当前运行链路；GUI 只保存一个本地 runtime 配置，并通过 Model Router 处理凭据与上游路由。
