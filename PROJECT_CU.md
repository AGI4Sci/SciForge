...
- [ ] P9b/P9c：补充 visual focused-editor verifier：利用 SciForge 视觉 / screenshot evidence 证明 active editor group、caret 或可写编辑区焦点，生成 tokenized visual verifier refs；视觉证据仍只归 Host / acceptance verifier 使用，不进入 Computer Use core planning，也不能泄漏 raw screenshot / provider payload。
...
本轮补充：`focus-editor` 现在也有 current VSCode live diagnostic 的 Host-side runner 和 primitive port 单元覆盖。Host producer 从 `bind` / `observe` refs 选择一个 `Command+1` key act；Computer Use primitive service 只执行该原子 key action，返回 action / executor / input / after observe refs，并在 release 时释放 scoped input lease / adapter / cursor、恢复前台 app 和鼠标位置。runner 只有在 after observe 产生 `focused-editor:` ref 时才生成 `verifier:...:focus-editor` 并 completed；真实当前 VSCode 诊断已执行该 key act 但 after observe 仍缺少 focused-editor evidence，因此正确 blocked，没有打字、保存、杀 VSCode 或清 profile。最新补充：focused-editor observe verifier 已支持保守复合 AX 证据，接受 Monaco/code-editor/editor-group context，拒绝 terminal/search/explorer/source-control 等非编辑目标；unit tests 证明 Monaco-like `AXGroup` 可生成 focused-editor ref，而 integrated terminal 的 `AXTextArea` 不会被误认。真实当前 VSCode 在复合 AX 后仍 blocked，说明本机 VSCode/Monaco 未向 macOS AX 暴露足够 focused editor evidence；下一步卡点是补 visual focused-editor verifier，而不是放宽 P9c 写入 gate。
