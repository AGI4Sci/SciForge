...
- [x] P9b：默认 Agent Host Computer Use Act materializer 已增加 current VSCode co-work Host producer 单元路径；它从 Host / runtimeTruth 已有的 tokenized target、observation、permission refs 组装 `CU-NEXT-09 current-vscode-cowork` runtime intent，先支持 `read-visible-text` refs-only observe decision；多窗口候选时可用唯一 observe/window/frontmost refs 自动选定 target window，证据仍冲突时才 `needs-confirmation`；不走通用 WindowAction planner，不让 Computer Use core 做 task planning；act primitive live runner 未接入时仍 fail closed。
