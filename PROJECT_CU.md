...
- [x] P9b/P9c：真实当前 VSCode focus-editor live diagnostic 在复合 AX verifier 后仍安全 blocked：Host 选择并执行了一个 `Command+1` key act，action / executor / input refs 与 release / front-app / mouse restoration refs 已产生，未解析 draft、未打字、未保存、未杀 VSCode、未清 profile；本机 VSCode 的 after observe 仍缺少可用 focused-editor AX evidence，因此 runner 正确 blocked，不能作为 P9c 写入 live-diagnostic 完成证据。
...
- [ ] P9b/P9c：补真实 Host focused-editor evidence provider：可利用 SciForge 视觉、screenshot、AX、text、image、editor/action refs 或其它观察能力证明 active editor group、caret 或可写编辑区焦点，并生成 tokenized `focused-editor:` / `verifier:...:focus-editor` refs；证据来源不必限定为视觉，但仍只归 Host / acceptance verifier 使用，不进入 Computer Use core planning，也不能泄漏 raw screenshot / provider payload。
...
本轮补充：`focus-editor` 现在也有 current VSCode live diagnostic 的 Host-side runner 和 primitive port 单元覆盖。Host producer 从 `bind` / `observe` refs 选择一个 `Command+1` key act；Computer Use primitive service 只执行该原子 key action，返回 action / executor / input / after observe refs，并在 release 时释放 scoped input lease / adapter / cursor、恢复前台 app 和鼠标位置。runner 的局部 focus completion 由 Host-owned evidence verifier 决定：默认只接受 after observe 原生 `focused-editor:` ref，注入式 verifier 可以用 AX/text/image/editor/action refs 或 SciForge 视觉等观察能力生成安全 `focused-editor:` / `verifier:...:focus-editor` refs；unsafe/raw verifier 输出会 blocked。真实当前 VSCode 默认诊断已执行该 key act 但 after observe 仍缺少足够 focused-editor evidence，因此正确 blocked，没有打字、保存、杀 VSCode 或清 profile。最新补充：focused-editor observe verifier 已支持保守复合 AX 证据，接受 Monaco/code-editor/editor-group context，拒绝 terminal/search/explorer/source-control 等非编辑目标；unit tests 证明 Monaco-like `AXGroup` 可生成 focused-editor ref，而 integrated terminal 的 `AXTextArea` 不会被误认。真实当前 VSCode 在复合 AX 后仍 blocked，说明本机默认观察 provider 没有暴露足够 focused editor evidence；下一步卡点是接入真实 Host evidence provider，而不是放宽 P9c 写入 gate。
...
本轮补充：P9b/P9c focused-editor 证明已从“只能依赖 observe 原生 `focused-editor:` / 视觉 verifier”改成 Host-owned evidence verifier。`runVSCodeCoWorkFocusEditorLiveDiagnostic` 会把 before observe、Host-selected `focus-editor` act、after observe、target/editor/action refs 交给 `focusedEditorEvidenceVerifier`；默认 verifier 仍只接受 observe 中的 `focused-editor:` ref，注入式 Host verifier 可以使用 SciForge 视觉能力、AX/text/image/editor/action refs 或其它 refs-first 观察能力，只要输出安全 `focused-editor:` 与 `verifier:...:focus-editor` refs。unsafe/raw verifier 输出会被 blocked，且不泄漏 raw screenshot path、data URL、provider payload 或 URL。current VSCode wrapper 已透传该 verifier。真实默认 current VSCode focus diagnostic 仍保守 blocked：primitive chain 为 `bind -> observe -> host-decision -> act -> observe -> control(release)`，lease / adapter / cursor 已释放，front app / mouse restoration refs 已保留，但当前默认 provider 没有给出足够 focused-editor evidence，因此不能宣称 focus completed。
