...
- [x] P9b/P9c：真实当前 VSCode focus-editor live diagnostic 已安全跑到 blocked：Host 选择并执行了一个 `Command+1` key act，action / executor / input refs 与 release / front-app / mouse restoration refs 已产生，未解析 draft、未打字、未保存、未杀 VSCode、未清 profile；after observe 仍缺少 `focused-editor:` evidence，因此 runner 正确 blocked，不能作为 P9c 写入 live-diagnostic 完成证据。
