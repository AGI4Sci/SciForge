...
- [x] P9b：完成第一个无写入 live-diagnostic：显式 env-gated `tools/current-vscode-cowork-readonly-live-acceptance.ts --activate-vscode --json` 在真实当前 VSCode 上跑通 `bind -> observe -> Host decision -> read-visible-text -> observe -> control(release)`，final answer / manifest 给出 refs-first evidence，且仍固定 `live-diagnostic` / `productReady=false`。
...
- [x] P9b：unit tests 证明可复用 Host live producer 能从 `bind` / `observe` primitive 返回的当前 VSCode refs 构造 `current-vscode-cowork` Host input、runtimeTruth 和 ready preflight；producer 只识别窄 `read-visible-text` / `insert-draft` 意图，operation 显式写入 Host target refs，后续 materializer 仍只选一个 refs-first primitive，不从 fallback commandText 或 Computer Use core 规划任务。
...
- [x] P9b：真实 read-only CLI acceptance 已在当前 Mac 上通过；manifest status=`passed`、primitive chain=`bind -> observe -> host-decision -> observe -> control(release)`、Host finalAnswer=`completed` 且 `computerUseCorePlanning=false` / `userTaskCompletionClaimed=false`，release refs 覆盖 scoped input lease / adapter / cursor，restore refs 覆盖 front app / mouse position，运行后前台 app 恢复为 Codex，且未杀 VSCode / 未清 profile / 未泄漏 raw path、base64、secret-like、provider payload 或 product-ready claim。
...
本轮补充：P9b read-only live acceptance manifest writer 已达到 `unit-proven`。`runCurrentVSCodeCoWorkReadonlyLiveAcceptance` 默认只在缺少显式 env 时写 blocked manifest，不启动、不聚焦、不关闭 VSCode；显式 env 或注入 runner 时只运行 `read-visible-text` 路径，并把 runner result 归一为 manifest：`bind -> observe -> host-decision -> observe -> control(release)`、Host final answer、Host producer refs、decision/observation/text/image/AX/editor/freshness refs、input lease / adapter / cursor release refs、front-app / mouse-position restore refs 都必须是 tokenized refs。manifest 固定 `userProfileUsed=true`、`sharedSystemInputUsed=true`、`productReady=false`、`userVSCodeKilled=false`、`userProfileCleared=false`，并过滤 raw path、URL、base64、secret-like、provider payload。该 writer 是 P9b 真实 live run 的验收承载，不等于真实桌面已跑通。

本轮补充：P9b read-only live acceptance 现在有固定 CLI：`node --import tsx tools/current-vscode-cowork-readonly-live-acceptance.ts --json`。默认无 env 时只写 blocked manifest；显式真实运行使用 `SCIFORGE_COMPUTER_USE_VSCODE_COWORK_LIVE_DIAGNOSTIC=1 node --import tsx tools/current-vscode-cowork-readonly-live-acceptance.ts --activate-vscode --json`。`--activate-vscode` 是验收入口的显式 opt-in，只在唯一 VSCode 窗口条件下机械激活 VSCode，bind 前捕获前台 app / 鼠标，并在 release 后恢复；多窗口或证据不足仍 blocked。CLI 只调用现有 manifest writer，不新增 Computer Use core planning、不启动 VSCode、不清 profile、不宣称 product-ready，并把 artifact 固定在 `docs/test-artifacts/current-vscode-cowork-readonly-live/manifest.json`。

本轮真实验收：P9b read-only live acceptance 已在当前 Mac 上以显式 env + `--activate-vscode` 跑通。manifest 为 `passed`，但 maturity 仍是 `live-diagnostic`、`productReady=false`、`userProfileUsed=true`、`sharedSystemInputUsed=true`；证据链包含 `bind -> observe -> host-decision -> observe -> control(release)`、Host final answer、Host producer refs、decision/observation/text/image/AX/editor/freshness refs、scoped input lease / adapter / cursor release refs、front-app / mouse-position restore refs；运行后前台 app 恢复为 Codex，未杀用户 VSCode、未清用户 profile，manifest 未包含 raw path、base64、secret-like、provider payload 或 product-ready claim。该结果证明 read-only CLI live-diagnostic 已完成，不代表普通聊天端到端 co-work 或 P9c 写入 live-diagnostic 已完成。
...
当前状态：P9a 已达到 `unit-proven`，P9b ordinary-chat Host input bridge、窄普通文本 route hook、ordinary/native route -> current VSCode live diagnostic runner hook、ordinary route Host producer evidence projection、route-level VSCode activation opt-in 透传、Agent Host current VSCode co-work producer、可复用 Host live producer、Host-side read-visible-text live diagnostic runner、Host final-answer evidence envelope、current VSCode observe-only primitive ports 和 Host wrapper、read-only live acceptance manifest writer、read-only live acceptance CLI，以及 P9c insert-draft primitive/Host runner、current VSCode insert-draft wrapper、默认 refs-first type executor 均达到 `unit-proven`；P9b read-only CLI 已达到真实当前 VSCode `live-diagnostic`，但普通聊天端到端 co-work `live-diagnostic` 与 P9c 真实当前 VSCode 单文件写入 `live-diagnostic` 仍未完成，P9 仍不能声明 `product-ready`。已完成的 P9a/P9b/P9c unit coverage 覆盖 policy / acceptance-controller contract、native-route bridge、ordinary route live runner hook sanitizer、Host producer evidence route projection、read-only live acceptance manifest writer / CLI、narrow ordinary text hook、route-level VSCode activation opt-in 透传、live producer observe-ref -> Host input/runtimeTruth/preflight、window bind identity refs gate、operation allowlist gate、target-file ambiguity gate、mixed raw/refs-first observe evidence gate、observe active-session refs gate、editor visibility gate、editor element target refs gate、cursor movement refs gate、draft text refs gate、selection replacement refs gate、real-file refs-first target/session/permission envelope gate、multi-window unique observe refs target selection、Host final-answer evidence refs、current VSCode primitive `act` evidence refs、insert-draft Host runner primitive chain、default text-ref resolver / type executor fail-closed refs、live-manifest bind/target refs validator、live-manifest bind input-resource refs validator、live-manifest editor-element target refs validator、Host decision observe-context refs validator、Host decision active-session refs validator、Host decision action refs validator、Host decision permission refs validator、before observe target-window refs validator、before observe target-file refs validator、before/after observe active-session refs validator、after observe target/freshness refs validator、after observe target-file refs validator、after observe editor-element refs validator、control release/restoration refs validator、control active-session refs validator、action session refs validator、action target-window refs validator、action target-file refs validator、action permission refs validator、action input/stale evidence validator、action release input-resource binding refs validator、before/after visual refs validator 和 before/after observe visual binding refs validator。旧 real-file approval / confirmation gate 已替换为 Host full-access permission envelope gate，并有 package / native-route focused tests 覆盖。下一步优先把普通聊天端到端链路接到这条真实当前 VSCode read-only live diagnostic，随后在 P9c 执行一个小范围 insert/replace/save 诊断动作：每一步只给 Computer Use 一个 primitive，释放 input lease / cursor / adapter、恢复焦点和鼠标位置，最后由 Agent Host 基于 refs-first 证据生成 final answer。P8 只证明临时 workspace/test file 的 VSCode 诊断验收；P9 live producer 才证明用户已打开 VSCode 的普通聊天 co-work。
