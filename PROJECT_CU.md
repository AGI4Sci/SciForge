...
本轮补充：P9 live manifest 的 approval evidence 现在必须绑定同一个 selected `file-ref:` 和本次 bind active session。真实文件保存、撤销或替换等 confirmation evidence 不能只携带 `risk:` 和 `approval:` refs；还必须直接包含本次 Host 选择的文件目标与 active session，否则 validator 返回 `missing-approval-ref:target-file` 或 `missing-approval-ref:active-session`。

本轮补充：P9 Host-side controller / native-route bridge 的真实文件 `confirmationRef` 现在也必须绑定目标 `file-ref:` 和当前 active session。保存、撤销、替换选区、批量替换或跨文件修改只有在 approval token 同时绑定 `riskActionHash`、当前 observe session 和当前 selected/唯一 visible `file-ref:` 时才会返回 ready；只绑定 risk/file 的旧 approval 会保持 `needs-confirmation`，不返回 primitive/action。
