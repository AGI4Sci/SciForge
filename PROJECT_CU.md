...
本轮补充：P9b read-only live acceptance 现在有固定 CLI：`node --import tsx tools/current-vscode-cowork-readonly-live-acceptance.ts --json`。默认无 env 时只写 blocked manifest；显式真实运行使用 `SCIFORGE_COMPUTER_USE_VSCODE_COWORK_LIVE_DIAGNOSTIC=1 node --import tsx tools/current-vscode-cowork-readonly-live-acceptance.ts --json`。CLI 只调用现有 manifest writer，不新增 Computer Use core planning、不启动 VSCode、不清 profile、不宣称 product-ready，并把 artifact 固定在 `docs/test-artifacts/current-vscode-cowork-readonly-live/manifest.json`，作为下一步真实当前 VSCode read-only live-diagnostic 的验收入口。
