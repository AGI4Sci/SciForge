...
当前目标：把 Computer Use 收敛为 **VirtualAppScreen：本地原生 app 的后台控制会话**。SciForge 打开右侧 `Screen` 时，必须默认创建或 attach 一个真实运行的本地 app screen；app 运行在 agent-owned virtual display / app surface 中，右侧结果栏显示 live frame，Computer Use 和人类都通过同一个 input lease / control plane 介入。当前路线只接受“本地原生 app + 平台 virtual display/app surface provider”这一条真相源；没有 fallback viewer 或 shell shortcut。

本文件只记录 Computer Use 的当前原则、任务板、TODO 和验收规则。总项目原则以 [`PROJECT.md`](PROJECT.md) 为准；详细设计参考 [`docs/VirtualAppScreenArchitecture.md`](docs/VirtualAppScreenArchitecture.md)、[`packages/actions/computer-use/vision_computer_use_agent_mvp.md`](packages/actions/computer-use/vision_computer_use_agent_mvp.md) 和 [`docs/NativeExtensionOwnershipMap.md`](docs/NativeExtensionOwnershipMap.md)。
...
当前产品观察：右侧结果栏已经有 `Screen` pane 和 `virtual-screen-viewer` 空态，但打开产品页后仍可能显示 `VirtualAppScreen attach state: no-session`。这说明 contract / viewer 基线已存在，真正缺的是默认创建本地 app session、provider attach、live frame、输入 lease、人类介入和当前会话 evidence 的产品闭环。

旧的 contract / fixture / readiness 工作已移入“完成归档”。下面登记当前会直接影响“在 SciForge 右侧虚拟屏幕自由使用本地 app”的活跃任务；已完成子项必须保留日期、evidence refs、验证命令和最终状态。注意：没有真实本地原生 `VirtualDisplayProvider` 当前会话证据时，只能记录 `blocked` / `permission-missing` / diagnostic contract，不能声称 user-level pass。

### P0：默认可见、真实运行、可控制

- [ ] P0-CU-VAS-AUTO-OPEN：Screen pane 默认自动打开真实虚拟屏幕。
  验收：用户打开右侧 `Screen` pane 或新建一个 screen window 时，SciForge 自动创建或 attach 一个真实运行的 `VirtualAppScreen`。默认 profile 优先使用低风险本地 app（首选 VSCode/editor profile）；成功时 UI 进入 `attached` 并显示当前 live frame；失败时进入 `permission-missing`、`adapter-unavailable`、`observe-only` 或 `blocked`，不能长期停留在 `no-session` placeholder。
  当前状态：部分完成。右侧 `Screen` 打开时已生成 refs-first activation / permission-handoff payload，默认 placeholder 会先 request attach/probe，只有明确 authorization-incomplete 才进入 handoff；产品 runtime 已能接收 `/computer-use screen attach` 并在无 executor/dry-run 时返回 refs-first fail-closed `VirtualAppScreen` artifact；注册 runtime-owned executor 返回完整 evidence 时，产品 runtime 会 materialize `attached` live refs，并要求 provider-owned `surfaceTransport` evidence 才能进入 live；macOS / Linux Xpra / Windows IDD opt-in driver hook factory 已落地，产品 runtime 已把 env opt-in 和 generic target-app env config 接到 bootstrap；native executor 已强制 provider lifecycle 同 current-run/session/surface/sequence，driver/provider shell 已要求 hook-owned refs 和 `providerEvidenceWritten=true`。真实 create/launch/attach/readFrame 仍需当前机器 provider/permission evidence 才能声明通过。
  TODO：
...
- [x] P0-CU-VAS-PROVIDER-ONE-TRUTH：本地原生 `VirtualDisplayProvider` 是唯一真相源。
  验收：当前产品只接受本地原生 app + 平台 virtual display / app surface provider。macOS 走 CGVirtualDisplay / ScreenCaptureKit / `node-mac-virtual-display` 类 bridge；Linux 走 Xpra app/window session；Windows 走 IDD virtual display driver。`serve-web`、code-server、OpenVSCode、Xvfb/noVNC、RDP、QEMU/VM、shell-only runner 和旧截图不能作为 fallback pass。
  当前状态：完成。Provider availability 通过 readiness/probe refs 表达；缺 provider、缺 driver 或缺 permission 只能 `blocked` / `permission-missing`，不能 fallback pass。
  TODO：
  ...
  验证：`node --import tsx --test src/runtime/computer-use/virtual-display-provider.test.ts`；`npm run smoke:virtual-app-screen-user-acceptance-contract --silent`；`npm run smoke:cu-next-live-acceptance --silent`；状态：provider contract/gate landed, native provider availability remains environment-dependent。

- [ ] P0-CU-VAS-VSCODE-CLOSED-LOOP：首个本地 VSCode 闭环。
  验收：在 agent-owned virtual display / app surface 中启动 VSCode，右侧 `Screen` 显示 live frame；Computer Use 发送一次鼠标/键盘 `InputIntent`；UI 和 manifest 返回 before/after frame refs、executor event refs、verification refs 和 bounded evidence ledger。不能通过 shell 直接写文件或旧截图伪造完成。
  当前状态：部分完成。VSCode/editor profile、smoke scaffold、provider lifecycle evidence 和 macOS native provider-executed smoke 已存在；缺 SciForge 右栏产品路径中“自动启动 VSCode -> Screen 展示 live frame -> 输入 -> before/after evidence”的 user-level 闭环，manifest 仍保持 `userAcceptanceEligible=false` 直到 product-path evidence 齐全。
  TODO：
...
- [ ] P0-CU-VAS-RIGHT-PANE-LIVE-BINDING：右侧结果栏绑定 live frame。
  验收：`Screen` pane 默认展示当前 active `VirtualAppScreen` 的 live frame、actor cursors、annotation overlay、lease state、blocked/permission state、timeline 和 refs。`Results` / `Screen` / workspace focus 切换不能丢 session；多个 screen window 要展示各自真实运行的 app surface。
  当前状态：部分完成。Right pane 已能消费当前 run 的 Screen payload、保留 host-owned live refs 和 `surfaceTransport` refs、显示 blocked/permission/observe-only state，并在 Screen tab activation 时发 `gui.present` / activation refs；host adapter 只通过 provider-owned presentation bridge 绑定 live surface；runtime/UI reconnect contract 已能复验同一 provider session，不创建第二个 truth source。Session manager 已 mint `liveBindingAttachGrantRef`，runtime artifact / right pane model / host attach request / reconnect command 均透传并要求该 grant；Model/host 已降级缺 platform driver ready、permission ready、providerExecuted、provider owner/reconnect、session-manager attach grant、safe surfaceTransport descriptor、ledger 或 current sequence 的 live-looking artifacts。真实 native media/data channel、多 screen 真实平台 evidence、host bridge 对 grant record 的实际 dereference/validation 和可操作刷新率仍需平台 evidence。
  TODO：
...
- [ ] P0-CU-VAS-EVIDENCE-LEDGER：所有动作都有当前会话 evidence refs。
  ...
  - [ ] SciForge 右栏产品 live provider path 仍需在执行时自动 materialize before/after frame 文件，frame 可以滞后但不能缺失或来自旧 session。
  - [ ] Generic native provider shell 仍需验证 provider-owned evidence 超过 hook 布尔声明：读取或验证 ledger entry existence/hash/current-run ownership，或把 shell-only hook evidence 明确限制为 contract-only，直到真实 driver writer 被 runtime/ledger 复验。
...
- [ ] P1-CU-VAS-STREAM-QUALITY：低延迟 live transport 收口。
  验收：主链路优先 WebRTC 或等价 native frame stream；右侧 `Screen` 达到本地 app 可操作的刷新率和输入回显，断线可重连，frame stream 不成为第二真相源。
  当前状态：部分完成。Provider -> Screen transport/telemetry/input-hot-path contract 已落地；runtime attached artifact 会携带 safe `surfaceTransportDescriptor` 和 session-manager `liveBindingAttachGrantRef`，Screen host adapter 已能 refs-only attach/present/detach provider-owned surface。真实 native media/data channel、host bridge grant validation、reconnect 测量和可操作刷新率仍需 provider evidence。
  TODO：
  ...
  - [ ] 真实产品 UI transport 仍需连接 native provider media/data channel，并测量 reconnect 与可操作刷新率。
...
- [x] P1-CU-VAS-APP-PROFILES：扩展本地科研 app profiles。
  验收：在 VSCode/editor 闭环后，逐步覆盖 Terminal、Browser research、Jupyter/notebook、PDF/Preview/Zotero、CSV/table viewer。每个 profile 都必须复用同一 `VirtualDisplayProvider` 和 Screen presentation chain。
  当前状态：完成为 diagnostic profile contract；没有 real provider evidence 时不得 `userAcceptanceEligible=true`。
  TODO：
...
- [ ] P1-CU-VAS-DOGFOOD：用 SciForge 自己验收右侧 Screen。
  验收：用 SciForge 当前产品 UI 打开 workspace，右侧 `Screen` 自动出现真实 app screen；Computer Use 能在其中操作本地 app，人类能接管；run 输出 bounded manifest。
