...
- [ ] P9b/P9c：补默认/真实 Host focused-editor evidence provider 实现和真实桌面验收：基于 SciForge 可用视觉、screenshot、AX、text、image、editor/action refs 或其它观察能力证明 active editor group、caret 或可写编辑区焦点，并生成 tokenized `focused-editor:` / `verifier:...:focus-editor` refs；证据来源不必限定为视觉，但仍只归 Host / acceptance provider/verifier 使用，不进入 Computer Use core planning，也不能泄漏 raw screenshot / provider payload。
