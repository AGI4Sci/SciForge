...
- [x] P9c：新增 current VSCode `insert-draft` live acceptance manifest writer；默认缺少显式 env 或缺少安全 `draftTextRef` 时只写 blocked manifest，不触发真实桌面、不接收 raw draft；注入 runner 时验证 `bind -> observe -> host-decision -> act -> observe -> control(release)`、Host producer evidence、action/executor/input/stale refs、release input lease / adapter / cursor refs、front app / mouse restoration refs，并固定 `live-diagnostic` / `productReady=false` / 不杀 VSCode / 不清 profile。
- [x] P9c：新增固定 CLI `tools/current-vscode-cowork-insert-draft-live-acceptance.ts`；参数只接受 `--draft-text-ref text-ref:...`，默认写 `docs/test-artifacts/current-vscode-cowork-insert-draft-live/manifest.json` blocked manifest，真实写入诊断必须显式设置 `SCIFORGE_COMPUTER_USE_VSCODE_COWORK_LIVE_DIAGNOSTIC=1` 并由 Host resolver 在私有侧解析草稿正文；CLI summary 和 manifest 不公开 raw draft、raw path、provider payload、base64、secret-like 或 product-ready claim。
...
本轮真实验收：P9b read-only live acceptance 已在当前 Mac 上以显式 env + `--activate-vscode` 经 ordinary/native route 跑通。manifest 为 `passed`、`ordinaryChatNativeRouteUsed=true`，但 maturity 仍是 `live-diagnostic`、`productReady=false`、`userProfileUsed=true`、`sharedSystemInputUsed=true`；证据链包含 `bind -> observe -> host-decision -> observe -> control(release)`、Host final answer、Host producer refs、decision/observation/text/image/AX/editor/freshness refs、scoped input lease / adapter / cursor release refs、front-app / mouse-position restore refs；未杀用户 VSCode、未清用户 profile，manifest 未包含 raw path、base64、secret-like、provider payload 或 product-ready claim。该结果证明 read-only ordinary/native route CLI live-diagnostic 已完成，不代表完整 HTTP/SSE 普通聊天端到端 co-work、P9c 写入 live-diagnostic 或 product-ready adapter gap 已完成。

本轮补充：P9b read-only HTTP/SSE acceptance writer / CLI 已达到 `unit-proven`。`runCurrentVSCodeCoWorkReadonlyHttpSseAcceptance` 默认在缺少 `SCIFORGE_COMPUTER_USE_VSCODE_COWORK_LIVE_DIAGNOSTIC=1` 时只写 blocked manifest，`httpSseTransportUsed=false` / `adapterBoundaryUsed=false`，不启动、不聚焦、不关闭 VSCode；显式 env 或注入 adapter 时启动本地 Runtime Codex HTTP/SSE endpoint，POST 普通聊天 request，并从 SSE public events 中验收 `realtime_session`、`turn`、`done`、Host producer refs、Host-owned final answer、decision / observation / text / image / AX / editor / freshness refs、input lease / adapter / cursor release refs 和 front-app / mouse-position restore refs。固定 CLI 为 `node --import tsx tools/current-vscode-cowork-readonly-http-sse-acceptance.ts --json`；默认 blocked manifest 已通过 cleanup / readiness 验收。该 harness 证明 HTTP/SSE transport contract 和 public event sanitizer，不等于真实当前 VSCode HTTP/SSE live pass，也不给 Computer Use core 增加 planning、窗口选择、文件选择或 final-answer 权限。
