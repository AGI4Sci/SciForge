...
- [x] P9c：unit tests 证明 current VSCode live primitive ports 已接入 refs-first `act`：Host 给定单个 atomic `type` action 和 `text-ref:` draftTextRef 后，port 返回 actionRef、executorEventRef、inputEventRef、before/after observation refs、stale invalidation refs、input adapter / cursor / scoped lease refs，并在 `control(release)` 后释放 lease / adapter / cursor、保留 front app / mouse restoration refs；无 action executor hook 时 fail closed。
