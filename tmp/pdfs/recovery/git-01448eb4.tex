...
\begin{figure}[ht!]
\centering
\includegraphics[width=0.95\textwidth]{figures/ai4ai-delta-pcc-v2.png}
\caption{Delta PCC improvement across AI4AI design iterations for cell perturbation prediction. The chart shows progressive gains from iterative algorithm-atom recombination on PBMC, Replogle, and Tahoe datasets under a NeMo/BioNeMo training stack.}
...
\textbf{Planned Enhancements.} The current prototype motivates several concrete experimental enhancements to strengthen the reproducibility claim: (1)~\textbf{Multi-dataset validation}---extending reproduction to additional spatial transcriptomics datasets (e.g., DLPFC, Mouse Brain) to test generalizability beyond Visium breast cancer; (2)~\textbf{Multi-method benchmarking}---comparing MCFST against alternative spatial domain methods (e.g., SpaGCN, STAGATE, GraphST) under a unified evaluation protocol; (3)~\textbf{Pre-registered verification contract}---defining the selection rule for best-N runs and the success threshold before execution to eliminate post-hoc selection bias; (4)~\textbf{Component ablation}---isolating the contribution of each graph view, the adversarial regularizer, and the mutual-information edge-pruning through systematic removal; (5)~\textbf{Statistical rigor}---reporting confidence intervals and significance tests (e.g., Wilcoxon signed-rank) across all 20 random seeds rather than reporting only the best and selected-5 means; (6)~\textbf{GPU-parity reproduction}---re-running the pipeline on the original GPU target to quantify the performance gap introduced by the Apple Silicon adaptation; and (7)~\textbf{Sensitivity analysis}---evaluating ARI stability under perturbations of PCA dimensions, mutual-information thresholds, and the number of graph views. These enhancements would transform the current feasibility demonstration into a rigorous, third-party-auditable reproduction benchmark.
