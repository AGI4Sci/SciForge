...
\begin{itemize}
    \item \textbf{Native scientific object processing with translate-then-reason:} end-to-end file ingress exposes four classes---protein sequences (.faa; .fasta/.fa with per-record conservative content classification), protein structures (.pdb/.cif/.mmcif), molecules (.smi/.smiles), and single-cell transcriptomics (via C2S worker contract)---routed through domain-expert translators (Esm2Text~\cite{abdine2023prot2text}, Prot2Text~\cite{abdine2023prot2text}, BioT5~\cite{pei2024biot5}, and C2S~\cite{rizvi2025c2s}) that return structured expert observations before the main agent reasons. The translator output is an evidence candidate, not a verified scientific fact; the human remains responsible for judgment.
...
\begin{figure*}[ht!]
    ...
    \caption{SciForge system framework. Six research capability patterns share a common evidence-aware control chain---ingress, translation, execution, evidence capture, audit, and release---built on the layered architecture of interaction surfaces, core engine, scientific model router, and local-first infrastructure.}
    \label{fig:sciforge_framework}
\end{figure*}

\subsection{User Interaction and Collaboration (Layer~1)}
Layer~1 exposes three interaction surfaces driven by the same underlying runtime. The \textbf{desktop application} provides a thin GUI reserved for human judgment---reviewing evidence, annotating figures, comparing candidate artifacts, and approving release decisions---while execution runs in the background. \textbf{Group chat integration} (Zulip, Discord, WeChat, Feishu) enables remote task submission, status polling, and lightweight approvals without switching context. \textbf{Mobile supervision} offers read-only monitoring of long-running experiments and review tasks, with interaction gated through the same governance model.
...
\noindent\begin{tabular}{@{}p{2.5cm} p{13.2cm}@{}}
...
\subsubsection{Multi-Level Evaluation Framework}

Current evaluation metrics measure task completion rather than scientific discovery quality. We propose a multi-level evaluation framework for future work:
\begin{enumerate}
    \item \textbf{Level 1---Task Completion.} Standard task-success metrics (accuracy, success rate, tool-call correctness) to ensure functional correctness of individual agentic steps and workflow execution.
    \item \textbf{Level 2---Scientific Novelty.} Assessment of whether agent-generated hypotheses, designs, or analyses introduce non-trivial scientific contributions beyond known results, evaluated through domain-expert blind review and comparison against literature baselines.
    \item \textbf{Level 3---Experimental Verifiability.} Measurement of whether agent outputs can be independently reproduced and experimentally validated in wet-lab or computational settings, including third-party re-execution of recorded workflows.
\end{enumerate}
Each level builds on the previous: reliably completing tasks (Level~1) is a prerequisite for generating novel contributions (Level~2), which in turn must be verifiable (Level~3) to constitute genuine scientific progress. Domain-expert blind review should supplement automated metrics at Levels~2 and~3.

\subsubsection{Planned Systematic Evaluation Program}

The following evaluation categories are explicitly identified as \textbf{future work}, with concrete protocols designed to address the current evidence gaps:

\begin{itemize}
    \item \textbf{Systematic benchmark comparisons.} Quantitative head-to-head comparisons against at least 2--3 alternative scientific agent systems (e.g., ChemCrow, Coscientist, PaperQA2) on standardized task corpora, controlling for LLM backbone, using the same evaluation protocol, and reporting statistical significance. Appendix~\ref{tab:capability_differentiation} currently provides a qualitative capability-level comparison; quantitative dimensions will be added in follow-up work.
    
    \item \textbf{Component-level ablation studies.} Isolating the contribution of individual SciForge components including: translator-based modality routing versus raw text input, Evidence DAG provenance tracking versus no provenance, multi-agent delegation versus single-agent baselines (majority voting, iterative self-reflection), self-correction mechanisms (with trigger frequency, success rate, and failure-mode analysis), retrieval-augmented generation versus closed-book reasoning, and different LLM backbones. Each ablation variant will be evaluated with statistical significance testing.
    
    \item \textbf{Statistical rigor and sample size.} Current use-case demonstrations employ limited sample sizes without reported standard deviations or confidence intervals. Future evaluations will use at least 30 independent cases per domain (or bootstrap confidence intervals where data are scarce), pre-registered selection criteria to avoid confirmation bias, and reporting of both successful and failed cases with quantitative metrics.
    
    \item \textbf{Inter-annotator agreement and expert-consensus studies.} Measuring agreement among domain experts on audit findings, evidence quality assessments, and scientific novelty judgments, using structured annotation protocols and Cohen's kappa or equivalent metrics.
    
    \item \textbf{Time-to-completion and researcher-efficiency studies.} Controlled protocols comparing researcher time, decision quality, and artifact completeness with and without SciForge assistance on standardized research tasks.
    
    \item \textbf{Scalability benchmarks.} Stress-testing the system under long-running multi-day agentic loops, measuring provenance capture overhead, Evidence DAG growth, and audit-sidecar performance.
    
    \item \textbf{Toolchain robustness.} Fault-injection experiments where 10--30\% of external tool calls (AlphaFold, MD simulation, literature search) are artificially failed, measuring recovery rate, error propagation, and degradation characteristics.
\end{itemize}

We encourage the community to contribute protocols, benchmark datasets, pre-registered evaluation designs, and third-party reproducibility studies that exercise these dimensions on the SciForge platform.
...
The current implementation has several concrete limitations, organized by category below.

\textbf{Ingress scope:} end-to-end file ingress is restricted to four modality classes (protein, protein\_structure, molecule, single-cell); Cell2Sentence (C2S) provides translate-then-reason for single-cell transcriptomics (.h5ad files), the same ingress pattern as the other three translators; VCF, BED, GFF, MGF, and additional omics formats are unsupported and fail closed. Future work includes expanding scientific modality support to crystal structures, mass spectrometry, cryo-EM, and additional omics formats.

\textbf{Scientific validation:} translator observations are evidence candidates, not verified scientific facts; no systematic validation against ground-truth annotations has been performed. \textbf{Self-correction validation:} the self-correction mechanism lacks independent validation; trigger frequency, success rate, precision/recall, and failure-mode analysis have not been measured, and whether self-correction introduces new errors is unknown.

\textbf{Memory model:} research memory stores scoped free-text records rather than a typed scientific state store; typed goals and decisions belong to the Project DAG.

\textbf{Evidence completeness:} the Evidence DAG schema can represent complete provenance but depends on runtime capture; software versions, parameters, environment metadata, log/output, and random seeds are not automatically populated by all execution paths.

\textbf{IM governance:} IM-based coordination is partially integrated; annotation, task transfer, and DAG-decision approval through chat are not end-to-end validated.

\textbf{Evaluation and experimental rigor:} no baselines, ablation studies, user studies, or third-party reproductions have been conducted. Current use-case demonstrations employ limited sample sizes without reported standard deviations or confidence intervals. Case study selection criteria have not been pre-registered, and whether selection was post-hoc or uses a consistent protocol is not documented; future evaluations should use pre-registered selection criteria with at least 30 independent cases per domain, bootstrap confidence intervals where data are scarce, and systematic reporting of both successful and failed cases. Multi-agent performance gains have not been measured against single-agent baselines such as majority voting or iterative self-reflection, so the incremental benefit of multi-agent coordination over additional inference compute remains unquantified.

\textbf{Methodological completeness:} complete prompt templates, full tool API parameters with version numbers, precise compute resources and wall-clock runtimes, LLM temperature and sampling strategies, and all code repository links are not fully documented in this manuscript; a detailed Supplementary Information document providing these details is planned.

\textbf{Data leakage risk:} LLM training data may include test-set protein structures, molecular properties, or scientific literature used in evaluations. No data contamination probing experiments have been performed, and no comparison between known (pre-training-cutoff) and unknown (post-cutoff, e.g., newly deposited PDB structures) task performance has been conducted. Closed-book baselines (LLM performance without tool access) should be reported to quantify potential memorization effects. Future work should design contamination detection protocols, use PDB structures deposited after the LLM training cutoff date, and compare known versus unknown task performance differences.

\textbf{Toolchain robustness:} SciForge depends on multiple external tools (e.g., AlphaFold, molecular dynamics simulation, literature search APIs). No fault-injection experiments have been conducted to measure system behavior when these external tools return errors, time out, or produce malformed outputs. Systematic tool-failure injection (10--30\% artificial failure rate) and recovery-rate measurement are needed.

\textbf{MCFST contract:} the verification contract has unresolved conflicts (pre-selection rule, run-count mismatch) and the 0.05 threshold is post-hoc; formal reproduction requires revision and third-party re-execution.

\textbf{Cell-atlas provenance:} the cell-atlas pipeline is a specification without execution artifacts; the heatmap is schematic, and \texttt{case45\_gate.py} has not been exercised.

Future work includes expanding scientific modality support to crystal structures, mass spectrometry, cryo-EM, and additional omics formats; implementing baselines, ablation studies, and systematic evaluations with statistical rigor; designing data contamination detection protocols; conducting toolchain fault-injection experiments; strengthening IM governance integration; improving provenance capture automation; validating self-correction mechanisms; and developing pre-registered verification contracts with third-party re-execution protocols.
...
The current implementation provides existence proofs of the core architecture but requires systematic quantitative evaluation. Key areas for future work include: benchmark comparisons against alternative systems with statistical rigor; component-level ablation studies including self-correction, multi-agent, and modality routing variants; data leakage and contamination analysis for LLM-based scientific reasoning; toolchain fault-tolerance characterization; pre-registered case-study selection with adequate sample sizes; and multi-level evaluation that captures not only task completion but also scientific novelty and experimental verifiability. We invite the community to contribute benchmarks, protocols, and third-party re-execution studies that exercise these dimensions on the open-source SciForge platform.
...
\begin{table}[ht!]
\caption{Qualitative capability positioning based on reviewed public materials. This table provides a feature-level comparison; systematic quantitative benchmark comparisons against alternative systems are identified as future work in Section~\ref{sec:system-architecture} and the Evaluation section. Typical-approach entries summarize common documented emphases and do not prove absence of unlisted features.}
...
& Provider-agnostic routing across OpenAI, Anthropic, DeepSeek, and local models with cost-aware fallback chains; localizable deployment preserves data sovereignty \\
Scientific Pipeline
& Convert scientific objects into agent-readable evidence
& Raw scientific text input or manual preprocessing
& Translate-then-reason with four C2S modality experts (protein, structure, molecule, single-cell); unrecognized formats fail closed with diagnostics \\
Agent Runtime
& Coordinate agents, tools, commands, and long tasks
& Single-session chat with limited delegation
& Research-aware runtime with sub-agent delegation, MCP workers, plan recovery, and cross-session task continuation \\
Workflow Engine
& Automate repeatable research protocols
& Scripts, cron jobs, and manual runbooks
& DAG-based workflows combining code, agent, tool, schedule, event, and approval nodes; natively integrated with the agent runtime \\
Research Memory
& Preserve project state across sessions
& Chat history or document RAG
& Structured Project DAG with confidence-tagged, timestamped records; typed goals and formal decisions persist as first-class DAG nodes \\
Evidence Governance
& Connect claims, evidence, audit findings, and approvals
& Outputs detached from data, scripts, and review history
& Thread-scoped Evidence DAGs with automatic provenance capture; scheduled audit runs; quality gates and configurable approval boundaries \\
Research Capability Patterns
& Expose human judgment points
& Generic chat and file views
& Human-in-the-loop judgment patterns (artifact review, approval, visual inspection) grounded in Evidence DAGs and audit records \\
\bottomrule
\end{tabular}
\end{table}
\clearpage
