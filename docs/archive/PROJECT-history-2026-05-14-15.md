# PROJECT History 2026-05-14/15

最后更新：2026-05-16

Archive/historical note: 本文件从 `../../PROJECT.md` 归档 2026-05-14/15 的旧 CAP/PKG/GT/PSM/MEM/H022 长段。它只保留问题来源、完成证据和历史上下文；未完成项不得直接驱动实现。当前实现入口只认 `../../PROJECT.md` 的 SA-* 任务板和 `../SciForge-SingleAgent-Architecture.md`。

旧任务处理规则：

- CAP/PSM/MEM/H022 中未完成或仍有价值的内容已迁移到当前 SA-GATEWAY、SA-CONTEXT、SA-KERNEL、SA-RUNTIME、SA-UI、SA-CONF 或 SA-WEB 条目。
- 无法迁移的历史 TODO 视为 archive/historical closed；后续不得为了完成它们新增兼容层、旧 prompt builder、旧 local memory、旧 preflight/direct-context 可见阶段或 projectionless UI fallback。
- 如需引用本文件，只能作为 evidence/source lineage，并在当前 SA-* 条目中重新写明文件、验证命令、失败边界和拒绝的兼容方案。
...
- [x] PSM-ARCH-01（archive/historical）：原 `ProjectSessionMemory.md` 专项稿已并入 [`../SciForge-SingleAgent-Architecture.md`](../SciForge-SingleAgent-Architecture.md)，最终 contract 定义 append-only ledger、content-addressed refs/blob store、context projections、AgentServer orchestration、backend handoff packet 和 KV cache-aware projection compiler。
- [x] PSM-ARCH-02：更新 [`../Architecture.md`](../Architecture.md)，将旧“AgentServer owns memory, SciForge owns projection”边界升级为“workspace owns canonical truth, AgentServer owns orchestration, backend consumes projection”。
- [x] PSM-ARCH-03：更新 [`../README.md`](../README.md)，把 `SciForge-SingleAgent-Architecture.md` 设为项目级多轮 runtime contract，并清理旧专项文档入口。
