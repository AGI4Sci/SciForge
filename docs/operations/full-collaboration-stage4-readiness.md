...
| 9.3 | Push only `codex/full-collaboration-loop-recovery` to the personal Fork; no upstream PR without all current-scope gates and later confirmation | Branch and origin are correct | Independent `ls-remote` matched pushed HEAD `03aae9155eec87e8d859ef0ed0e9e8b8fa74e3ce` before this source-only evidence update | Packaging is deferred and no artifact publication occurred | Public edge still selects the old stack; candidate remains detached and no upstream PR was created | Git push completed | None | Re-verify remote HEAD after each later evidence-only update | `passed_local` |
...
The retained U0 profile previously had a real OIDC User and active Device, a
configured local Runtime, and a live-attested OpenContent connection. Its current
OIDC session is expired and it has no Agent, so Human re-login from the exact
source commit is required before the immediate post-cutover Agent/WSS smoke.
