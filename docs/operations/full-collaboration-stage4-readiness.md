...
7.1 is therefore complete, while the first mutating boundary remains 7.2.
...
The separately authorized read-only receipt closes the hidden topology portion
of 7.1; neither observation performs or satisfies backup/restore, candidate or
cutover work in 7.2–7.4.
...
3. Preserve the completed 7.1 read-only baseline, then present an exact bounded
   7.2 backup/isolated-restore plan and obtain separate mutation authorization.
