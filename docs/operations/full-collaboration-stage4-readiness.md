...
| 7.4 | Switch only the existing Caddy `cloud-test` upstream after all candidate gates; retain exact rollback target | Existing edge contract is documented; no second issuer/DNS/listener is allowed | Four separately approved attempts proved the same Edge revision/mount/public 200/200/401/issuer selection and exact old-stack rollback; attempt 4 proved exact-source Identity, the same Device, the sole U0 Agent, Collaboration connected and a durable real Project create, then exposed a Host-broker success-envelope defect | Formal packaging is deferred and is not a source-app cutover prerequisite | Old Caddy upstream is selected and candidate is again isolated with restart `no`; the attempt-4 Project is retained in candidate DB, but the Desktop rejected the post-write result and never rendered Project focus or Coordinator counts | The retained U0 profile has real User/Device/Agent state; its process is stopped and a new public cutover approval is required after a clean pushed source correction | Live U0-U4 profiles remain required after the U0 Coordinator smoke | Correct the generic changed-resource contract and add a real broker-path regression test; then obtain a new explicit cutover approval and prove Project focus plus Coordinator online User/Agent counts without duplicating the retained Project | `approval_required` |
