...
| 7.4 | Switch only the existing Caddy `cloud-test` upstream after all candidate gates; retain exact rollback target | Existing edge contract is documented; no second issuer/DNS/listener is allowed | Both separately approved attempts proved the same Edge revision/mount/public 200/200/401/issuer selection and exact old-stack rollback; attempt 2 also proved the public packaged Identity recovers the same Device | Attempt 2 exposed and repaired a stale renderer phone-endpoint prerequisite before the first Agent could be registered; one new sealed clean/pushed artifact must pass packaged preflight before another approval request | Old Caddy upstream is selected and candidate is again loopback-only with restart `no`; both failed packaged gates rolled back before any Agent/Project/Provider write | A new exact artifact receipt and a separate third cutover approval are required | Live profiles remain required after U0 smoke | Seal the repaired artifact, prove its no-endpoint registration control, then present the unchanged server selection/rollback packet for fresh approval | `approval_required` |
| 8.2 | Repository architecture principles gate: no central feature map, Host-private/domain switch, compatibility/dual registration or hard-coded acceptance/provider behavior; same-package backend/UI; source and packaged composition | Standard manifests/generated composition, generic Domain SDK contracts, package-owned Collaboration/Coordinator/Content Space entrypoints; canonical changed-path/formal architecture gate and sealed Stage 4 receipt are implemented | Runtime-authorization, optional-skill, package/receipt/generator/manifest/tarball and changed-path gates pass locally | Formal gate still requires one exact clean/pushed artifact receipt and independently extracted packaged executable; no static verification-profile or optional supplier overlay is required | None | None | None for automated gate | Post-commit source and sealed packaged executable evidence from one unchanged commit | `ready_local` |
...
- one optional Provider supplier overlay, `opencontent-attachment-assets`
  ...
  file and Team paths do not consume it, and the base Stage 4 artifact must be
  built with this optional overlay absent;
...
and `3/3` generic private-skill installer tests, then audited 408 paths from the frozen baseline, including 142 production source
files and all 27 domain packages, with zero findings. The separate production
marker/private-import scan and the 421-file secret audit also passed. This closes
...
the Connector revalidates that binding before dispatch. The Stage 4 artifact no
longer waits for or embeds a static verification profile. It still requires the
reviewed private Provider deployment configuration, and the build-issued receipt
rechecks the applicable private composition layers after canonical after-pack
validation.
...
1. Rerun the local/source gates, verify Provider use without an Agent skill, and
   finish one sealed packaged artifact with the reviewed private Provider
   deployment configuration and runtime authorization intact.
...
path architecture gates, but it still requires a newly sealed exact artifact
and a new explicit cutover approval. Therefore 7.4 remains
