...
| 7.4 | Switch only the existing Caddy `cloud-test` upstream after all candidate gates; retain exact rollback target | Existing edge contract is documented; no second issuer/DNS/listener is allowed | Three separately approved attempts proved the same Edge revision/mount/public 200/200/401/issuer selection and exact old-stack rollback; attempt 3 additionally proved source-app Identity, the same Device, the sole U0 Agent and Collaboration connected | Formal packaging is deferred and is not a source-app cutover prerequisite | Old Caddy upstream is selected and candidate is again loopback-only with restart `no`; attempt 3 failed before Project creation and then restored the exact old public state | The retained U0 profile has the real User/Device/Agent state, but its process is stopped and a new public cutover approval is required | Live U0-U4 profiles remain required after the U0 Coordinator smoke | Push and freshly clone the approval repair, re-run the unchanged preflight, obtain a new explicit cutover approval, then create the real U0 Project and verify Coordinator online User/Agent counts | `approval_required` |
