...
该门禁对本变更新增或修改的生产路径逐字执行以下冻结要求：**不得编辑 central feature map、Host 只能依赖通用 SDK、不得保留兼容 shim/双注册、不得写 showcase/provider/domain 硬编码、backend/UI 同包版本，以及 source/packaged 两条 composition 都必须验证。** 它直接运行 changed-path generated byte-freshness、capability governance、Host/package 静态边界以及 source/packaged Electron smoke；不会接受调用方提供的“已通过”JSON。全仓模式只输出历史债务报告；除非某项直接阻断 changed collaboration path，否则不阻塞本交付，也不授权顺手重构。
