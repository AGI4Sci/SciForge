...
当前版本预期分别输出 `success` 和 `7`。schema v5 为远程审批记录增加交互模式字段，并将既有
记录标记为 `command_v1`；新建 reaction 卡片显式写入 `reaction_v1`，两者不会交叉匹配。schema v6
为托管 Channel 增加 Desktop installation 作用域；schema v7 增加手机原生私人 Channel 的短期发现证明与
安装级独占 claim，同一 Zulip 用户在每台电脑上也不能复用已被其他安装占用的 Channel。迁移失败时
保持服务停止，先回滚或修复，不能跳过版本检查
