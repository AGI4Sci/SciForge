...
`vision-sense` 兼容 id 的新真相源位于 [`../packages/observe/vision/README.md`](../packages/observe/vision/README.md)。旧 `packages/senses` 不再作为新增能力落点；若必须保留 alias，应在 package manifest 或 adapter README 写明删除条件，并保证 `smoke:no-legacy-paths` 基线不增加。
...
runtime 会在 [`../src/runtime/runtime-ui-manifest.ts`](../src/runtime/runtime-ui-manifest.ts) 中做组件绑定和兼容投影：

- scenario override 默认组件优先。
- 当前 turn 显式选择组件其次。
- 已登记的兼容规则只用于旧会话投影；新增 renderer 不能依赖 prompt 文案匹配获得选择权。
...
- [ ] 找到源码真相源，避免复制旧文档里的字段。
- [ ] 对照 `docs/legacy-cutover-inventory.md`，确认没有新增 UI 语义 fallback、prompt/scenario/provider 特例或 legacy facade。
