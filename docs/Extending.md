...
能力分为 5 类，类型真相源在 [`../src/shared/capabilityRegistry.ts`](../src/shared/capabilityRegistry.ts)：

- `observe`：sense，例如 vision。
- `reasoning`：skill/tool，例如 AgentServer generation。
...
当前 UI handoff 会把 scenario override 中的 `selectedSkillIds`、`selectedToolIds`、`selectedSenseIds`、`selectedActionIds`、`selectedVerifierIds`、`selectedComponentIds` 传给 runtime；Python conversation-policy 会进一步生成能力摘要和 handoff plan。

## Sense Contract

Sense ABI 真相源是 [`../src/shared/senseProvider.ts`](../src/shared/senseProvider.ts)。稳定字段：
...
- Sense 可以被主 agent 多次窄化调用。

`vision-sense` 是当前样板包，见 [`../packages/senses/vision-sense/README.md`](../packages/senses/vision-sense/README.md)。
...
Verifier runtime ABI 真相源是 [`../src/runtime/runtime-types.ts`](../src/runtime/runtime-types.ts)，runtime gate 与 normalization 真相源分别是 [`../src/runtime/gateway/verification-policy.ts`](../src/runtime/gateway/verification-policy.ts) 和 [`../src/runtime/gateway/verification-results.ts`](../src/runtime/gateway/verification-results.ts)。handoff payload 里只保留轻量 policy snapshot，见 [`../src/shared/agentHandoffPayload.ts`](../src/shared/agentHandoffPayload.ts)。
...
- `packages/*` 放可复用 capability、renderer、contract、skill 和 tool。它们不能 import `src/ui/src/**` 或 `src/runtime/**` 私有文件；共享类型先放到 `packages/runtime-contract`、`packages/scenario-core` 或包自己的 public export。
- `src/runtime/**` 放 workspace server、gateway、task runner、policy 和 runtime-only adapters。需要暴露给 package 或 UI 的稳定契约应提升到 package 或 shared handoff contract，不让 package 反向依赖 runtime 私有模块。
