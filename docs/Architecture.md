...
- 多轮对话的权威事实来源是 workspace 本地 append-only Project Session Ledger；AgentServer 负责 context orchestration、retrieval、compaction 和 backend handoff；SciForge 不把完整聊天历史当 prompt 记忆，只把 ledger/ref/blob store 投影成当前 turn 所需的 bounded context packet。详见 [`ProjectSessionMemory.md`](ProjectSessionMemory.md)。
