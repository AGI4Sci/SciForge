...
- 多轮对话记忆的权威来源是 AgentServer session/current-work 机制；SciForge 不维护第二套独立记忆系统，只向 AgentServer 投递当前 turn 意图、refs、证据边界和恢复策略。
...
- AgentServer 负责 session memory、current work、recent turns、context compaction 和多轮恢复；SciForge 只生成 `contextPolicy`、`contextEnvelope`、current refs、digest 和 audit refs 这些 handoff 投影。
...
- Agent backend / AgentServer：负责用户意图理解、多轮指代、session/current-work 记忆、上下文压缩、能力选择、任务规划、胶水代码生成、artifact 内容读取、失败诊断、继续执行和修复。
...
- bounded memory projection、current reference digest、artifact index 和 clickable refs；这些字段只描述本轮 handoff 应暴露什么，不保存或替代 AgentServer 记忆。
...
因此，未来不应继续在 conversation-policy 模块里扩大 keyword intent、任务 workflow、UI 文案或本地长期记忆。若需要新增行为策略，应进入 harness callback/profile；若需要新增能力，应进入 capability manifest/package；若需要多轮记忆能力，应优先接入或扩展 AgentServer 的 session/current-work/compaction contract。
...
- Context projection：artifact index、failure history refs、current references、current reference digests、context compaction signals。长期 session memory 归 AgentServer，不作为 SciForge 本地 capability 重新实现。
...
多轮对话的长期事实来源是 AgentServer session/current-work 与 workspace refs，而不是 SciForge 本地自然语言历史。用户说“你还记得一开始的问题吗”“给我 markdown 报告”“继续刚才那个结果”“按失败原因修一下”时，backend 应先复用 AgentServer 记忆，再通过通用工具读取事实：
...
- `contextPolicy`：声明是否允许 AgentServer 复用 current work、recent turns、run summary 和 compacted session state；该字段是 AgentServer 记忆机制的开关，不是 SciForge 本地记忆副本。
...
AgentServer context endpoint 是多轮记忆和压缩的优先入口：

- dispatch 前读取 `/api/agent-server/agents/{agentId}/context`，获取 session/current-work snapshot、context-window state 和 compacted turn refs。
- context-window 接近上限时优先调用 AgentServer compact API；SciForge handoff slimming 只作为传输降级，不接管长期记忆。
- fresh turn 使用 fresh agent scope，避免旧 session 污染；continuation/repair 使用稳定 agent scope，让 AgentServer 能命中自己的 recent turns、current work 和 compacted summaries。
...
会话策略的主路径是 Python package，默认开启。它的职责是把 UI/session/runtime facts 编译成本轮 handoff 策略，而不是保存长期对话记忆：
...
如果 Python policy 失败，runtime 会发出 `conversation-policy` failed event，并继续用 transport-only fallback；这保证策略层问题不会阻塞普通运行。fallback 仍必须遵守“AgentServer owns memory, SciForge owns projection”的边界。

## Context 与恢复

SciForge 不把完整历史和大文件无界塞进 backend，也不复制 AgentServer 的会话记忆。当前 turn 优先使用显式 refs、bounded digest、artifact index、最近 projection summary 和 audit refs；多轮指代、早期问题回忆和 current-work continuity 优先交给 AgentServer session/context 机制。大内容只通过 ref、digest、preview 和 checkpoint 进入上下文。

恢复策略包括：

- context-window preflight 和 handoff slimming。
- AgentServer context snapshot / compact API 优先；handoff slimming 只处理传输预算。
