...
## 最终形态：Backend-first Capability Architecture

SciForge 的目标形态是 **Backend-first, Contract-enforced, Capability-driven**：
...
最终 cutover 的旧链路清单维护在 [`legacy-cutover-inventory.md`](legacy-cutover-inventory.md)，`smoke:no-legacy-paths` 负责冻结 UI 语义 fallback、provider/scenario/prompt 特例和 legacy facade/re-export 基线。该清单只说明还被容忍的兼容面、下一步 owner 和删除证据；只有源码迁移完成并同步降低 guard baseline 后，才可以更新 PROJECT 里的完成状态。
...
- validation failed 或需要修复时，才展开 full schema、examples、repair hints 和相关日志 refs。
...
短期稳定服务时，优先做最少但改变边界的事情：

1. 增加 backend artifact resolver/read/render/resume 工具。
2. 让 SciForge request payload 稳定传 artifact descriptors、object refs、failure records 和 capability brief。
3. 加强 completed contract：计划句、空 artifact、无效 refs、schema 不合格都进入 repair loop。
4. 将 backend 输出 materialize 为稳定 `.sciforge/task-results` refs。
5. 降级 UI/场景层 natural-language fallback，只保留诊断、显示和安全边界。
6. stream 断开时先 poll/resume backend run，只有 backend 明确失败才追加 failed run。
7. 用少量高价值多轮 smoke 覆盖 report 追问、上一轮 artifact 继续处理、失败后修复。
...
User turn
  ...
  -> compatibility adapter for selected observe/action capability, if required
