...
- With local ignored config plus `SCIFORGE_PROXY_PORT=3891`, `SCIFORGE_MODEL_ROUTER_PORT=3891`, `SCIFORGE_WORKSPACE_PORT=6173`, and `SCIFORGE_WORKSPACE_WRITER_URL=http://127.0.0.1:6173`, `npm run smoke:computer-use-chat-live-preflight --silent` is now `ready`: service checks pass, `runtimeProviderPreflight.status=ready`, and `checkedInference.category=ready`.
- `npm run smoke:computer-use-chat-live-e2e:product-strict --silent` now submits the ordinary Desktop chat request (`requestSubmitted=true`) but still fails as `expected-completed-got-repair-needed`. The current run does not yet produce the required live acceptance bundle, ordinary Desktop product shell classification, WindowActionSession/BrowserHost target, display/screen/cursor/user-control/native-isolation/action-ledger/replay/validator refs.
- The ordinary chat request now carries CU task-map target metadata (`recommendedTargetMode`, `recommendedTargetApp`, `recommendedMaxSteps`, and `desktop-file-save` marker for CU-NEXT-08) through Runtime Codex host intent, UI sanitizer, native route sanitizer, and Computer Use planner acceptance contract.
- An opt-in TextEdit WindowActionSession bridge now exists for ordinary-chat-shaped TextEdit local save tasks. It creates runtimeTruth target/observation/permission refs and injects the existing WindowActionSession/Appium Mac2 materializer. It only enables when Appium Mac2 env gates and an absolute local artifact path are present.
- This machine currently has no Appium Mac2 service listening at `http://127.0.0.1:4723/status`, so the opt-in bridge cannot be used as live release evidence here yet. Without those gates, product-strict safely falls back to the package bridge and remains failed rather than claiming T1 complete.
- The broader historical fixture suites `tests/smoke/computer-use-chat-live-e2e.test.ts` and `tests/smoke/computer-use-chat-live-complex-matrix.test.ts` still have unrelated expectation drift around diagnostic/product-path classification; targeted T1 bridge/metadata tests are used as implementation evidence, not release completion evidence.
