...
- P9 面向用户已打开的 VSCode / IDE co-work：Host 可以根据当前 run 的 observe refs、用户选择的 window refs 和风险 envelope 决定下一步原子 primitive；window candidate 必须带 app/process/window/title/frontmost refs，缺少绑定身份 refs 时必须 `blocked`；observe/text/element/visible file evidence 必须保持 refs-first，合法 refs 与 raw payload 混用时必须 `blocked`，不能 silent-drop raw 项后继续；多窗口时 Host 应先用视觉 / AX / text / title / visible file / editor refs 尝试确认唯一正确窗口，证据冲突或无法唯一确认时才 `needs-confirmation`；目标文件不明确、编辑区不可见、缺少结构化 editor element ref 或 observation stale 必须 `needs-confirmation` / `blocked`；保存用户真实文件、撤销用户编辑、批量替换和跨文件修改必须有 Host-collected confirmationRef，`risk:` 形态 riskActionHash 必须直接绑定同一个目标 `file-ref:`，且 confirmationRef 必须同时绑定该 risk、当前 active session 与该 file；`userFile=false` 也必须由 latest observe refs 中的 `non-user-file-scope:` ref 支撑，缺少该 ref 时真实文件类 operation 返回 `blocked`，不能用裸 boolean 绕过确认链。Computer Use core 仍只接受 Host 指定的 primitive，不接受 task plan。
...
补充：P9 real-file live manifest 的 approval evidence 还必须直接绑定同一个 selected `file-ref:` 和 bind active session ref。Host 仍负责收集用户确认；manifest validator 只要求 confirmation evidence 同时保留 `risk:`、`approval:`、active session 和目标文件 ref，避免确认证据与实际 session / 文件目标脱钩。

补充：P9 Host-side controller / native route 的真实文件 `confirmationRef` 也必须绑定目标文件 ref 和当前 active session ref。保存、撤销、替换选区、批量替换或跨文件修改只有在 approval token 同时绑定 `riskActionHash`、当前 observe session 和当前 selected/唯一 visible `file-ref:` 时才会继续；只绑定 risk/file 但不绑定 session 的旧 approval 会保持 `needs-confirmation`，Computer Use core 仍不收集确认、不选择文件、不做 task planning。

补充：P9 Host-side controller / native route 的真实文件 `riskActionHash` 也必须直接绑定目标文件 ref。保存、撤销、替换选区、批量替换或跨文件修改只有在 `risk:` ref 自身携带当前 selected/唯一 visible `file-ref:` 时才会进入确认匹配；泛泛 `risk:save-current-file:paper` 即使配有目标文件绑定的 approval 也会 `blocked` / `vscode_cowork_real_file_change_risk_hash_target_ref_required`。P9 live manifest 的 Host decision 和 approval evidence 同样要求 risk evidence 直接绑定 selected file，否则返回 `missing-host-decision-ref:risk-action-hash-target-file` 或 `missing-approval-ref:risk-action-hash-target-file`。
