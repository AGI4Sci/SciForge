...
补充：P9b/P9c `focus-editor` primitive/Host runner 已达到 `unit-proven`，并完成真实桌面安全 blocked 诊断。current VSCode primitive ports 现在支持 Host 选出的单个 `Command+1` key act；该 act 返回 action / executor / input / after observe refs，不解析 `text-ref:`、不触达私有 draft、不保存文件。Host-side `runVSCodeCoWorkFocusEditorLiveDiagnostic` 和 current VSCode wrapper 只编排 `bind -> observe -> host-decision -> act -> observe -> control(release)`，并要求 after observe 产生 `focused-editor:` ref 才生成 `verifier:...:focus-editor` completion evidence；缺失时返回 blocked completionTruth 并仍释放 input lease / adapter / cursor、恢复 front app / mouse position。focused-editor observe verifier 已从单一 `AXTextArea` role 扩展为保守复合 AX 证据：先拒绝 terminal/search/explorer/source-control 等非编辑目标，再接受 `AXTextArea` 或 Monaco/code-editor/editor-group context；unit tests 证明 Monaco-like `AXGroup` 可生成 focused-editor ref，而 integrated terminal 的 `AXTextArea` 不会被误认。真实当前 VSCode focus 诊断在复合 AX 后仍缺少 `focused-editor:` evidence，action / executor / input refs 和 cleanup / restoration refs 均产生，未打字、未保存、未杀 VSCode、未清 profile；该结果只能记录为 blocked `live-diagnostic`，不能作为 P9c 写入完成证据。下一步卡点是补 visual focused-editor verifier，而不是把 Computer Use core 扩成 planner 或放宽 verifier gate。
