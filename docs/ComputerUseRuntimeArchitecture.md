...
P10 起允许一个很窄的 Host-owned bridge：当 ordinary chat 同时明确指向 VSCode 和 Computer Use / 桌面操纵 / command palette 时，`CodexAppServerClient` 可以先构造 refs-first `sciforge.codex-agent-host-input.v1`，再进入 current VSCode live-diagnostic native route。这个 bridge 只能生成 structured Host input 和 operation refs，不能直接调用 VSCode module、不能执行 Computer Use primitive、不能生成 final answer，也不能把普通 VSCode 问答或模糊文本路由到桌面操纵。UI 可能已经附带 generic `ui-normal-composer-transport` Host input；generic input 不能压掉显式 VSCode Computer Use bridge，后端必须把这类明确文本重新收口成 current VSCode co-work Host input，而不是落回通用 app-server discovery。
...
current VSCode live-diagnostic 的所有早退路径也必须遵守同一 final-answer contract：missing env、invalid `text-ref:`、bind / observe / act blocked、ambiguity 和 evidence conflict 都要由 Host producer 返回 `agentHostFinalAnswer` envelope。Computer Use core 仍只产出 primitive refs / blocked reason；用户可见文字来自 Host-owned envelope，UI 只做确定性投影。
...
frontend provider preflight 不能阻断本地 Host-owned deterministic route：只有文本明确同时指向 VSCode 和 Computer Use / 桌面操纵 / command palette，且能由后端桥接成 current VSCode co-work native route 时，UI 可以跳过远端 provider/model preflight。其它普通聊天和模型请求仍必须走原 provider preflight 与错误处理。
...
- P8：VSCode command palette 原子能力，按 fail-closed -> concept refs -> open -> query -> observe -> mocked select -> verify 递进；真实桌面 live 从 open / query / observe / close 扩展到 current observed item select，仍只标 `live-diagnostic`。
...
- P10：VSCode real-bind command palette diagnostic，按 explicit ordinary chat Host bridge -> structured current VSCode operation -> native route direct live runner -> default macOS command palette adapter -> focused verify -> opt-in real desktop cleanup -> direct route ambiguity / evidence-conflict UX gate -> select-current-item dogfood 验收递进；P10 不扩大 Computer Use core surface，不让 VSCode module 成为第二个 agent，也不宣称 `product-ready`。
- P11：SciForge UI dogfood 与用户可见闭环；只把 P10 Host final-answer public events 稳定呈现在对话 UI 中，不新增 Computer Use core planning、不新增聊天旁路。
...
Command Catalog 是稳定性目录，不是权限系统。VSCode module 不接受模型随意传入的 raw command id；Host 必须基于 allowlisted capability 或当前 observe 产生的 command palette item refs 选择下一步。当前 live diagnostic 只在 Host 已通过 `text-ref:` 发送 palette query 后，基于当前 observation 合成 `command-palette-item/rank/hash` refs；这些 refs 不包含 raw label 或 command id。VSCode module 只验证 item ref 来自 current observe，并把选择映射为一个 `Enter` 原子 primitive；palette `Enter` 与 terminal submit 分流，不能要求 terminal refs。
...
- `save-current-file` 是 VSCode app module readiness：它只在 structured Host operation、current editor/file refs、same-file + mutation verifier 和 Host action evidence 同时存在时，返回一个 `Meta+S` `computer_use.act` primitive candidate；它不从 raw path、raw command、ordinary chat、terminal output、history 或 completed action 推断。
- 批量 / 跨文件不能作为 Computer Use core batch plan 执行，必须由 Host 基于每次 observe refs 拆成多次单步 primitive。
- `bulk-replace` / `cross-file-modify` 属于 Host-owned decomposition guard，而不是 VSCode module capability：guard 只能返回 next-step refs、partial evidence refs 或 blocked reason，不能返回 `computer_use.act`、`computer_use.run_procedure` 或用户任务 completion truth。
...
- P8 palette live harness 已有独立 env gate、palette observation refs、palette-safe act context、mocked `open -> query -> observe -> close/release` 与 `open -> query -> observe -> select current item -> observe -> release` 链路，并验证 cleanup manifest。query action 后才合成 current `command-palette-item/rank/hash` refs；select 只接受 current observation item ref，并用 palette `Enter` 分流，不走 terminal submit gate。真实桌面路径仍默认关闭，只能标 `live-diagnostic`，不能宣称 `product-ready`；默认 executor 仍不能把 raw command id / raw label / paletteLabel / commandText 变成执行旁路。
- P10 current VSCode command palette native route 是 env-gated `live-diagnostic` 并已完成真实桌面与 UX gate 验收：显式 VSCode + Computer Use ordinary chat 会先被 Agent Host bridge 包装成 refs-first Host input，再由 native route 直接启动 `open-command-palette` live diagnostic runner；无预绑定 window candidates 时允许进入 bind/observe，由 Computer Use primitive chain 产出 refs。`targetMode=smart-detect-current-vscode-window` 会让 live runner 尝试激活唯一 VSCode 窗口；若 Host input 已带多个 VSCode window/frontmost candidates、live bind 发现多个非前台 VSCode 窗口，或 selected window 与 latest observation 冲突，direct live route 必须先返回 Host-owned `needs-confirmation` / `blocked` final-answer envelope，不能启动 live runner 猜窗口。默认 macOS shared-system-input adapter 已支持菜单方式打开 command palette、action-backed palette observation refs、palette input `text-ref:`、`Escape` 和 current observed item `Enter`；public projection 只保留 refs、safe summary 和 Host final-answer envelope。真实桌面验收要求独立开启 `SCIFORGE_COMPUTER_USE_VSCODE_COWORK_PALETTE_LIVE_DIAGNOSTIC=1`，完成后必须 release input lease / adapter / cursor 并恢复前台焦点和鼠标位置；不能宣称 `product-ready`。
- P11.0-P11.2 / P11.4-P11.5 SciForge UI runtime event projection / dogfood / verify 是 `unit-proven` 到 env-gated `live-diagnostic`：P10 Host final-answer envelope 的 `completed` / `needs-confirmation` / `blocked` 三态都会结束 Runtime Codex 等待并投影为用户可见结果；`needs-confirmation` 在 message provenance 标记 `requiresUserConfirmation`，`blocked` / `needs-confirmation` 的 final answer 不可被当作 live acceptance。该投影只消费 unified native route public events，不新增 Computer Use core planning 或聊天旁路。显式 VSCode + Computer Use ordinary chat 即使携带 UI generic Host input，也会被后端桥接成 current VSCode co-work Host input；本地 deterministic native route 可绕过远端 provider preflight。`localhost:5173` dogfood 已验证“用 Computer Use 操纵当前 VSCode，打开命令面板”进入 Host-owned native route，跑通真实 `bind -> observe -> open -> observe -> query -> observe -> close -> observe -> release`；也已验证“用 Computer Use 操纵当前 VSCode，打开命令面板并执行 Help: About”跑通真实 `bind -> observe -> open -> observe -> query -> observe -> select current item -> observe -> release`，最终只显示 `live-diagnostic` safe answer。P11 剩余卡点是多 VSCode 窗口 / evidence conflict 的 UI dogfood；所有 shared-system-input 路径仍不能宣称 `product-ready`。
...
- P9.21 save-current-file unit path 是 `unit/materializer-proven`：`save-current-file` 在 VSCode app module capability catalog 中，但只从 structured Host operation、current editor/file refs、same-file verifier、mutation verifier 和 Host mutation action evidence 生成一个 `Meta+S` `computer_use.act` candidate；full-access 不触发类别式 confirmation gate，也不会从 ordinary chat、raw path、raw command、terminal output、history 或 completed action 推断保存目标。public projection 只保留 editor/file/verifier/action/freshness refs，不回灌 preflight permission 或宽 window/operation refs。
- P9.22 batch / cross-file decomposition guard 是 `unit/materializer-proven`：`bulk-replace` / `cross-file-modify` 不进入 VSCode app module readiness，不返回 `computer_use.act` 或 `computer_use.run_procedure`，也不返回 user-level completion truth。默认 materializer 只从 structured Host operation 进入 Host-owned guard，输出 next-step refs、partial evidence refs 或 blocked reason，要求 Host 在下一轮 observe 后继续拆成单个 primitive。
- P9.23-P9.25 current selection mocked apply diagnostic 是 env-gated `live-diagnostic`：使用独立 `SCIFORGE_COMPUTER_USE_VSCODE_COWORK_CURRENT_SELECTION_APPLY_LIVE_DIAGNOSTIC=1` gate；缺少该 env 时 blocked，即使 generic VSCode live env 已开启也不构造 primitive service、writer、adapter、input lease 或 cursor。env-on mock 跑 `bind -> observe -> host-decision -> act -> observe -> control(release)`，Host 调 Host-owned narrow apply bridge 生成最多一个 `replace-selection` / `insert-draft` primitive candidate，随后用 `verifyVSCodeEditorNarrowApply` 证明 same-file、same-window、same-editor、same-selection、after-observe、release 和 cleanup refs。public result 只保留 current editor/file/selection/cursor/range/freshness refs、Host `text-ref:`、app-module verifier、narrow-apply verifier、reason 和 cleanup refs；raw draft、raw selected text、raw diff、visible text、image/accessibility refs、window/observation refs、action/executor/input-event refs、provider payload、base64、completion truth 和 `product-ready` claim 均不能进入 public result。
...
- Current selection apply 真实桌面路径仍默认关闭且只能标 `live-diagnostic`；不能据此宣称 `product-ready`。
