...
- P10 current VSCode command palette native route 是 env-gated `live-diagnostic` 并已完成真实桌面验收：显式 VSCode + Computer Use ordinary chat 会先被 Agent Host bridge 包装成 refs-first Host input，再由 native route 直接启动 `open-command-palette` live diagnostic runner；该路径不需要旧 bridge 预先提供 window candidates，因为 bind/observe 由 Computer Use primitive chain 产出 refs。默认 macOS shared-system-input adapter 已支持菜单方式打开 command palette、action-backed palette observation refs、palette input `text-ref:` 和 `Escape`；public projection 只保留 refs、safe summary 和 Host final-answer envelope。真实桌面验收要求独立开启 `SCIFORGE_COMPUTER_USE_VSCODE_COWORK_PALETTE_LIVE_DIAGNOSTIC=1`，完成后必须 release input lease / adapter / cursor 并恢复前台焦点和鼠标位置；多窗口或证据冲突必须 `needs-confirmation` / `blocked`，不能宣称 `product-ready`。
