...
- P9 面向用户已打开的 VSCode / IDE co-work：Host 可以根据当前 run 的 observe refs、用户选择的 window refs 和风险 envelope 决定下一步原子 primitive；多窗口、目标文件不明确、编辑区不可见或 observation stale 必须 `needs-confirmation` / `blocked`；保存用户真实文件、撤销用户编辑、批量替换和跨文件修改必须有 Host-collected confirmationRef。Computer Use core 仍只接受 Host 指定的 primitive，不接受 task plan。
...
当前 P9 状态是 `unit-proven`：`packages/actions/computer-use/vscode-cowork-acceptance.ts` 记录了 Host-side current VSCode co-work 的 acceptance controller contract，并登记 `CU-NEXT-09 current-vscode-cowork`。该 contract 只根据 Host 提供的窗口候选、选定 windowRef、selectedFileRef、fresh observe refs、draftTextRef、riskActionHash 和 confirmationRef 返回一个下一步原子 primitive，或返回 `needs-confirmation` / `blocked`；selectedFileRef 必须来自当前 window/observe refs 中的 `file-ref:` 形态 visible file refs，不能复用 stale 或外部文件 ref，raw path / 裸文件名也不能作为目标文件 ref；`insert-draft` 缺少 refs-first `text-ref:` 形态 `draftTextRef` 时会 `blocked`，不能把 raw draft text、clipboard payload 或 provider payload 嵌入 Computer Use decision；`read-visible-text` 只返回 `primitive=observe` 和当前 observation/text/AX refs，不产生 `act` action 或 visible text 原文；riskActionHash 和确认后的 approvalRef 都作为 evidence refs 保留，保存、撤销、批量替换或跨文件修改等真实文件操作缺少 `riskActionHash` 时即使出现 approvalRef 也会 `blocked`，且 confirmationRef 必须精确绑定为 `approval:<riskActionHash>:` 前缀，不能只在字符串中包含 risk hash。它不新增 MCP public surface，不进入 Computer Use primitive core，也不产生用户级 completion truth。Runtime Codex native route 现在有一个 VSCode co-work Host bridge：只有 schema/kind/source 均为 Host-owned native route、taskId 为 `CU-NEXT-09` 且带 `current-vscode-cowork` + `refs-first` semantic markers 的 intent 会被路由到该 bridge；bridge 只消费 sanitized refs / operation / confirmationRef，并在多窗口、目标文件不明确或目标不明确时返回 refs-first `needs-confirmation`，不回落到 broader runtime 猜测窗口。P9 cleanup / live manifest validator 要求 manifest 明确 `userProfileUsed=true` 和 `sharedSystemInputUsed=true`；live manifest 还要求 `bind -> observe -> act -> observe -> control(release)`、before/after observe refs、Host decision/action/control refs、release input lease / cursor / adapter refs、front app 与 mouse position restoration refs，并禁止杀用户 VSCode 或清理用户 profile；真实文件操作必须带 riskActionHash 与 approvalRef evidence refs；所有 evidence group 都拒绝 rawScreenshot、providerPayload、data URL、base64、URL、token/password/secret-like 值，避免 live manifest 把 raw payload 当作证据或把共享输入路径误写成 profile-isolated/product-ready 证据。该状态仍不代表真实用户 VSCode co-work live gate 已完成，也不能声明 `product-ready`。
