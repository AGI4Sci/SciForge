...
- P10：VSCode real-bind command palette diagnostic，按 explicit ordinary chat Host bridge -> structured current VSCode operation -> native route direct live runner -> default macOS command palette adapter -> focused verify -> opt-in real desktop cleanup -> direct route ambiguity / evidence-conflict UX gate 验收递进；P10 不扩大 Computer Use core surface，不让 VSCode module 成为第二个 agent，也不宣称 `product-ready`。
...
- P10 current VSCode command palette native route 是 env-gated `live-diagnostic` 并已完成真实桌面与 UX gate 验收：显式 VSCode + Computer Use ordinary chat 会先被 Agent Host bridge 包装成 refs-first Host input，再由 native route 直接启动 `open-command-palette` live diagnostic runner；无预绑定 window candidates 时允许进入 bind/observe，由 Computer Use primitive chain 产出 refs。若 Host input 已带多个 VSCode window/frontmost candidates，或 selected window 与 latest observation 冲突，direct live route 必须先返回 Host-owned `needs-confirmation` / `blocked` final-answer envelope，不能启动 live runner 猜窗口。默认 macOS shared-system-input adapter 已支持菜单方式打开 command palette、action-backed palette observation refs、palette input `text-ref:` 和 `Escape`；public projection 只保留 refs、safe summary 和 Host final-answer envelope。真实桌面验收要求独立开启 `SCIFORGE_COMPUTER_USE_VSCODE_COWORK_PALETTE_LIVE_DIAGNOSTIC=1`，完成后必须 release input lease / adapter / cursor 并恢复前台焦点和鼠标位置；不能宣称 `product-ready`。
- P11.0-P11.2 / P11.4 SciForge UI runtime event projection / dogfood / verify 是 `unit-proven` 到 env-gated `live-diagnostic`：P10 Host final-answer envelope 的 `completed` / `needs-confirmation` / `blocked` 三态都会结束 Runtime Codex 等待并投影为用户可见结果；`needs-confirmation` 在 message provenance 标记 `requiresUserConfirmation`，`blocked` / `needs-confirmation` 的 final answer 不可被当作 live acceptance。该投影只消费 unified native route public events，不新增 Computer Use core planning 或聊天旁路。显式 VSCode + Computer Use ordinary chat 即使携带 UI generic Host input，也会被后端桥接成 current VSCode co-work Host input；本地 deterministic native route 可绕过远端 provider preflight。`localhost:5173` dogfood 已验证“用 Computer Use 操纵当前 VSCode，打开命令面板”进入 Host-owned native route，跑通真实 `bind -> observe -> open -> observe -> query -> observe -> close -> observe -> release`，最终只显示 `live-diagnostic` safe answer；focused tests、typecheck、Computer Use no-bypass 和 no-legacy smoke 已通过。P11 剩余卡点是多 VSCode 窗口 / evidence conflict 的 UI dogfood；所有 shared-system-input 路径仍不能宣称 `product-ready`。
