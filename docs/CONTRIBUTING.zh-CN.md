...
在 SciForge 里，品味意味着清晰的流程、克制的界面、自然的文案，以及用一次就能理解的行为。好的贡献不只是把功能做出来，也要体现判断力。
...
- `gui`：桌面端、云端协作服务和手机协作协议的共同主分支
- 功能分支：从 `gui` 创建、合入后删除的短期分支

规则如下：

- 除维护者发布或紧急修复外，不要直接向 `gui` 推送
- 日常开发从最新 `gui` 创建短期分支
- PR 默认以 `gui` 为目标分支
- 桌面、云端和未来原生手机端通过目录、package 和发布产物隔离，不建立永久的端专用分支

## 开始之前

1. 先确保本地仓库已同步到最新状态。
2. 切换到 `gui` 分支，并从它创建短期功能分支。
3. 运行 `npm install` 安装依赖。
4. 在修改前先确认项目可以正常启动或构建。

## 典型 PR 的结构

一个结构良好的 SciForge PR 应该聚焦且自包含。通常：
...
对于大多数贡献，建议从短期功能分支发起指向 `gui` 的 PR，而不是直接向 `gui` 推送提交。
...
- 操作系统及版本
- SciForge 版本（可在设置页或关于对话框查看）
- 内置的 `sciforge` 版本（如可用，在同目录下执行 `kun --version`）
- 复现步骤
- 预期行为与实际行为
- 相关错误信息、日志或截图
