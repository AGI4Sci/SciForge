...
| Computer Use                       | Codex TUI 原生 action provider；可选消费 sense provider | Codex app-server native tool/plugin/MCP production path；Codex CLI/native plugin debug path；`virtual-app-screen-user-acceptance` active gate；adapter-first app/window/session native/streaming live surface；BrowserRuntime DOM/AX observation refs as hints；research workflow user-acceptance manifest；`module.invoke(moduleId='actions', intent='execute')`；`packages/actions/computer-use` 的 `runTask(request, hostPorts)`、`packages/observe/vision` 的 observation/grounding/verifier 输出、desktop/remote/dry-run host ports | Computer Use action provider 拥有执行和 VirtualAppScreen single live surface truth，但不是第二个 L2。`vision-sense` 不拥有真实桌面动作。React/UI/Screen viewer 只做 GUI presentation，不执行 Computer Use；raw screenshot/log payload 不进入主结果或长期 trace；snapshot/replay/PDF/document 只能是 evidence/artifact，不得作为第二个 Screen truth；legacy gateway/exec/docker/noVNC/RDP/M6 multi-screen 只能是 diagnostic/test-only、historical evidence、historical regression 或 backend packaging，不能作为当前 active gate。 |
...
|---|---|---|
| Browser | 展示 typed browser projection、host-owned BrowserHostSession native/streaming live surface、host-returned cursor/caret/pointer projection、writer health/preflight 诊断、blocked/error/offline 状态、snapshot/search/log evidence refs 和 `/browser ...` terminal-equivalent command。桌面 shell 只 attach `WebContentsView` bounds；外部 HTML 的 proxy/snapshot/PDF/document 不能作为 live browser、第二画面真相源或替代交互路径。 | `BrowserHostSession` live session/tab/action/cursor/pointer/surface/snapshot/DOM/AX/console/network/search refs、Workspace Writer `/api/sciforge/browser-host/*` host routes、`/computer-use-actions` 输入通道、desktop native embedded adapter、非桌面 `/frame-stream` websocket-binary transport、`/frame` evidence route、`browser_runtime` provider route、human takeover 和 high-risk approval；Tauri/Electron/Web 仅为 shell/adapter。 |
| Screen | 展示 refs-first VirtualAppScreen native/streaming live surface、annotation overlay、actor cursor、lease/proposal/permission 状态、`gui.present` evidence 和 `/computer-use ...` terminal-equivalent command；replay/snapshot 仅作为证据视图，不能作为替代交互路径。 | Computer Use action provider、app/window/session adapter、platform sidecar、live surface binding、capture/ground/execute/verify、executor lease、approval、replay/evidence refs、`virtual-app-screen-user-acceptance-manifest` 和 completion candidates。 |
...
大对象必须 refs-first：截图、录屏、terminal transcript、DOM/AX snapshot、provider payload、audit、trace、replay 和 artifact 不得内联到 pane state；projection 只保留 ref、摘要、尺寸/hash、live surface handle/ref 和脱敏 diagnostics。GUI 按钮只能生成 focus/open、declared GUI intent、confirmation result 或 terminal-equivalent text；Computer Use、BrowserRuntime、Terminal 和 Files 的真实副作用仍由 Agent Host 通过对应 native module 执行。Browser pane 和 Screen pane 的交互真相源只能各有一个 owner-owned live surface；证据 artifact 不得变成第二套可交互 UI，也不得作为替代交互路径。

Computer Use 的内部拆分必须保持单一执行 owner，但不能形成第二个 Root Agent Host：`packages/actions/computer-use` 拥有 request/result schema、domain-local action loop、safety/approval、VirtualAppScreen/session contract、actor cursor contract、scheduler/executor adapter contract、trace contract 和 compact handoff；用户级 planning、跨模块 pipeline、repair 和 completion 仍归 Codex TUI Agent Host。`packages/observe/vision` 只提供 sense、coarse-to-fine focus region、KV-Ground/visual grounding helper、verifier feedback 和 file-ref-only visual memory。KV-Ground 默认 endpoint 为 `http://127.0.0.1:18081`，Computer Use grounder provider 应记录实际 `/health` 与 `/predict/` endpoint；没有共享路径映射时使用 inline image upload，避免把本机截图路径误当服务端路径。最终输入隔离目标是 adapter-first background app control with native/streaming live surface：SciForge 维护 VirtualAppScreen、target app/window/session refs、single live surface ref、virtual cursor、annotation overlay、input intent、scoped executor state 和 before/after evidence；底层优先使用 app-command、BrowserRuntime/native surface、terminal PTY、editor extension、accessibility、vision-grounded 或 offscreen-display adapter，不移动系统鼠标、不发送全局系统键盘事件、不把窗口弹到用户物理屏。真实 OS multi-pointer / multi-seat 只允许作为未来可替换 executor backend，不进入 planner、GUI 或 schema 的核心假设。真实输入如果没有独立 app/window scoped adapter，就属于 shared system input，只能作为迁移期诊断、blocked 或 explicit handoff evidence，不能替代 live surface。Trace 输出只保存 refs、sha256、尺寸、target description、screen/window/crop-local coordinates、actor/cursor ids、lease scope、provider metadata、diagnostics、approval/audit refs，不保存 raw screenshot/base64。
...
Computer Use 当前仍处于 `migrating`，剩余拆分不再用笼统 partial 记录，而是登记在 `docs/native-extension-ownership-map.json` 的 `computer-use.remainingMigrationSubtasks`。当前登记项覆盖 action schema、window target contract、scheduler lease contract、executor adapter contract、package bridge 的 plan/locate/verify/trace 收缩、coordinate/focus policy 归属、stale policy wrapper 删除、planner/text policy 迁移、capture/OCR/visible text observation 归属、独立 input adapter、VirtualAppScreen / actor cursor contract、viewer overlay、acceptance validator、Codex app-server production path、legacy/test-only gateway/exec 降级、Docker/container/noVNC/RDP backend packaging 降级和 import boundary guard。新增 active backlog 以 `PROJECT_CU.md` 为准：`virtual-app-screen-user-acceptance` active gate、adapter-first background app control、annotation-to-proposal、background native window capture/action、research workflow user-level acceptance，以及 action-provider public surface parity guard。JSON manifest 中保留的 M6/multi-screen 条目是现有 smoke/parity 的 historical regression 登记；迁移到 VirtualAppScreen user-acceptance manifest 前仍不得让 runtime 无记录地变厚，且不得让 M6 阻塞当前路线或替代验收。
...
| `src/ui/**` and future viewer package | GUI presentation | VirtualAppScreen frame/replay refs、actor cursor overlay、proposal/lease/evidence/ref 展示、confirmation UI、terminal-equivalent text 和 `gui.present` 投影。 | import action provider、observe provider implementation、runtime bridge、executor 参数或执行 desktop action。 |
...
| Docker/container/noVNC/RDP | legacy diagnostic / historical evidence / backend packaging | 只负责 sandbox、deps、resource limits、filesystem/network policy、viewer transport 和 lifecycle；并发协作模型来自 display group、actor cursor、scheduler lease 和 executor adapter。不得作为当前 active gate、产品验收 owner 或 rich-client fallback。 |
...
7. **Computer Use 用户级验收**：一次点击输入框的 smoke 只能证明基础链路。最终 success 至少需要真实用户产物，例如报告、notebook、figure、CSV、PPT/DOCX 或实验日志；目标打通需要 `virtual-app-screen-user-acceptance` active gate、adapter readiness、后台 app/window frame、input intent、before/after evidence、annotation/proposal refs、BrowserRuntime DOM/AX observation refs/hints、`gui.present` 和 refs-first 证据链。GUI 不因参与展示或确认而拥有 Computer Use 执行权；Docker/noVNC/RDP/M6 multi-screen 只能作为 legacy diagnostic/historical/backend packaging 或 sidecar/ref historical regression 证据来源，不能替代当前 user-acceptance manifest。
