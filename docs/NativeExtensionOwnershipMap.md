...
| 右侧结果交互模块                         | SciForge GUI presentation packages + Host adapters | `ResultShell` pane contract；`packages/presentation/components/browser-workbench`、`virtual-screen-viewer`、`terminal-session-viewer`、`workspace-file-viewer`；References object inspector；`packages/presentation/interactive-views` registry slot policy | Browser/Screen/Terminal/Files/References 都是可组合 presentation module。GUI 只渲染 projection、refs、buffer、tree、draft 和 view-local intents；真实 browser provider、Computer Use executor、PTY/process、workspace read/write 和 ref materializer 由 TUI/Host adapter 执行。 |
...
| Browser runtime                    | Codex TUI observe capability + SciForge GUI presentation | TUI: `packages/observe/web` 的 `browser_runtime` manifest/provider wrapper；shared: `@sciforge-ui/runtime-contract/browser-runtime`；GUI: `packages/presentation/components/browser-workbench` 和 `/browser ...` 终端等价文本 | GUI 只渲染 browser projection、tabs、snapshot/log refs 和 host-declared preview；不得 import `@sciforge-observe/web/browser-runtime`、选择 provider、执行页面动作、读取跨域 DOM 或保存 base64/完整 DOM/log。Computer Use 只能把 DOM/AX snapshot refs 当 observation refs/hints，不能把 DOM/AX/Playwright 当执行或完成证据。 |
| Computer Use                       | Codex TUI 原生 action provider；可选消费 sense provider | Codex app-server native tool/plugin/MCP production path；Codex CLI/native plugin debug path；native multi-screen/multi-actor cursor product gate；multi-screen live demo；BrowserRuntime DOM/AX observation refs；native multi-app workflow/live acceptance matrix；`module.invoke(moduleId='actions', intent='execute')`；`packages/actions/computer-use` 的 `runTask(request, hostPorts)`、`packages/observe/vision` 的 observation/grounding/verifier 输出、desktop/remote/dry-run host ports | Computer Use action provider 拥有执行，但不是第二个 L2。`vision-sense` 不拥有真实桌面动作。React/UI 不执行 Computer Use；raw screenshot/log payload 不进入主结果或长期 trace；legacy gateway/exec/docker/noVNC/RDP 只能是 diagnostic/test-only、historical evidence 或 backend packaging。 |
...
|---|---|---|
| Browser | 展示 typed browser projection、safe preview、blocked/error/offline 状态、snapshot/log refs 和 `/browser ...` terminal-equivalent command。 | `browser_runtime` session/tab/action/snapshot/DOM/AX/console/network refs、provider route、human takeover 和 high-risk approval。 |
| Screen | 展示 refs-first virtual screen frame、replay、actor cursor、lease/proposal/permission 状态和 `/computer-use ...` terminal-equivalent command。 | Computer Use action provider、platform sidecar、capture/ground/execute/verify、executor lease、approval、replay/evidence refs 和 completion candidates。 |
...
大对象必须 refs-first：截图、录屏、terminal transcript、DOM/AX snapshot、provider payload、audit、trace、replay 和 artifact 不得内联到 pane state；projection 只保留 ref、摘要、尺寸/hash、safe preview URL 和脱敏 diagnostics。GUI 按钮只能生成 focus/open、declared GUI intent、confirmation result 或 terminal-equivalent text；Computer Use、BrowserRuntime、Terminal 和 Files 的真实副作用仍由 Agent Host 通过对应 native module 执行。

Computer Use 的内部拆分必须保持单一执行 owner，但不能形成第二个 Root Agent Host：`packages/actions/computer-use` 拥有 request/result schema、domain-local action loop、safety/approval、multi-screen/session contract、actor cursor contract、scheduler/executor adapter contract、trace contract 和 compact handoff；用户级 planning、跨模块 pipeline、repair 和 completion 仍归 Codex TUI Agent Host。`packages/observe/vision` 只提供 sense、coarse-to-fine focus region、KV-Ground/visual grounding helper、verifier feedback 和 file-ref-only visual memory。KV-Ground 默认 endpoint 为 `http://127.0.0.1:18081`，Computer Use grounder provider 应记录实际 `/health` 与 `/predict/` endpoint；没有共享路径映射时使用 inline image upload，避免把本机截图路径误当服务端路径。最终输入隔离目标是独立 simulated input adapter：SciForge 维护 display group、virtual screen、actor cursor 和 scoped executor state，不移动系统鼠标、不发送全局系统键盘事件；多鼠标首先是协作层 actor cursor / intent / overlay，不默认等价于多个 OS 级真实 pointer。真实 OS multi-pointer / multi-seat 只允许作为未来可替换 executor backend，不进入 planner、GUI 或 schema 的核心假设。真实输入如果没有独立 input adapter，就属于 shared system input，只能作为迁移期诊断或 blocked evidence，必须绑定低风险目标窗口、显式确认并串行持有 executor lease。当前第一片实现只让 `remote-desktop` 在 Host 注册 `sciforge-simulated-remote-desktop` provider 时可执行，并写出 `independent-input-adapter.json` / pointer icon refs；`virtual-hid`、真实 multi-pointer backend 和未注册 provider 仍 fail closed。Trace 输出只保存 refs、sha256、尺寸、target description、screen/window/crop-local coordinates、actor/cursor ids、lease scope、provider metadata、diagnostics、approval/audit refs，不保存 raw screenshot/base64。
...
Computer Use 当前仍处于 `migrating`，剩余拆分不再用笼统 partial 记录，而是登记在 `docs/native-extension-ownership-map.json` 的 `computer-use.remainingMigrationSubtasks`。当前登记项覆盖 action schema、window target contract、scheduler lease contract、executor adapter contract、package bridge 的 plan/locate/verify/trace 收缩、coordinate/focus policy 归属、stale policy wrapper 删除、planner/text policy 迁移、capture/OCR/visible text observation 归属、独立 simulated input adapter、multi-screen / actor cursor contract、viewer overlay、acceptance validator、Codex app-server production path、legacy/test-only gateway/exec 降级、Docker/container/noVNC/RDP backend packaging 降级和 import boundary guard。新增 active backlog 明确登记为：native multi-screen/multi-actor cursor product gate、multi-screen live demo、BrowserRuntime DOM/AX observation refs、native multi-app workflow/live acceptance matrix，以及 action-provider public surface parity guard。只要 `computer-use.status` 仍为 `migrating`，`npm run smoke:native-extension-ownership` 与 `npm run smoke:module-boundaries` 必须校验这些 backlog 项存在，防止 runtime 继续无记录地变厚。
...
| `packages/actions/computer-use` | L1/L0 owner | request/result schema、VirtualDisplayGroup、VirtualScreen、ActorCursor、ActionProposal、ExecutorLease、ExecutorAdapter、domain-local loop、safety、trace、compact handoff 和 L0 handler routing。 | 直接 import/call GUI、browser、file、verifier；选择跨模块下一步；判断用户级 completion。 |
| `packages/observe/vision` | L0 sense/grounding provider | observation、focus region、OCR/VLM/KV-Ground helper、verifier feedback、file-ref-only visual memory。 | 真实桌面输入、scheduler lease、executor adapter、desktop bridge 或 completion 判断。 |
...
| `src/ui/**` and future viewer package | GUI presentation | multi-screen replay、actor cursor overlay、proposal/lease/evidence/ref 展示、confirmation UI、terminal-equivalent text。 | import action provider、observe provider implementation、runtime bridge 或执行 desktop action。 |
...
| Docker/container/noVNC/RDP | legacy diagnostic / historical evidence / backend packaging | 只负责 sandbox、deps、resource limits、filesystem/network policy、viewer transport 和 lifecycle；并发协作模型来自 display group、actor cursor、scheduler lease 和 executor adapter。不得作为 active product gate、产品验收 owner 或 rich-client fallback。 |
...
7. **Computer Use 用户级验收**：一次点击输入框的 smoke 只能证明基础链路。最终 success 至少需要真实用户产物，例如制作并保存一页 PPT；目标打通需要 native multi-screen/multi-actor cursor gate、多屏 live demo、多 App 工作流、BrowserRuntime DOM/AX observation refs/hints 和 refs-first 证据链。GUI 不因参与展示或确认而拥有 Computer Use 执行权；Docker/noVNC/RDP 只能作为 legacy diagnostic/historical/backend packaging 证据来源。
