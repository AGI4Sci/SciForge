...
| 右侧结果交互模块                         | SciForge GUI presentation packages + Host adapters | `packages/presentation/components/browser-workbench`、`terminal-session-viewer`、`workspace-file-viewer`；`packages/presentation/interactive-views` registry slot policy | Browser/terminal/file viewer 都是可组合 presentation module。GUI 只渲染 refs、buffer、tree、draft 和 view-local intents；真实 browser provider、PTY/process 和 workspace read/write 由 TUI/Host adapter 执行。 |
