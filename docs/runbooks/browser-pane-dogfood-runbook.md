...
Browser operation 只返回 refs-first result，例如 source refs、page refs、search result refs、page evidence refs、blocked reason、approval request 和 compact observation。它不负责选择是否搜索、不负责综合答案、不负责 repair，也不宣布用户任务完成。

## 局部动作范围

Browser 未来可以支持点击、下载、打开下一页、站内探索等局部动作，但必须放在 `module.invoke(executeBoundedOperation)` 里：
