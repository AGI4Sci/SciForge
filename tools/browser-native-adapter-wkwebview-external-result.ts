...
const WKWEBVIEW_CANDIDATE: BrowserNativeAdapterCandidateId = 'wkwebview';
const WKWEBVIEW_PLATFORM: BrowserNativeAdapterPlatform = 'macos';
export const WKWEBVIEW_REAL_COMMAND_ENV =
  'SCIFORGE_BROWSER_NATIVE_ADAPTER_WKWEBVIEW_REAL_COMMAND' as const;
export const WKWEBVIEW_REAL_ARGS_JSON_ENV =
  'SCIFORGE_BROWSER_NATIVE_ADAPTER_WKWEBVIEW_REAL_ARGS_JSON' as const;
export const WKWEBVIEW_RUN_LIVE_SMOKE_ENV =
  'SCIFORGE_BROWSER_NATIVE_ADAPTER_WKWEBVIEW_RUN_LIVE_SMOKE' as const;
...
export async function buildWkwebviewExternalBenchmarkResult(
  ...
  const realCommand = env[WKWEBVIEW_REAL_COMMAND_ENV];
  if (realCommand) {
    return runRealWkwebviewAdapterCommand({
      env,
      candidateId,
      platform,
      requiredSections,
  ...
  return {
    ...
    diagnosticRefs: [
      ...
      'wkwebview:platform-macos',
      'wkwebview:display-input-adapter-role',
      'wkwebview:missing-browser-host-session-native-surface-attach',
      'wkwebview:missing-native-input-routing-proof',
      'wkwebview:real-native-adapter-run-refused',
      'wkwebview:real-native-adapter-run-required',
      `env:${WKWEBVIEW_REAL_COMMAND_ENV}:missing-real-adapter-command`,
      'browser-host-session:owner',
      'native-embedded:single-interactive-truth',
      'browser-native-adapter-platform-benchmark:typed-blocked-external-result',
    ],
  };
}

async function runRealWkwebviewAdapterCommand(input: {
  env: EnvRecord;
  ...
  try {
    const args = parseArgs(input.env[WKWEBVIEW_REAL_ARGS_JSON_ENV]);
    ...
    return failedExternalResult({
      ...
      diagnosticRefs: [
        'wkwebview:real-native-adapter-command-failed',
...
function candidateFromEnv(env: EnvRecord): BrowserNativeAdapterCandidateId {
  ...
  return candidate === WKWEBVIEW_CANDIDATE ? candidate : WKWEBVIEW_CANDIDATE;
}
...
function platformFromEnv(env: EnvRecord): BrowserNativeAdapterPlatform {
  ...
  return platform === WKWEBVIEW_PLATFORM ? platform : WKWEBVIEW_PLATFORM;
}
...
function parseArgs(raw: string | undefined): string[] {
  ...
  if (!Array.isArray(parsed)) {
    throw new Error(`${WKWEBVIEW_REAL_ARGS_JSON_ENV} must be a JSON string array`);
  }
  return parsed.map((item) => {
    if (typeof item !== 'string') {
      throw new Error(`${WKWEBVIEW_REAL_ARGS_JSON_ENV} must contain only strings`);
    }
    return item;
  });
}
...
function assertExternalResultForCandidate(
  ...
  if (
    ...
    throw new Error('WKWebView real adapter command returned an invalid bounded external result');
  }
  assertBoundedExternalResult(JSON.stringify(external));
}

function assertTrustedLiveSmokeOptIn(env: EnvRecord): void {
  if (env[WKWEBVIEW_RUN_LIVE_SMOKE_ENV] !== '1') {
    throw new Error('WKWebView external real command pass evidence requires trusted live adapter helper provenance');
  }
}

function assertBoundedExternalResult(text: string): void {
  if (Buffer.byteLength(text, 'utf8') > MAX_EXTERNAL_RESULT_BYTES) {
    throw new Error('WKWebView external result must stay bounded.');
  }
  if (/data:image|;base64,|iVBORw0KGgo|<\s*(?:!doctype|html|body|script|iframe|webview)\b/i.test(text)) {
    throw new Error('WKWebView external result must not include raw payloads.');
  }
  if (/https?:\/\//i.test(text)) {
    throw new Error('WKWebView external result must not include raw URL strings.');
  }
  if (/"(?:rawUrl|url|requestedUrl|currentUrl|finalUrl|rawDom|dom|domSnapshot|screenshot|rawScreenshot|screenshotBase64|screenshotBytes|clipboard|selection|menu|provider|payload|rawPayload|providerPayload|consoleLog|networkLog|secret|token|password|credential|cookie|authorization|apiKey)"\s*:/i.test(text)) {
    throw new Error('WKWebView external result must not include forbidden raw URL, payload, log, or secret fields.');
  }
}
...
async function main(): Promise<void> {
  process.stdout.write(`${JSON.stringify(await buildWkwebviewExternalBenchmarkResult())}\n`);
}
