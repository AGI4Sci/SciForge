...
} from '../src/runtime/codex/agent-host-vscode-cowork-current-live-acceptance.js';
...
export async function runCurrentVSCodeCoWorkReadonlyLiveAcceptanceCli(
  argv = process.argv,
): Promise<void> {
  ...
  const outputDir = resolve(args.outputDir ?? join(workspacePath, 'docs', 'test-artifacts', 'current-vscode-cowork-readonly-live'));
  ...
  const blocked = manifest.blockedReasons.length
    ...
    : '';
  process.stdout.write(`[${manifest.status}] Current VSCode read-only live acceptance manifest: ${manifestPath}${blocked}\n`);
}
...
export function parseArgs(argv: string[]): CliArgs {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--workspace') {
      ...
      throw new Error(`Unknown current VSCode read-only live acceptance argument: ${arg}\n${usage()}`);
    }
  }
  return args;
}
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? '').href) {
  await runCurrentVSCodeCoWorkReadonlyLiveAcceptanceCli(process.argv).catch((error) => {
