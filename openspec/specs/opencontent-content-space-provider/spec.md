...
The OpenContent Connector and Content Space Provider adapter SHALL remain independently versioned public domain packages installed through the standard public workspace and generated composition path. The Connector SHALL own supplier wire/transport/process isolation; the Provider SHALL own receipt-to-Content-Space semantics. No standalone OpenContent runtime package, compatibility package, or private cross-package implementation import SHALL exist. Optional supplier executable assets SHALL remain receipt-backed runtime data beneath `internal/opencontent/**`, outside the public workspace, lockfile, and package dependency graph. A clean checkout with no attachment SHALL install, build, test, start, and package SciForge normally; ordinary Content Space and the public Team Administration adapter MAY remain composed, while attachment-backed native-document and extended operations SHALL be absent or unavailable without a stub, private registry, or fallback resolver.
...
The four OpenContent directory searches SHALL remain `blocked_by_contract / provider_contract_missing` because current supplier evidence does not freeze their exact success item, identity, literal-kind, and pagination shapes. They SHALL fail before supplier transport even though the provider-neutral extended contract defines the shape a future evidenced implementation must return.
