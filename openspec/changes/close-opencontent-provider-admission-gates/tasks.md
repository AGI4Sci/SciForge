...
- [x] 2.3 Make all four provider-neutral directory searches literal-kind typed; freeze the exact 50-operation extended catalog at 40 PoC / 10 blocked with an overlay and 1 PoC / 49 blocked without one; block `resolveInternalLink`, `listMetadataChoices`, `updateFileVersion`, all four searches, `resolveCollaborationInvitation`, `listKnowledgeCollections`, and `searchKnowledgeCollections`; use only an independently authoritative same-instance user reference for ordinary add/list/remove membership; reject legacy Host `contentUserId` mutation payloads; and keep ordinary membership incapable of creating Project authority.
...
- [x] 2.5 Remove the public member-role/ownership delegates and every Administration revision/CAS field; require fresh per-invocation confirmation for all five ordinary-root Administration mutations, complete stable Team or Team-user pagination before dependent mutation, exact `{ member }` page items, and exact request binding for all ten Administration outputs with read/write error classification.
- [x] 2.6 Require fresh per-invocation confirmation for every Agent native-document or extended destructive operation, remove its autonomous-write grant, and prove through the real Broker that a listed ordinary child cannot reach Provider dispatch without approval.
...
- [x] 5.3 Regenerate domain composition and capability governance documentation at the final 266 registered actions and audit for private imports, raw clients, stale runtime artifacts, duplicate paths, and Host domain hard-coding.
...
- [x] 6.9 Keep the no-credential supplier inventory characterization inside the Connector package: freeze attachment CLI version `1.0.0`, all 86 supplier commands, and the exact 50-command admitted adapter union; keep `download`, `file-list`, `kbox-list`, `file-internal-link`, `meta-modeldata`, and `collab-link` inventory-only. Do not treat direct CLI execution as packaged/canonical callability or readiness evidence.
- [x] 6.10 Remove the compiled demonstration origin; add the strict fixed-path package-owned deployment sidecar resolver with bounded same-descriptor reads, one Connector runtime getter/gate, synthetic source/packaged integration fixtures, isolated packaged namespace, immutable declaration/receipt Electron composition with active-match/inactive-absence after-pack checks, public-release rejection, npm tarball exclusion, and final architecture/runbook/ADR/spec documentation.
- [x] 6.11 Keep confirmed node-local unbind available in the integration-owned unavailable enrollment view without constructing Provider runtime, contacting the Provider, or implying remote-file deletion.
