...
The local Plan editor changes the complete reviewable task content and Worker User assignments before immutable submit. Offer creation stores the Task capability requirements; exact Agent/Device authority and readiness are revalidated only when a Device claims. Review CAS facts are derived from the currently visible Project/Task/execution/result revisions. Final completion is shown only for a confirmed Plan whose current Tasks each have one accepted current result and whose shared ProjectRecord memory contains an Owner decision; it submits the exact accepted result IDs and final summary atomically.
