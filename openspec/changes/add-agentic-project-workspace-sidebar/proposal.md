...
- Project 创建不要求预先存在 Team-root；Team-root 仍是创建后的 Project-scoped provisioning 产物，本变更不修改 membership、invitation、provisioning、dispatch、continuation 或 review 状态机。
...
<!-- None. Cloud collaboration and Project lifecycle semantics remain owned by the existing full-loop change. -->

## Impact

- 影响 Domain SDK renderer contribution contract、renderer contribution registry、Workbench/Sidebar 的通用导航组合、Project Coordinator renderer/manifest/i18n、其聚焦 activation 与 package-owned composer context。
- 不修改 collaboration contracts/server、数据库 migration、Team provisioning、Task 派发、continuation 或 Provider 路径。
