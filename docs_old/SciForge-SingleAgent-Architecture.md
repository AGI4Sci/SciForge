...
SciForge 的运行体验必须遵循“自由探索 -> 专业沉淀”原则。Backend 保留通用 Agent 的开放探索能力，可以按当前目标读取 refs、生成代码、试错和修复；Runtime 不用前端模板或 prompt 特例提前限制任务形态。只有当中间过程或最终结果需要成为科研资产时，Runtime Bridge、Gateway 和 Projection 才把它收敛为 artifact、ExecutionUnit、evidence、verification、failure owner、recover action 和 audit refs。严格 contract 是交付和复用边界，不是用户开始工作的门槛。

Capability 不能只作为后置校验或诊断摘要出现。凡是 Capability Gateway 已解析出 ready provider/tool route，AgentServer generation prompt 必须把它渲染成可执行 authoring contract：helper/API 签名、任务输入字段、失败输出规则和可复制 adapter skeleton。运行时 preflight、strict retry、Projection failure 只是最后防线；最终形态要求 Backend 从一开始就按 provider-first 工具路径生成任务代码，而不是先生成直连脚本再让 Gateway 拦截。

Capability Discovery 也必须进入同一条 agent 行为链路。当前状态为 partial backend retry consumption + ledger replay refs + default UI summary card + debug folding / blocked-on-P1-P6-browser-validation，目标设计见 [`CapabilityDiscovery.md`](CapabilityDiscovery.md)：SciForge 不应把“什么时候检索能力”硬编码成一组固定触发点；初始 context 只暴露极简 `capability_discovery` API brief，AgentServer/backend 在能力不足、provider 失败或验证/修复需要换路时主动调用 discovery。Runtime/harness 只负责 API contract、审计、预算/权限和 provider-first 兜底。当前已有 contract/service/manifest/tiny brief/generated-task helper bridge、AgentServer stream-side tool-call -> tool-result bridge、session-bundle audit record、session-bundle ledger event、单向 stream 无终态时的 bounded retry handoff consumption，以及 `ProjectionApi.getCapabilityPlanSummary` / default Results UI 能力计划卡 / debug folding action boundary；还需补齐真实 P1-P6 browser 验收。

UI 与执行层的解耦也必须成为同等级边界。目标设计见 [`UIExecutionDecoupling.md`](UIExecutionDecoupling.md)：网页端只通过函数式 Projection/UserAction API 展示、预览和干预数据；函数 API 是语义 contract，不绑定 HTTP。AgentServer 原文、handoff JSON、stdout/stderr、task attempt 和 raw ToolPayload 只能进入 runtime ledger / audit，不得成为 UI 直接判断完成态或主答案的输入。

---

## 新 Tool 接入最小 Contract
...
5. **Authoring Contract**：prompt renderer 必须从 manifest/route 生成标准 helper/API 签名和 adapter skeleton，例如 generated Python 统一走 `sciforge_task.load_input`、`invoke_capability(task_input, capabilityId, input)`、`write_payload`。具体工具只扩展 schema/examples，不扩展散落 prompt 分支。
6. **Runtime Invocation**：Runtime Bridge 只调用 `Gateway.execute`；generated task 只通过 helper SDK 或 Gateway tool-call 调用 capability，不能直连 provider 实现。
7. **Preflight / Validation**：Gateway 内部按 manifest 做权限、预算、输入/输出和 side-effect preflight；失败必须成为 `failed-with-reason` / `repair-needed` Projection evidence。
8. **ArtifactDelivery / Projection**：tool 输出必须声明哪些 artifact 是 primary/supporting/audit/diagnostic/internal；UI 只消费 Projection 和 ArtifactDelivery，不从 raw tool output 猜主结果。
9. **Conformance Fixture**：每个新 capability 至少有 manifest fixture、context injection fixture、authoring prompt fixture、invoke/empty/failure fixture 和 no-secret/no-endpoint leakage guard。

这九项是最小闭环。少了 manifest，Backend 不知道能力边界；少了 authoring contract，模型会退回自己熟悉的库或习惯；少了 Projection/ArtifactDelivery，用户会看到 raw/audit 或空结果；少了 conformance，后续新工具会重新把策略塞回 prompt builder。

### Capability Discovery 原子能力（partial backend retry consumption + ledger replay refs + default UI summary card + debug folding / blocked-on-P1-P6-browser-validation）

详见 [`CapabilityDiscovery.md`](CapabilityDiscovery.md)。当前代码已有 manifest registry、capability broker、provider preflight、harness candidate projection、`invoke_capability` authoring path、`capability_discovery.search/expand/plan/explain` 的 TypeScript service、tiny handoff brief、generated-task helper、AgentServer stream-side discovery tool-call bridge、session-bundle audit record、session-bundle ledger event、bounded retry result consumption、最小 UI summary projection 和 targeted tests。本文只保留架构边界；API 细节、分层揭示、UI 摘要和剩余实现任务以该专项文档为准。
...
- 新聊天 / 新 session 的首轮必须是强隔离 fresh turn：清空旧 `contextReusePolicy`、current work、recent failure、recover action、AgentServer session reuse 和历史 handoff hints。
- Runtime Bridge 只中继结构化事件和调用 `Gateway.execute`，不拼 prompt、不猜意图、不直接执行工具。
- UI 的最终事实只来自 Projection；stream delta 只是临时显示。UI 用户动作必须通过函数式 action contract 回到 runtime，不直接修改 run/artifact/session 内部结构。
...
type SessionBirthKind = 'fresh-chat' | 'forked-from-active-run' | 'explicit-import'
```
...
- `fresh-chat` 新 session 的首个 user turn 必须从空的会话 Projection 开始，只能继承 workspace identity、scenario/capability policy 和用户显式选择的 refs；不得继承旧 session 的 `contextReusePolicy`、`recentExecutionRefs`、`agentHarnessHandoff`、`RecoverAction`、`failureRef`、AgentServer current work 或 stable AgentServer session id。
...
- archived/historical session 只能作为 audit、retrieval 或显式 import 来源；它们的失败状态、bounded-stop、repair attempts 和 recover actions 不能被推断成新 session 的 repair 上下文。
...
对于 web/literature/retrieval 这类外部访问任务，Backend 的正确输出是标准 capability invocation 或 provider-route adapter，而不是临时生成 `urllib`、`requests`、raw socket、站点 scraper 等直接外网脚本。只要 Capability Registry / ProviderManifest 中已有可用或可诊断的 `web_search`、`web_fetch`、`arxiv_search`、`semantic_scholar_search` 等 route，Backend 必须 capability-first；缺 route 时返回结构化 `provider-unavailable` / `failed-with-reason`，不能绕过 Gateway 自行联网。
...
- **Capability-first 执行**：当标准 capability/provider route 存在时，Backend 不得生成直接外网、文件系统或 worker endpoint 访问脚本来替代 capability invocation；preflight 发现 `urllib` / `requests` / raw network bypass 时必须 fail closed，并 materialize 为 Projection 可见的可恢复失败或 provider 缺失说明。
...
type AgentServerContextRequest = {
  ...
  contextPolicy: {
    mode: 'fresh' | 'continue' | 'repair' | 'answer-from-registry'
    contextReusePolicy: 'isolate' | 'continue-current-session' | 'repair-current-session'
...
- fresh turn 默认隔离旧 recent turns；fresh-chat 首轮必须设置 `contextReusePolicy='isolate'`、`includeCurrentWork=false`、`includeRecentTurns=false`、`persistRunSummary=false`，并且不得携带旧 `failureRef` / recover actions / AgentServer current work；continue/repair 才显式打开 current work 和 recent turns
- 新聊天首轮必须强制视为 fresh boundary：历史 failed run、archived session、旧 `contextReusePolicy`、旧 `agentHarnessHandoff`、旧 `recentExecutionRefs` 只能作为归档/审计记录存在，不能把首轮 current turn 推导成 repair。用户在 fresh turn 中提到 “failure / 失败 / provider unavailable / 不要编造失败原因” 只表示缺证据时的输出要求；只有显式 prior refs、current-session failure refs、Projection `RecoverAction` 或“继续/修复上一轮”这类结构化 continuation 信号才能打开 repair mode。
...
3. 根据 mode 和 `contextReusePolicy` 收紧上下文：`fresh` / `isolate` 不带 recent turns、current work、历史 recover action 或旧 AgentServer session state；`continue-current-session` 才允许 current work；`repair-current-session` 只允许同一 session 当前 Projection 中的 `failureRef`、evidence refs 和当前 projection refs。
...
#### Fresh turn / new-chat 隔离边界

新聊天不是“带空输入框的旧 session”，而是新的 session boundary。UI 点击 `开启新聊天` 后，首个 `submit-turn` 必须让 Runtime Bridge 看到新的 `sessionId`、空的当前 Projection、`contextReusePolicy='isolate'` 和 `currentTask.mode='fresh'`。Runtime Bridge 可以保留 workspace identity、scenario package、capability brief 和用户显式选择的 refs，但必须清空或忽略以下来源：

- 旧 session 的 `recentExecutionRefs`、`agentHarnessHandoff`、context envelope、selected refs 和 UI recent messages。
- 旧 session 或 archived session 的 `RecoverAction`、`failureRef`、bounded-stop、repair attempt、`repair-needed` / `failed-with-reason` 状态。
- AgentServer previous current work、recent turns、stable backend session id 或 repair continuation metadata，除非用户通过 `CrossSessionRef` / explicit import 明确导入为 evidence。

进入 `repair-current-session` 只能由两类同 session 事实触发：Projection 中仍有效的 recover action 被 `trigger-recover` 触发，或当前用户 turn 明确引用本 session 的 failure/run 并且 Context Bridge 能解析到同 session `failureRef`。Runtime Bridge、conversation policy 和 AgentServerAdapter 不得从 workspace/global latest run、archived session、历史 handoff snapshot 或 provider health 文本推断 repair mode。跨 session failure 即使被显式导入，也只能作为 evidence/ref 进入 fresh/continue，本身不能把新 session 首轮改写成 repair。

新聊天首轮的 context request 应满足：

```text
currentTask.mode = fresh
contextPolicy.contextReusePolicy = isolate
contextPolicy.includeCurrentWork = false
contextPolicy.includeRecentTurns = false
contextPolicy.persistRunSummary = false
currentTask.failureRef = undefined
refSelectionAudit.sourceCounts.failureEvidence = 0 unless user explicitly imported evidence
```
...
8. **Provider route 不可绕过**：Backend 可以组合能力，但不能在已有 capability/provider route 时生成 `urllib` / `requests` / raw network script、worker endpoint 直连或 provider-specific scraper 来替代 Gateway `execute`。
...
| C19 | Fresh session isolation | fresh-chat 首轮 request 必须为 `mode=fresh` + `contextReusePolicy=isolate`，且不含旧 session failure/repair/current-work/handoff refs |
| C20 | Capability-first provider route | fresh/repair tool generation 在已有 provider route 时必须产出 capability invocation/adapter；不得生成 `urllib`/`requests`/raw network bypass；preflight 拦截必须进入 Projection 可见 failure/recover action |
...
| fresh literature run 生成 `urllib` / `requests` 直连外网脚本 | preflight fail closed；Projection 显示 capability-first 约束失败、可用 provider route 摘要和 recover action，不停留在 projectionless waiting |
...
| fresh-chat first turn after failed/archived session | `contextReusePolicy='isolate'`，不包含旧 `failureRef`、recover actions、AgentServer current work、recent execution refs 或 repair continuation metadata |
