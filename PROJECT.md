...
- [ ] 不能通过修改 `PROJECT.md` 或输出建议来伪造 coding 成功。
- [ ] SciForge 必须真实读取目标代码或 selected file 内容。
- [ ] 生成 patch 时必须说明通用性、验证命令和风险。
- [ ] 如果需要人工接管，必须给出真实 blocker 和可执行下一步。
