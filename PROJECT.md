...
- [x] 设计独立输入通道：评估 macOS CGEvent / accessibility-per-window / 虚拟 HID / 远程桌面隔离 / 浏览器或 app sandbox 等通用方案，要求默认不移动用户真实鼠标、不抢用户键盘焦点；不可隔离时必须 fail closed 或要求显式确认。当前 trace 已输出 `inputChannelContract`：dry-run 是 `virtual-no-user-pointer-movement` / `virtual-no-user-keyboard-events`，配置 `inputAdapter` / `SCIFORGE_VISION_INPUT_ADAPTER=virtual-hid|remote-desktop|browser-sandbox|accessibility-per-window` 时标记 `sciforge-independent-input-adapter` 且 `userDeviceImpact=none`，macOS CGEvent 共享路径明确标注 `shared-system-pointer-keyboard` 和 `may-use-system-input-after-focused-target-verification`；CU-LONG real preflight 在没有独立适配器且未显式 `SCIFORGE_VISION_ALLOW_SHARED_SYSTEM_INPUT=1` 时 fail closed，并生成 scheduler repair action。
- [x] 增加 Computer Use scheduler：每个子 agent / scenario 绑定独立 target window、run directory、trace ledger 和输入锁；同一物理显示器或同一窗口上的真实执行不得并行，dry-run / 纯分析可并行。当前 trace/run/step 记录 `schedulerLockId`、`lockScope`、`actionConcurrency`、`analysisConcurrency`、`focusPolicy` 和 `interferenceRisk`；真实 executor 现在会通过 `/tmp/sciforge-computer-use-locks/<schedulerLockId>.lock` 获取跨进程 lease，step trace 写入 `scheduler.executorLease` 的 lock/acquire/release 证据；`schedulerLockTimeoutMs` / `schedulerStaleLockMs` 可由 request/env/config 注入，顶层 trace 写入 `scheduler.executorLock`，CU-LONG validator 会要求真实 GUI trace 具备该策略。
