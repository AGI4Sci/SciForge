...
P1、P2 与 P4 已完成本轮 strict-eval/fix/reverify 闭环；P4 续修复已关闭 `DISC-20260517-P4-003`，下一条 P4 hardening 优先处理 `DISC-20260517-P4-002` direct-answer coding evidence guard。下一步由 P3/P5/P6 继续各自 strict-eval 批次，或由 P2 开启下一条数据分析任务时优先覆盖 executable rerun command / selected chart follow-up / verification-required completion gate。每个进程继续按对应人类角色自由探索 SciForge；不要把任务剧本定死，也不要为了通过验收而写提示词。Worker 默认直接在 Web UI 点击查看、选择 artifact、reload、继续追问，并从 workspace 产物/records 中查证据；不要为每轮维护第二份 evidence。只有失败、假成功、修复对比、UI/workspace 不一致或 milestone 提交验收时，才升级保存截图/DOM/console/network/manifest。每个 milestone 结束后更新本文件、提交并 push GitHub、关闭上一批 sub agents，再启动下一批。
