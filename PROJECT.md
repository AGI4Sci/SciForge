...
- 验证：`node --import tsx --test packages/contracts/runtime/verification-policy.test.ts src/runtime/gateway/verification-policy.test.ts src/runtime/gateway/direct-context-fast-path.test.ts src/runtime/gateway/result-presentation-contract.test.ts src/runtime/gateway/agent-harness-shadow.test.ts src/ui/src/app/appShell/workspaceState.test.ts` 通过 98/98；`node --import tsx --test src/runtime/gateway/direct-context-fast-path.test.ts src/runtime/gateway/artifact-materializer.test.ts` 通过 53/53；`npm run typecheck` 通过。
...
- [ ] 中途改变关键约束，要求 SciForge 更新所有受影响结论。当前仍在修复：follow-up 已正确避开 direct-context planning register 并进入 backend，但 AgentServer/verification 边界仍会把可用 artifact 或 raw ToolPayload text 标为 needs-human / 长时间运行。
...
- [ ] audit/raw details 可审计但不得污染主结果。当前发现 direct-text guard 会把可解析 ToolPayload fragment 作为诊断主结果暴露，见 `DISC-20260517-P6-002`。
...
升级证据：同 session 下 artifacts `research-report.json` 与 `agentserver-direct-text-diagnostic-a458dd42a7.json`；用户可见主回复要求 human approval，未把已有 structured JSON 作为 final answer。
通用性说明：任何 AgentServer 以 plain text channel 返回可解析 ToolPayload JSON 时，gateway 应尽力 normalize，而不是把 artifact 变成诊断主结果；不依赖 mini grant、literature 场景、端口或 provider。
...
P1、P2、P3、P4 与 P5 已完成本轮 strict-eval/fix/reverify 闭环；P2 额外关闭 rerun command / selected chart grounding 边界：数据分析包 artifact 可见、workspace rerun 命令真实执行成功、selected chart follow-up 的 answer 与 audit refs 都只使用被选图表，并且 unverified 仍保持 honest degraded/partial。P1 额外补强 arXiv provider fallback：显式 arXiv query 现在走 arXiv API、保留 arXiv ID、无结果时 fail-closed，且未完成全文证据时仍不冒充 success。P5 已完成 longitudinal microbiome RCT protocol 任务：file-backed protocol artifact、durable evidence refs、needs-work/blocker projection、reload 后 72 libraries direct-context answer 均通过 Browser 复验；P4 已接手并关闭 `DISC-20260517-P5-001`，P5 新增关闭 `DISC-20260517-P5-002` / `DISC-20260517-P5-003`，并把 generated-task SyntaxError + supplement projection ambiguity 登记为 `DISC-20260517-P5-004` todo。P3 已关闭 `DISC-20260517-P3-001` 与 `DISC-20260517-P3-002`：direct-context read-only answer 的 visible unverified verification 现在是 non-blocking，fallback direct-context payload 也能从 payload 自身识别，selected report PASS/FAIL/literal facts 只用选中报告，旧 partial run 不再抢占 newer satisfied 结果焦点。下一步由 P6 继续 long-context deliverable iteration strict-eval，或由任一 owner 继续从 discovered queue/真实用户路径中选择新的失败边界。每个进程继续按对应人类角色自由探索 SciForge；不要把任务剧本定死，也不要为了通过验收而写提示词。Worker 默认直接在 Web UI 点击查看、选择 artifact、reload、继续追问，并从 workspace 产物/records 中查证据；不要为每轮维护第二份 evidence。只有失败、假成功、修复对比、UI/workspace 不一致或 milestone 提交验收时，才升级保存截图/DOM/console/network/manifest。每个 milestone 结束后更新本文件、提交并 push GitHub、关闭上一批 sub agents，再启动下一批。
