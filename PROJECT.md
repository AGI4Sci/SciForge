...
Browser Multiturn Stability Sprint 当前进入 Parallel Browser Stability & Speed milestone。下一步按 P1-P4 同时启动独立端口进程和 in-app browser：P1 测 fresh/continue 速度，P2 从 provider/tool latency 扩展到 capability-first 多轮连续、recover-to-ready、empty-result refinement 和 route restore，P3 测 repair bounded-stop，P4 测 refresh/artifact selection。所有发现必须记录 run/session id、timing、DOM/console evidence，并把慢点归因到 policy、transport、runtime、AgentServer、Projection 或 UI restore 边界后再修。
