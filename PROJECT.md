...
升级证据：当前不保存额外 DOM；用户可见页面包含 `Verification: 未验证`、`当前聚焦 run ... failed`、`Repair boundary diagnostic...`、`nextStep: Review the repair-boundary audit...`。
...
P1、P2、P3、P4 与 P5 已完成本轮 strict-eval/fix/reverify 闭环；P5 关闭 verification-required completion gate 的 fake-success 边界，并新增 `DISC-20260517-P5-001` 追踪 AgentServer repair-boundary 对 generated-task output containment 的后续治理。P3 仍有 `DISC-20260517-P3-001` 追踪 direct-context answer 内容正确时被 verification gate 标为 partial/unverified。下一步由 P6 继续 long-context deliverable iteration strict-eval，或由任一 owner 优先处理 discovered queue 中的 P3/P5 未关闭项。每个进程继续按对应人类角色自由探索 SciForge；不要把任务剧本定死，也不要为了通过验收而写提示词。Worker 默认直接在 Web UI 点击查看、选择 artifact、reload、继续追问，并从 workspace 产物/records 中查证据；不要为每轮维护第二份 evidence。只有失败、假成功、修复对比、UI/workspace 不一致或 milestone 提交验收时，才升级保存截图/DOM/console/network/manifest。每个 milestone 结束后更新本文件、提交并 push GitHub、关闭上一批 sub agents，再启动下一批。
