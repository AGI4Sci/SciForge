...
状态：已完成；本轮已完成 contract/package boundary、UI 大文件两轮拆分、runtime gateway/project/server 四轮拆分、文档漂移修正和 module boundary smoke。`packages/support/object-references` / `packages/support/artifact-preview` 已不再反向依赖 `src/ui/src/domain.ts`；`domain.ts` 已降到约 658 行，稳定 session/message/stream/view/execution/event contract 已下沉到 `packages/contracts/runtime/*`；`ChatPanel.tsx`、`SciForgeApp.tsx`、`ResultsRenderer.tsx`、`Dashboard.tsx`、`ShellPanels.tsx`、`task-projects.ts`、`generation-gateway.ts`、`workspace-server.ts` 已降到 1500 行以下；`workspace-server.ts` 已抽出 file preview、workspace open gateway、HTTP primitives 和 scenario/task-attempt/skill-proposal routes，入口回到 route 注册与服务启动。本任务基于 2026-05-09 对 `src` 和 `packages` 的结构巡检，目标是把“产品 app、runtime 执行壳、策略算法、能力 package、稳定 contract、交互 renderer”拆成更清晰的所有权边界，让多人并行开发时可以按目录负责、按 contract 集成、按 smoke 守门，而不是在 `SciForgeApp.tsx`、`ChatPanel.tsx`、`workspace-server.ts`、`generation-gateway.ts` 这类大入口文件里互相踩线。
...
- [x] 拆分 `src/runtime/workspace-server.ts`：按 `routes/config`、`routes/workspace-state`、`routes/feedback-repair`、`routes/file-preview`、`routes/scenario-library`、`routes/workspace-open`、`server/http` 组织；入口只注册 routes 和启动 server。已抽出 `src/runtime/server/file-preview.ts`、`workspace-open.ts`、`http.ts` 与 `scenario-library-routes.ts`，scenario/task-attempt/skill-proposal CRUD 不再堆在入口文件中，`workspace-server.ts` 降到约 1116 行并退出 1500 行风险区。验证命令：`npm run typecheck`、`npm run smoke:long-file-budget`、`npx tsx tests/smoke/smoke-workspace-scenarios.ts`、`npx tsx tests/smoke/smoke-task-attempt-api.ts`、`npx tsx tests/smoke/smoke-workspace-file-api.ts`、`npx tsx tests/smoke/smoke-workspace-instance-feedback-api.ts`、`npx tsx tests/smoke/smoke-stable-version-registry.ts`。
...
- [x] Thread C4 - Runtime scenario route split：已完成；新增 `src/runtime/server/http.ts` 与 `src/runtime/server/scenario-library-routes.ts`，从 `workspace-server.ts` 抽出 HTTP JSON/NDJSON helpers、scenario package CRUD、task-attempt APIs 和 skill-proposal APIs；`workspace-server.ts` 降到约 1116 行。验证命令：`npm run typecheck`、`npm run smoke:long-file-budget`、`npx tsx tests/smoke/smoke-workspace-scenarios.ts`、`npx tsx tests/smoke/smoke-task-attempt-api.ts`、`npx tsx tests/smoke/smoke-workspace-file-api.ts`、`npx tsx tests/smoke/smoke-workspace-instance-feedback-api.ts`、`npx tsx tests/smoke/smoke-stable-version-registry.ts`。
...
- [x] 主要 app/runtime 入口文件回到编排职责，超过 1500 行的非生成源码均有已执行的语义拆分或明确豁免。本轮已拆 `ChatPanel`、`SciForgeApp`、`ResultsRenderer`、`Dashboard`、`ShellPanels`、`task-projects`、`workspace-server`、`generation-gateway`；当前非生成源码均已低于 1500 行，`workspace-server.ts` 约 1116 行。
