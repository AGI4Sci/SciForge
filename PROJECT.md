...
进度备注：`smoke:fixed-platform-boundary` 当前剩余 0 个 tracked warnings；`generated-task-runner.ts` 在 `smoke:no-src-capability-semantics` 中已清零到 artifact 0 / domain regex 0 / provider 0；`direct-answer-payload.ts` 的 standalone workspace artifact 包装、existing-context source 和 markdown fence 清理语义已迁到 `packages/presentation/interactive-views/direct-answer-result-policy.ts`，该文件 no-src baseline 从 artifact 5 / domain regex 1 清零到 0 / 0；`skill-promotion.ts` 的 ledger promotion domain default routing 已迁到 `packages/skills/runtime-policy.ts`，该文件 no-src domain-default baseline 从 1 降到 0；`observe/orchestration.ts` 的 provider-unavailable diagnostic helper 已迁入 `packages/contracts/runtime/observe.ts`，该文件 no-src provider baseline 从 1 清零到 0；`payload-validation.ts` 的 preview/file-kind/extension acceptance 语义已迁到 `packages/support/artifact-preview`，gateway 只调用 package helper，该文件 no-src domain regex baseline 从 1 降到 0；`smoke:no-src-capability-semantics` 当前收敛到 424 个 tracked findings，无新增；`workspace-event-normalizer.ts` 的 no-legacy provider/prompt special-case baseline 已清零，`smoke:no-legacy-paths` 当前收敛到 19 个 tracked findings，无新增。
