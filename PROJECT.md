...
- 所有修改必须通用、可泛化到任何场景，不能在代码里面硬编码和为当前案例打补丁;skills, tools等插件化能力尽可能做到即插即用、易拓展
- 有多种修改方案的时候，优先实现最简洁、通用的方案
- 算法相关的代码优先用Python实现，方便人类用户优化、检查算法
- 代码路径保持唯一真相源：发现冗余链路时删除、合并或降级旧链路，避免长期并行实现。
- 设计和实现保持同一真相源：如果真实 browser 使用、conformance、性能或用户体验证明关键设计不满足需求，必须在同一 milestone 中同步修改设计文档和代码；不能只改代码绕过 contract，也不能只改文档掩盖实现缺口。设计变更至少更新 [`docs/SciForge-SingleAgent-Architecture.md`](docs/SciForge-SingleAgent-Architecture.md) 或相关背景文档，并在 `PROJECT.md` 记录 evidence、失败边界和拒绝的兼容方案。
- Capability 必须在生成层成为可执行 authoring contract：当系统已有 ready provider/tool route 时，Backend prompt 不能只收到抽象规则或诊断摘要，必须收到标准 helper/API 签名、任务输入字段和可复制 adapter skeleton；运行时 preflight/guard 只作为最后防线，不能替代正确的工具优先生成。新增 tool 的最小接入闭环是 CapabilityManifest → ProviderManifest/route resolver → HarnessContract decision → compact context envelope → authoring contract/helper SDK → Gateway.execute → validator/preflight → ArtifactDelivery/Projection → conformance fixture。
...
- [x] SA-ARCH-01：新增最终 runtime contract 文档 [`docs/SciForge-SingleAgent-Architecture.md`](docs/SciForge-SingleAgent-Architecture.md)，并把旧 `ProjectSessionMemory.md` 内容吸收进去。Evidence：最终文档定义 Workspace Kernel、AgentServer Context Core、Capability Gateway、Runtime Bridge、Projection-only UI 与 C01-C18；`smoke:docs-scenario-package` 验证文档入口可读；失败边界：旧记忆文档不得再作为第二 contract；拒绝兼容方案：不保留旧 ProjectSessionMemory 文档作为并行权威。
- [x] SA-ARCH-02：更新 [`docs/README.md`](docs/README.md) 和 [`docs/Architecture.md`](docs/Architecture.md)，将 `SciForge-SingleAgent-Architecture.md` 设为最终 runtime contract，`Architecture.md` 降级为当前实现背景。Evidence：docs README/Architecture 指向最终 contract，Architecture 仅记录实现背景与 guard inventory；`smoke:docs-scenario-package` 通过；失败边界：Architecture 不得覆盖最终文档；拒绝兼容方案：不在背景文档重新定义 runtime contract。
- [x] SA-ARCH-03：删除旧 [`docs/ProjectSessionMemory.md`](docs/ProjectSessionMemory.md)，避免文档第二真相源。Evidence：旧文件已删除，历史内容归档/吸收到最终文档；`smoke:docs-scenario-package` 校验旧 docs 不存在或仅 archive/historical 引用；失败边界：任何新增旧权威入口都应失败；拒绝兼容方案：不保留 stub 文档继续承载实现说明。
...
- [x] SA-DEL-02：删除或迁移 `handoffMemoryProjection`、`memoryPlan`、`availableSkills` 这类旧兼容字段；如果 backend/harness 仍需读取，改成 explicit `AgentServerContextRequest.contextRefs` / `capabilityBriefRef` / `cachePlan`。Evidence：conversation-policy response/service、context envelope、prompt snapshot 和 planner payload 主路径改为 `contextProjection`、`contextRefs`、`capabilityBriefRef`、`cachePlan`；UI/runtime skill selection 主路径改为 `selectedSkillIds`，旧 `availableSkills` 只作为 ingress/deprecated alias 或 prompt-forbidden negative fixture；旧 `handoffMemoryProjection` 只作为 explicit migration alias 被读取并记录 rename decision；`agentserver-context-contract` forbidden-field guard、prompt/context/planner tests、C01-C18 smoke 与 typecheck 通过。
...
- [x] SA-DEL-06：清点所有 `legacy` / `compatibility` / `fallback` 命名；凡是仍参与主流程的条目必须迁移到最终 contract 或删除，不能只改文案。Evidence：`tools/check-no-legacy-paths.ts` 扩展 Gateway public API、direct-context implicit strategy、provider discovery endpoint leak guards；当前剩余 legacy/fallback 只以冻结 baseline warning 输出，新增/增加会失败；`npm run smoke:no-legacy-paths` 通过；拒绝兼容方案：不新增主流程 fallback baseline。
- [x] SA-DEL-07：删除 `src/runtime/gateway/direct-context-fast-path.ts` 中的 Runtime 本地 prompt regex/关键词判断和本地回答模板；保留时只能作为 `DirectContextDecision` consumer，不允许生成 strategy。Evidence：`directContextFastPathPayload()` 现在必须消费显式 `DirectContextDecision`，`agentHarness` / `turnExecutionConstraints` / `intentMode` 暗示不再生成 strategy；`direct-context-fast-path.test.ts` 覆盖无 `DirectContextDecision` 时让路 AgentServer；`smoke:single-agent-runtime-contract` C06 静态 guard 与 `smoke:no-legacy-paths` 通过。
...
- [x] SA-KERNEL-01：实现明确 `StorageAdapter` interface，支持 SQLite/filesystem/in-memory adapter，但对外统一 `appendEvent` synchronous-on-write。Evidence：`WorkspaceStorageAdapter` 明确 `contractVersion: sciforge.workspace-storage-adapter.v1`、`kind: in-memory|filesystem|sqlite`、`synchronousWrites: true`，`createWorkspaceKernel()` 对 adapter contract 做 runtime guard；当前 concrete in-memory adapter 通过 `appendEvent` 同步写 ledger/projection/ref，filesystem/sqlite 作为同一 contract family 接入。
...
- [x] SA-KERNEL-03：将小事实内联到 `WorkspaceEvent`，大正文/长日志/snapshot/audit bundle 才注册为 `ProjectMemoryRef`；新增 guard 防止 health/degraded/failure 摘要被无脑 ref 化。Evidence：`validateConversationEvent()` 对 `DegradedResult` / `ExternalBlocked` / `RepairNeeded` / `NeedsHuman` 的 ref event 强制保留 inline summary/reason，小事实不能只躲进 refs；大 inline payload 仍由 `inline-payload-too-large` guard 拦截。`conversation-kernel.test.ts` 与 `smoke:conversation-kernel-final-shape` 覆盖。
- [x] SA-KERNEL-04：实现 `RefKindGroup` 派生 retention，禁止调用方逐个传任意 retention；archive/pin/delete/tombstone 全部 append event。Evidence：`ProjectMemoryRef` retention 继续由 `RefKindGroup` 派生，`WorkspaceKernel.registerRef()`/adapter `putRef()` 对 per-ref retention override 抛出 `ref-retention-override-forbidden`；新增 `RefArchived` / `RefPinned` / `RefDeleted` / `RefTombstoned` append-only event types 并要求 inline reason/summary。`conversation-kernel.test.ts`、`project-session-memory.test.ts` 与 smoke 覆盖。
- [x] SA-KERNEL-05：实现 `CrossSessionRef` / explicit import 记录，禁止复制裸路径跨 session 充当记忆。Evidence：新增 `CrossSessionRef` 与 `ExplicitImportRecorded`/`sciforge.explicit-import-event.v1` 最小 contract；validator 要求 source/target session、digest、importedRef，并拒绝 `sourceRef === importedRef` 的裸路径跨 session copy。`project-session-memory.ts` 将 explicit import 归一为 `explicit-import-recorded` ledger fact，targeted tests 与 smoke 覆盖。
...
- [x] SA-CONTEXT-08：新增 `src/runtime/gateway/agentserver-context-contract.ts`，集中定义 `AgentServerContextRequest`、`AgentServerContextResponse`、`BackendHandoffPacket`、`DegradedHandoffPacket` 和 canonical serialization。Evidence：contract 文件集中定义 request/response/handoff/degraded packet、forbidden-field guard 与 canonical serializers；`agentserver-context-contract.test.ts`、C02-C05/C13 smoke 与 typecheck 通过；失败边界：raw history/full refs/legacy fields fail closed；拒绝兼容方案：不在 prompt/context envelope 内散落第二套 context contract。
- [x] SA-CONTEXT-09：替换 `src/runtime/gateway/agentserver-prompts.ts` 中直接组装 prompt/currentTurnSnapshot 的本地 context 包；AgentServer prompt renderer 只能消费 `BackendHandoffPacket` / bounded render plan。Evidence：generation prompt current-turn snapshot 改为 `BackendHandoffPacket` refs-first projection + bounded render plan summary，legacy promptRenderPlan/raw renderedText carriers 被忽略；`node --import tsx --test src/runtime/gateway/agentserver-prompts.test.ts` 通过。
- [x] SA-CONTEXT-10：合并或废弃 `src/runtime/gateway/conversation-handoff-planner.ts` 与 `src/runtime/workspace-task-input.ts` 中重复的 handoff budget/compaction/audit 逻辑，只保留一个 canonical handoff normalizer。Evidence：`normalizeCanonicalHandoffValue()` / `CANONICAL_HANDOFF_FORBIDDEN_KEYS` 从 `workspace-task-input.ts` 导出并被 `conversation-handoff-planner.ts` 复用，统一 forbidden carrier 清理与数组去重；`node --import tsx --test src/runtime/gateway/conversation-handoff-planner.test.ts src/runtime/workspace-task-input.test.ts` 通过。
...
- [x] SA-GATEWAY-01：Runtime Bridge 只能调用 `Gateway.execute`；`resolveRoute` / `preflight` / `invoke` / `materialize` / `validate` 改为内部 API，并加 lint/contract test 防外部调用。Evidence：`tools/check-no-legacy-paths.ts` 与 `smoke-single-agent-runtime-contract.ts` 新增 execute-only public API guard，禁止 Runtime Bridge 外部调用 `Gateway.resolveRoute/preflight/invoke/materialize/validate`；`npm run smoke:no-legacy-paths` 与 `npm run smoke:single-agent-runtime-contract` 通过；拒绝兼容方案：不把 internal stage 暴露成可见 Runtime strategy。
- [x] SA-GATEWAY-02：`Gateway.validate` 只做结构校验；科学/语义判断必须来自 Backend 或 verifier capability 的 `verification-record`。Evidence：`payload-validation.ts` 暴露 `validateToolPayloadStructure()`，仅返回 ToolPayload schema shape errors；`payload-validation.test.ts` 断言 plan-only 但结构合法的 payload 不被结构 validator 做语义完成判断，后续 completion/work-evidence 仍作为 post-structure guard；targeted gateway tests、`smoke:single-agent-runtime-contract` 通过；拒绝兼容方案：不在结构 validator 内推断科学正确性或多轮语义。
...
- [x] SA-GATEWAY-04：Worker discovery 统一归一成 `ProviderManifest`，不得把单个 endpoint shape 泄漏给 Backend 或 Runtime Bridge。Evidence：`requestWithDiscoveredCapabilityProviders()` 只写入 provider manifest/health 投影，不再 spread AgentServer worker record；discovery payload 中的 `endpoint/baseUrl/invokeUrl/workerId/runtimeLocation` 被测试禁止泄漏；`capability-provider-preflight.test.ts`、`smoke:no-legacy-paths` 与 `smoke:single-agent-runtime-contract` 通过。
- [x] SA-GATEWAY-05：scenario package contract 改为 policy-only guard：禁止 execution code、prompt regex、provider branch、多轮 semantic judge、preset answer/system prompt。Evidence：`smoke-single-agent-runtime-contract.ts` C18 新增 policy-only negative fixture，拒绝 `executionCode`、`promptRegex`、`providerBranches`、`multiTurnSemanticJudge`、`answerTemplate/systemPrompt`；`npm run smoke:single-agent-runtime-contract` 通过；拒绝兼容方案：不允许 scenario package 携带执行代码、provider 分支或预设答案模板。
...
- [x] SA-RUNTIME-05：FailureNormalizer 输出 `failureClass`、`recoverability`、`owner`、`failureSignature`；是否 repair 只由 TurnPipeline.onFailure + RepairPolicy 决定。Evidence：新增 `normalizeRuntimeFailure()` 稳定 signature 与 class/recoverability/owner 推导，TurnPipeline failure path 只 normalize 并委托 `onFailure`；runtime test 与 C09 smoke 覆盖。
- [x] SA-RUNTIME-06：direct-context fast path 改为结构化 `DirectContextDecision`；Runtime Bridge/UI 不得用关键词、本地模板或 artifact kind 直接回答。Evidence：`directContextFastPathPayload()` 只消费显式结构化 decision；无 `DirectContextDecision`、仅 harness hints、turn constraints 或 prompt-only 输入时让路 AgentServer；`direct-context-fast-path.test.ts`、`smoke:web-final-conformance`、`smoke:single-agent-runtime-contract` 通过。
- [x] SA-RUNTIME-07：Harness policy 只输出 decision/contract/trace refs；Runtime Bridge 只把它们作为 context refs，不解释领域语义。Evidence：新增 `HarnessPolicyRefs` contract 与 `normalizeHarnessPolicyRefs()`，TurnPipeline 只保存 `harnessPolicyRefs` / contextRefs，不消费 policy body；runtime test 覆盖无 `policy` / `domainSemantics` body，no-legacy guard 禁止 conversation-kernel inline harness semantic policy body。
- [x] SA-RUNTIME-08：补 conformance：C06、C09、C10、C11、C16、Runtime Bridge 最小验收用例。Evidence：`smoke-single-agent-runtime-contract.ts` C08/C09/C11 改为调用真实 `EventRelay`、`WriteAheadSpool`、`FailureNormalizer` 和 `TurnPipelineDefinition`；targeted runtime/direct-context/UI tests、`npm run typecheck`、`npm run smoke:single-agent-runtime-contract`、`npm run smoke:no-legacy-paths` 通过。
- [x] SA-RUNTIME-09：`DirectContextDecision` 必须包含 `decisionRef`、`decisionOwner`、`requiredTypedContext`、`usedRefs`、`sufficiency`；无 decision 或 insufficient 时必须 route-to-agentserver。Evidence：`normalizeDirectContextDecision()` 要求 `decisionRef`、`decisionOwner`、`requiredTypedContext`、`usedRefs`、`sufficiency`；缺字段或 `sufficiency=insufficient` 均 fail-closed 并 route-to-agentserver；`direct-context-fast-path.test.ts`、`smoke:web-final-conformance`、C16 runtime smoke 通过。
...
- [x] SA-UI-01：所有 UI 写操作收口为 `UIAction`：submit-turn、trigger-recover、cancel-run、concurrency-decision、open-debug-audit；UI 不直接写 Kernel。Evidence：`uiActionBoundary.ts` 提供五类 typed UIAction creator 与 session-level audit log，`ChatPanel` 的 submit/auto-run/edit-submit/cancel/concurrency 均通过 boundary，`ResultsRenderer` 的 Projection recover 与 debug audit 打开动作生成 `trigger-recover` / `open-debug-audit`，`SciForgeWorkbench` 统一记录结果区 UIAction；`uiActionBoundary.test.ts` 覆盖五类 action、session audit log、app-level typed creator/static Kernel-write guard，targeted UI tests 与 `smoke:no-legacy-paths` 通过。
- [x] SA-UI-02：UI terminal state 只来自 Projection；`answer-delta` 只能作为 transient display，和 Projection 冲突时丢弃 transient。Evidence：`conversationProjectionForSession()` 成为 results/chat/archive/workspace terminal source；`smoke:web-final-conformance` 覆盖 stale stream delta 输给 Projection terminal，`smoke:single-agent-runtime-contract` C12 与 `verify:single-agent-final` 通过。
- [x] SA-UI-03：删除 results/archive/chat 中 raw run/backend stream 推断 terminal state 的逻辑；历史 raw 内容只能 audit/debug 展示。Evidence：`ResultsRenderer.test.ts`、`ArchiveDrawer.test.tsx`、`ChatPanel.test.ts` 和 `workspaceState.test.ts` 覆盖 raw failure/recover/verification/displayIntent 不驱动主展示；SA-WEB-14 no-legacy UI raw fallback case 和 `smoke:no-legacy-paths` 通过。
- [x] SA-UI-04：按 `ArtifactDelivery.previewPolicy` 分流 inline/open-system/audit-only/unsupported，禁止 JSON/raw fallback 伪装为主结果。Evidence：`viewPlanResolver.ts` / `results-renderer-registry-slot.tsx` 只从 Projection + user-facing ArtifactDelivery 生成主 plan，open-system/unsupported 走系统/unsupported 状态，audit-only/diagnostic 不入主结果；`viewPlanResolver.test.ts`、`smoke:web-final-conformance` 与 Web E2E SA-WEB-09 通过。
- [x] SA-UI-05：Debug panel 只消费 RunAudit/context snapshot/audit refs；用户可见失败原因来自 Projection 的 visibleAnswer/recoverActions。Evidence：`results-renderer-execution-model.ts`、`RunExecutionProcess.tsx`、`ArchiveDrawer.tsx` 通过 Projection diagnostics/recoverActions/auditRefs 决定可见失败与审计入口；raw legacy resultPresentation 保留在 audit/debug negative fixtures；targeted UI tests 与 `smoke:web-multiturn-final` SA-WEB-10/14 通过。
- [x] SA-UI-06：补 conformance：C12、C17、UI/Projection 最小验收用例和 browser smoke。Evidence：新增 `smoke:web-final-conformance` 并接入 `verify:single-agent-final`；本轮 targeted UI suite（ChatPanel/ResultsRenderer/workspaceState/ArchiveDrawer/RunExecutionProcess/viewPlanResolver）通过；完整 `npm run verify:single-agent-final` 通过。
- [x] SA-UI-07：`src/ui/src/app/conversation-projection-view-model.ts` 的 `conversationProjectionForRun` 数据源迁移为 session-level materialized `ConversationProjection`；`run.raw` / `run.response` 内嵌 projection 只允许 migration/audit fixture。Evidence：`conversationProjectionForSession(session, run)` 优先读取 `materializedConversationProjection(s)` / `currentConversationProjection` / session projection map；旧 run-raw reader 重命名为 `conversationProjectionMigrationAuditFixtureForRun()` 且只在 tests/migration fixture 中使用；typecheck、targeted UI tests 与 `verify:single-agent-final` 通过。
...
- [x] SA-CONF-04：新增 browser smoke：Projection terminal 胜过 stream delta、ArtifactDelivery audit-only 不可见、direct-context insufficient 必须 route-to-agentserver。Evidence：新增 `tests/smoke/smoke-web-final-conformance.ts`，复用 Web final fixture 校验 Projection-over-delta、ArtifactDelivery audit-only visibility 和 insufficient DirectContextDecision route-to-agentserver；`package.json` 将 `smoke:web-final-conformance` 接入 `verify:single-agent-final`，`smoke:web-final-conformance`、`smoke:single-agent-final-gate` 与 `typecheck` 通过。
...
- [x] SA-CONF-10：新增最终完成 gate 命令 `npm run verify:single-agent-final`，串联 typecheck、核心单测、C01-C18、browser web-multiturn-final；任何一步失败都不能标记最终版本完成。Evidence：`package.json` 的 `verify:single-agent-final` 串联 `typecheck`、核心测试、`smoke:single-agent-runtime-contract` C01-C18、`smoke:no-legacy-paths`、`smoke:single-agent-final-gate`、`smoke:web-multiturn-final` 和 `smoke:single-agent-final-evidence`；`npm run verify:single-agent-final` 通过。
...
##### SF-STAB-008 - P1 - Fresh literature run generates direct urllib network script despite ready provider routes

- 发现者：Orchestrator Round 6
- 修复者：Orchestrator Round 7
- 来源：真实 in-app browser 使用，不是 terminal-only check。
- 前置状态：`SF-STAB-007` repair-context 污染已由本轮修复；新聊天首轮不再进入 `context=repair` / repair bounded-stop。打开真实 in-app browser `http://127.0.0.1:5173/`，进入 `literature-evidence-review@1.0.0`。
- 复现步骤：
  1. 点击 `开启新聊天`，确认页面显示 `新聊天已就绪`，输入框 context window 为 `0%`。
  2. 发送 fresh literature/retrieval prompt，例如：`请用最小检索验证 arXiv 是否可访问：搜索 CRISPR prime editing review，返回 3 篇候选论文标题和来源；如果外部 provider 不可用，请给出可恢复失败原因，不要编造结果。`
  3. 观察首个 run 的 process trace 和结果区。
- 期望行为：fresh literature run 必须 capability-first：Backend 选择 `web_search` / `web_fetch` / related retrieval capability，Gateway/AgentServer policy 解析 provider route；如果 provider route 不可用或权限不足，应返回合法 `provider-unavailable` / `failed-with-reason` Projection，不应生成 `urllib`、`requests`、raw socket 或站点直连脚本绕过 Gateway。
- 原实际行为：首轮 fresh run 已不再被历史 repair context 污染，但 AgentServer 生成了包含 `urllib` 直接外网访问的脚本；preflight 正确拦截 direct external network API。主结果仍等待 ConversationProjection。
- 根因：ready provider 没有在生成层成为“默认可执行工具”。AgentServer prompt 只有 provider route 摘要和 prohibition 文案，真正的 `sciforge_task.invoke_provider(task_input, capabilityId, input)` helper/API 直到任务执行输入阶段才完整出现；模型在生成 Python 时退回熟悉的 `urllib`/`requests` 默认模式。
- 修复说明：将 provider-first 从运行时拦截上移为生成 authoring contract：AgentServer generation prompt 现在包含 helper SDK 模块名、`invoke_provider(task_input, ...)` 签名、任务输入字段和 canonical Python adapter skeleton；`agentServerCapabilityRoutingPromptPolicyLines()` 明确 authoring template。运行时保留 capability-first payload preflight 和一次 strict retry 作为最后防线；preflight fail-closed 的 runtime diagnostic 现在能通过 ConversationProjection 进入右侧主结果，不再停在 projectionless empty。
- 文件变更：`src/runtime/gateway/agentserver-generation-prompts.ts`、`packages/skills/runtime-policy.ts`、`src/runtime/gateway/generated-task-runner-generation-lifecycle.ts`、`src/runtime/gateway/generated-task-runner-generation-lifecycle.test.ts`、`src/ui/src/app/conversation-projection-view-model.ts`、`src/ui/src/app/results-renderer-execution-model.test.ts`、相关 prompt/preflight/execution lifecycle tests。
- 验证命令：
  - `node --import tsx --test src/runtime/gateway/agentserver-prompts.test.ts src/runtime/gateway/generated-task-runner-generation-lifecycle.test.ts packages/skills/runtime-policy.test.ts src/runtime/gateway/generated-task-payload-preflight.test.ts src/runtime/gateway/generated-task-runner-execution-lifecycle.test.ts` 通过，21 tests。
  - `node --import tsx --test src/ui/src/app/results-renderer-execution-model.test.ts src/ui/src/app/ResultsRenderer.test.ts src/ui/src/app/results/viewPlanResolver.test.ts src/ui/src/app/chat/RunExecutionProcess.test.ts src/ui/src/app/chat/sessionTransforms.test.ts src/ui/src/app/ChatPanel.test.ts` 通过，104 tests。
  - `npm run typecheck`、`npm run smoke:single-agent-runtime-contract`、`npm run smoke:no-legacy-paths` 通过。
- 浏览器复验状态：刷新旧失败 session 后，右侧已从“主结果等待 ConversationProjection”变为可恢复的 `运行需要恢复`，显示 capability-first failure reason 和 recover action。仍需在新 dev runtime 下再跑一次 fresh provider prompt，验证生成层 adapter skeleton 能降低直连脚本发生率；即使 backend 再次绕过，strict retry/visible Projection 应 fail closed。
- 为什么是通用修复：任何 Backend 在 ready provider/tool route 存在时都应收到“如何写工具优先任务”的结构化 authoring contract，而不是靠后置 preflight 猜测；该修复不依赖文献场景、arXiv、单个 prompt 或单一 backend。
...
- 浏览器复验状态：Orchestrator 已在真实 in-app browser 恢复 Workspace Writer URL 到 `http://127.0.0.1:5174` 并重跑文献场景。后续 repair continuation 最新 run `project-literature-evidence-review-mp6rl5bt-m6nutb · failed · recoverable` 没有再次出现 bounded-stop，而是在 AgentServer 模型渠道 `503 Service Unavailable` 处可恢复失败；browser 本地 console error 为空。该条作为稳定性证据保留；模型渠道 503 属于外部可用性观察，不再作为 single-agent final cutover 的架构迁移缺口。

#### Verified

##### SF-STAB-007 - P1 - Fresh new chat inherits repair context and bounded-stops on first prompt

- 发现者：Orchestrator Round 5
- 修复者：本轮 fixer workers
- 来源：真实 in-app browser 使用，不是 terminal-only check。
- 前置状态：`main` at `0b53293`，dev services freshly started；`verify:single-agent-final` 已通过；打开真实 in-app browser `http://127.0.0.1:5173/`，进入 `literature-evidence-review@1.0.0`。
- 复现步骤：
  1. 点击 `开启新聊天`，确认页面显示 `新聊天已就绪`，输入框 context window 为 `0%`。
  2. 发送 fresh prompt：`请用最小检索验证 arXiv 是否可访问：搜索 CRISPR prime editing review，返回 3 篇候选论文标题和来源；如果外部 provider 不可用，请给出可恢复失败原因，不要编造结果。`
  3. 观察首个 run 的 process trace 和结果区。
- 期望行为：新聊天首轮 fresh request 应走 fresh/retrieval context，不应继承历史 failed run 的 repair context；如果 provider/model 不可用，应返回可读的 provider/model failure 或合法 `failed-with-reason` Projection，不应进入 repair continuation bounded-stop。
- 原实际行为：首轮 fresh request 的 process trace 显示 `context=repair; executionMode=repair-or-continue-project; digests=0; capabilities=8`，随后 run 失败为 `AgentServer repair generation bounded-stop after 86019 total tokens (limit 60000)`；右侧主结果仍等待 ConversationProjection。
- 修复/验证证据：真实 browser 复验显示新聊天首轮已不再进入 `context=repair` / `repair-or-continue-project`，也不再触发 repair bounded-stop；fresh run 已进入 fresh literature/provider path。后续新发现是 capability-first provider route 约束缺口，已另拆为 `SF-STAB-008`。
- 为什么是通用修复：新 session/current turn 边界必须清空或隔离旧 session 的 `contextReusePolicy`、`recentExecutionRefs`、`agentHarnessHandoff`、`RecoverAction`、`failureRef`、AgentServer current work 和 stable AgentServer session id；不依赖文献场景或单个 prompt。
...
- 2026-05-16 - Orchestrator Round 6 - 真实 browser 复验确认 `SF-STAB-007` repair-context 污染已由本轮修复：fresh new chat 不再继承历史 repair context 或触发 repair bounded-stop。新发现 `SF-STAB-008`：fresh literature run 仍生成 `urllib` 直接外网脚本，被 capability preflight 正确拦截，但主结果仍等待 Projection；文档同步 capability-first provider route 约束，代码修复另行认领。
- 2026-05-16 - Orchestrator Round 7 - 修复 `SF-STAB-008` 的根因和防线：ready provider/tool route 在 AgentServer generation prompt 中升级为可执行 authoring contract，新增 generic `sciforge_task.invoke_capability` helper alias，保留 direct-network preflight strict retry 和 Projection 可见 runtime diagnostic；补新 tool context injection 最小 contract 文档。`npm run verify:single-agent-final` 通过。

### Current Handoff

当前最高优先级仍是 `SF-STAB-008` 的真实 fresh browser 复验。不要 reopen `SF-STAB-007`：fresh new chat repair-context 污染已修复。下一轮应在新 dev runtime 下重跑 fresh literature/web prompt，重点验证 AgentServer generation prompt 的 canonical provider-first adapter skeleton 是否让生成任务直接使用 `sciforge_task.invoke_provider(task_input, ...)`；若 backend 仍生成 `urllib` / `requests`，确认 strict retry 发生且最终右侧为 Projection 可见 failed-with-reason/recover action，而不是等待 Projection。
