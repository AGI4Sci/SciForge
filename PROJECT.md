...
状态：完成；`packages/agent-harness` 已成为行为治理入口，context、broker、prompt metadata、validation、repair audit、progress、verification、backend selection、continuity、recovery 与 stream guard 均通过 contract/stage hook/trace 消费，不再保留开放式 harness TODO。
...
2026-05-10：T130 guarded repair-consume experiment 首片完成。`validation-repair-audit-bridge` 低层 projection 也改为只有 `consume === true` 才收紧 repair 行为，runtime-state 新增 consume-only 显式 flag 与 consume kill switch；smoke 覆盖 default audit-only、显式 consume fail-closed、kill switch 保留 audit projection 但关闭行为消费。
...
- [x] T130 guarded repair-consume experiment：只在显式 flag 下把 canonical repair policy 推进到 contract-owned 行为收紧，默认继续 audit-only；smoke 覆盖 payload/runtime verification bridge、observe/action finding model regression、repair rerun policy shape 与 consume kill switch。当前结论是不升级默认，保留显式 consume。
...
状态：完成；所有可调度、可审计或可治理的 package/core capability 面已通过统一 `CapabilityManifest`、`HarnessCandidate`、`CapabilityBudget` 与 `CapabilityBudgetDebit` 进入 broker/harness 治理；普通 internal helper 明确不为清单完整性强行 manifest 化或造账。
...
2026-05-10：T129 manifest breadth inventory 继续收口。默认 core registry 新增真实 runtime verifier manifest：`sciforge.payload-validation` 与 `sciforge.runtime-verification-gate`，覆盖 payload schema/completed/current-ref gate 与 runtime verification gate；两者都已在 registry、harness candidate projection 与 broker audit smoke 中可见，普通 observe/core internal helper 继续不 manifest 化。

2026-05-10：T129 budget debit inventory 小切片完成。remaining core runtime invocation surface 盘点结果：observe invocation 已覆盖；workspace open gateway 是真实用户触发 action 且带外部打开 side effect，已新增 `runtime.workspace-open` debit，并回挂 `executionUnit` / `workEvidence` / `audit` / 顶层 `budgetDebits`，debit id 由 action/workspace/path/dryRun 稳定生成；`runtime.artifact-list` 与 `runtime.run-resume` 后续仍只在真实 backend tool invocation 返回面补账，`resolveWorkspaceOpenPath` 等 internal helper 不造账。

2026-05-10：T129 budget debit inventory 收尾完成。`listSessionArtifacts()` 与 `resumeRun()` 作为真实 backend artifact tool invocation 返回面，分别写入 `runtime.artifact-list` 与 `runtime.run-resume` debit，并回挂 `budgetDebits`、`executionUnit`、`workEvidence` 和 audit refs；ref parsing、path resolution、JSON hydration 等 internal helpers 继续不造账。

Todo：

- [x] T129 manifest breadth inventory：只补齐可被 broker/harness 选择、组合、计预算、验证、修复、渲染或审计的 observe/core runtime 能力面；packages/skills、tool_skills、actions/verifiers、全部 26 个 presentation/view manifest、`runtime.artifact-list`、`runtime.run-resume`、`sciforge.payload-validation` 与 `sciforge.runtime-verification-gate` 已进入默认 registry，普通 internal helper 不作为 manifest TODO。
...
状态：完成；fresh/continuation、workspace read policy、current refs、repair retry、tool-use policy、context/rate-limit recovery 与 stream guard 决策均已迁入 contract/harness stage trace 或 deterministic metadata，prompt builder 不再作为策略真相源。
...
2026-05-10：T128 structured decision wiring 收尾完成。AgentServer context/rate-limit recovery 携带 `onPolicyDecision` harness signals；silent stream guard policy/audit 携带 `onStreamGuardTrip` signals；backend selection、continuity、recovery 与 stream guard 现在共用声明式 external hook trace metadata，不新增 prompt 散文。
...
- [x] T128 structured decision wiring：context/rate-limit recovery 与 stream guard 的剩余 live 决策面已接到声明式 harness stage hook/trace；backend selection、continuity decision、beforeAgentDispatch external hook metadata 与 silent stream decision 均有结构化 trace。
...
状态：完成；direct payload、generated task、observe/action、verification gate、repair rerun 和 failure/readback 路径已进入同一 `ValidationDecision -> RepairDecision -> AuditRecord` 链路，runner 保持 generate/run/validate/repair/audit 生命周期编排。
...
- 2026-05-10：T127-T observe real failure wiring 完成。真实 observe provider 返回 failed result 或抛错时会规范化为 failed `ObserveInvocationRecord`，继续走 budget debit、validation/repair/audit chain、observe invocation sink 与 telemetry sink；provider unavailable、provider failed response 和 provider error 三条路径均有 smoke 覆盖。
...
- [x] T127 observe real failure wiring：observe 真实 failure result 已接入同一 `ValidationDecision -> RepairDecision -> AuditRecord` read/write path；`generated-task-runner` 继续保持 generate/run/validate/repair/audit 生命周期编排。
