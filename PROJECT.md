...
当前处理目标（P4，2026-05-17）：收紧 plain direct-answer coding completion guard；coding/repair prompt 中宣称已修复、已实现、tests pass 或 PR ready 的纯文本，如果没有结构化 refs 或“文件路径 + 验证命令”这类可审计 evidence，必须落入 runtime diagnostic。
...
状态：todo
发现者：P3
轻量证据：Browser `http://127.0.0.1:5373/`；修复后 selected reproduction report follow-up run `project-literature-evidence-review-mp8unjqv-z81dk6` 已直接回答 selected report 的 credibility/metrics/risk/next step，但 Web UI 仍显示 `只得到部分结果` / `partial`，原因是 runtime verification gate 对 direct-context answer 给出 `unverified`。
升级证据：当前不保存额外 DOM；`workspace/parallel/p3/.sciforge/sessions/2026-05-16_workspace-biomedical-knowledge-graph-_kras-g12d_-_-mp8koxek_session-workspace-biomedical-knowledge-graph-_kras-g12d_-_-mp8koxek-mp8tkh0o-tdin1l/records/session.json` 中 run `project-literature-evidence-review-mp8unjqv-z81dk6` 保存 response 与 verification refs。
...
P1、P2、P3 与 P4 已完成本轮 strict-eval/fix/reverify 闭环；P3 关闭 selected reproduction report follow-up 被 generic risk regex 误路由为 planning register 的边界，但新增 `DISC-20260517-P3-001` 追踪 direct-context answer 内容正确时仍被 verification gate 标为 partial/unverified 的后续治理。P1 额外关闭 selected report follow-up 边界，metadata-only literature report 现在不会被旧“上一轮可见答案”摘要冒充全文阅读结论。P4 续修复已关闭 `DISC-20260517-P4-003`，下一条 P4 hardening 优先处理 `DISC-20260517-P4-002` direct-answer coding evidence guard。下一步由 P5/P6 继续各自 strict-eval 批次，或由 P2 开启下一条数据分析任务时优先覆盖 executable rerun command / selected chart follow-up / verification-required completion gate。每个进程继续按对应人类角色自由探索 SciForge；不要把任务剧本定死，也不要为了通过验收而写提示词。Worker 默认直接在 Web UI 点击查看、选择 artifact、reload、继续追问，并从 workspace 产物/records 中查证据；不要为每轮维护第二份 evidence。只有失败、假成功、修复对比、UI/workspace 不一致或 milestone 提交验收时，才升级保存截图/DOM/console/network/manifest。每个 milestone 结束后更新本文件、提交并 push GitHub、关闭上一批 sub agents，再启动下一批。
