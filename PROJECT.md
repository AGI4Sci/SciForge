...
进展：shadow-mode MVP 已完成。`packages/agent-harness` 提供 contract/profile/callback/trace 基础运行时，gateway 在 conversation policy 之后、既有 dispatch/fast-path 之前评估 `HarnessRuntime.evaluate()`，并把 `HarnessContract` / `HarnessTrace` 写入 request/uiState/metadata 与 stream trace。progress plan 已默认投影为结构化 progress event，verification policy 已默认由 contract 收紧；context envelope、broker 和 repair loop 仍保持 opt-in 消费，continuity decision 和 backend selection 已有结构化 audit 首片。
...
- [ ] 第二阶段打开 feature flag，让 context envelope、broker、prompt renderer、verification policy、repair loop、UI progress 逐项消费 contract。（progress plan 和 verification policy 默认消费已完成；context envelope、broker、repair loop 仍保持 opt-in；continuity/backend selection 与 prompt renderer metadata 已有结构化首片。）
...
- [ ] 每次 capability invocation 写入 `budgetDebits` 到 executionUnit/workEvidence/audit；已完成 `literature.retrieval` offline runner、`action.sciforge.computer-use` / `local.vision-sense` Computer Use loop、observe invocation success/provider-unavailable、runtime verification gate、payload schema validation failure、completed-payload work-evidence failure 和 current-reference usage failure。
