...
% === Abstract keywords ===
\abstract{
Scientific work increasingly spans heterogeneous artifacts---papers, code, datasets, scientific file formats, model outputs, figures, manuscripts, and team decisions---yet general-purpose AI assistants rarely preserve these objects as a coherent, auditable research state. We present SciForge, a multimodal research-native AI workbench that reserves the graphical interface for human judgment while search, parsing, model routing, workflow execution, plotting, writing, and presentation generation run as modular agent-accessible services. SciForge is built around four pillars: (i) \emph{goal-scoped scientific decision governance} for \textbf{goal-oriented} research, with review gates and shared review surfaces; (ii) \emph{translate-then-reason} for \textbf{multimodal} input, routing scientific objects through domain translators before the agent reasons; (iii) \emph{evidence governance} for \textbf{auditable} traceability, linking claims to provenance chains and audit findings; and (iv) \emph{collaborative team science} for \textbf{collaborative} research, enabling multi-role decision governance and shared team workspaces. The system combines a thin interaction layer, contextual research capability patterns, an Agent Runtime and Workflow Engine, an Evidence-DAG audit sidecar, a Scientific Model Router, and local scientific memory backed by reusable worker services. SciForge currently runs as a desktop application, with mobile supervision support; future releases will deepen team collaboration through shared review surfaces and decision governance. The system is open-source and available at \url{https://github.com/AGI4Sci/SciForge}.
}
...
We present \emph{SciForge}, a multimodal research-native AI workbench for scientific discovery. SciForge is designed as a local-first operating environment for research rather than a wrapper around a single model. Its architecture organizes the following interconnected functional systems: (1) \textbf{User Interaction \& Collaboration} surfaces for desktop execution, group coordination, and mobile supervision; (2) \textbf{Research Capability Patterns} for literature synthesis, data analysis, hypothesis design, reproduction, research writing, and visual review, all entering a shared evidence-aware control chain; (3) a \textbf{Core Engine} combining agent runtime, workflow automation, scoped research memory, and evidence-and-release governance; (4) a \textbf{Scientific Model Router} for modality detection and domain-expert translation; and (5) an \textbf{Infrastructure} layer providing local scientific memory, modular workers, scientific connectors, and a reproducible run substrate.

SciForge's core scientific contribution is organized around four pillars, each addressing a distinct gap in current research AI systems. \textbf{(i) Native scientific object processing with translate-then-reason:} end-to-end file ingress exposes four classes---protein sequences (.faa; .fasta/.fa with per-record conservative content classification), protein structures (.pdb/.cif/.mmcif), molecules (.smi/.smiles), and single-cell transcriptomics (via C2S worker contract)---routed through domain-expert translators (Esm2Text~\cite{abdine2023prot2text}, Prot2Text~\cite{abdine2023prot2text}, BioT5~\cite{pei2024biot5}, and C2S~\cite{rizvi2025c2s}) that return structured expert observations before the main agent reasons. C2S is a worker API contract, not an end-to-end file-ingress route; .mol/.sdf are not auto-translated. VCF, BED, GFF, and MGF are unsupported and fail closed; raw content is not routed to the general reasoner. The translator output is an evidence candidate, not a verified scientific fact; the human remains responsible for judgment.



\textbf{(ii) Artifact- and run-grounded evidence governance:} the Evidence DAG schema can capture ArtifactVersion, SourceAnchor, and run lineage when the runtime supplies structured evidenceLineage; software version, parameters, environment, log/output, and random seed are explicit provenance breakpoints that the schema represents but does not infer from prose. Immutable thread-level Evidence Snapshots are committed at user-designated breakpoints; provenance is not automatically complete per claim. A read-only audit sidecar inspects these snapshots without blocking agent exploration. \textbf{(iii) Goal-scoped scientific decision governance:} a Project DAG composes committed Evidence Snapshots with ReviewItem, DecisionEvent, an execution policy (autonomous, checkpointed, or supervised per goal), and candidate/certified release semantics. Audit findings are fail-open by default; blocking applies only to product actions explicitly integrated with the release API.

\textbf{(iv) Collaborative team science:} SciForge provides shared team workspaces, multi-role decision governance with reviewer/approver roles, and IM-based coordination channels with supported IM-based coordination channels (Zulip, Discord, WeChat, Feishu) for remote task submission and status supervision. The Project DAG supports collaborative review workflows where multiple researchers can inspect Evidence Snapshots, record ReviewItems with role-attributed decisions, and advance artifacts through candidate/certified release gates. Team workspaces synchronize research memory, evidence trails, and decision records across distributed collaborators, ensuring that multi-PI and cross-institutional research teams maintain a single source of truth for scientific governance.

The local-first workspace, multi-provider Model Router, GUI judgment surfaces, and IM-based team coordination serve as the supporting architecture that makes these four pillars operational.
...
\begin{itemize}
    \item \textbf{Goal-Scoped Scientific Decision Governance.} A Project DAG structure composes Evidence Snapshots with ReviewItem, DecisionEvent, and execution policy, providing review and release-decision surfaces with candidate/certified release semantics.
    \item \textbf{Native Scientific Object Processing with Translate-then-Reason.} End-to-end file ingress routes protein, molecular, and single-cell data through domain-expert translators before the agent reasons over structured, inspectable observations.
    \item \textbf{Artifact- and Run-Grounded Evidence Governance.} An Evidence DAG captures provenance and run lineage; a read-only audit sidecar inspects snapshots and produces Risk Digests without blocking agent exploration.
    \item \textbf{Collaborative Team Science.} Shared workspaces, multi-role decision governance, and IM-based coordination channels (Zulip, Discord, WeChat, Feishu) enable distributed teams to maintain a unified evidence trail and governance record.
\end{itemize}
...
\textbf{Goal-oriented research design.} Beyond per-task execution, SciForge anchors every thread and project to an explicit goal scope with defined review gates, decision records, and release semantics. This \textbf{goal-oriented} design distinguishes SciForge from systems that treat each conversation as an independent session: scientific objectives in SciForge are persistent, auditable, and governable across multiple turns, tools, and contributors. The combination of goal-centered memory, the Project DAG, and shared review surfaces gives research teams a concrete mechanism to track progress toward scientific milestones rather than accumulating disconnected interactions.
...
SciForge is organized as a layered research operating environment rather than a single-purpose chat interface. \textbf{Layer~1---User Interaction \& Collaboration}---provides desktop execution with a thin GUI for human judgment, reviews, and annotations. Supported team-collaboration surfaces provide \textbf{goal-oriented} shared review, multi-PI decision governance, and IM-channel integration (Zulip, Discord, WeChat, Feishu) for remote task supervision and coordination. \textbf{Layer~2---Research Capability Patterns}---separates six recurrent operations with distinct input--output boundaries: literature synthesis, data analysis, hypothesis design, reproduction, research writing, and visual review. All six share one evidence-aware control chain connecting task and object ingress, conditional fail-closed translation, delegated execution, evidence candidates with artifact/run lineage, thread-scoped Evidence Snapshots, and goal-scoped human review and release. \textbf{Layer~3---Core Engine}---combines the Agent Runtime, Automation Engine, Scoped Research Memory, and Evidence \& Release Governance for long-running execution. \textbf{Layer~4---Scientific Model Router and Domain Translators}---detects modality, routes supported scientific objects to domain experts, and returns provenance-attached evidence candidates to the reasoning agent. \textbf{Layer~5---Infrastructure}---provides local scientific memory, modular worker capabilities, scientific connectors, and a reproducible run substrate.
...
Figure~\ref{fig:sciforge_framework} summarizes SciForge as a layered workbench in which every research capability pattern enters the same evidence-aware control chain, while execution, scientific translation, evidence capture, and release decisions are supplied by the lower architectural layers.

\begin{figure*}[p]
    \centering
    \includegraphics[width=0.98\textwidth,height=0.86\textheight,keepaspectratio]{figures/SciForge_Framework_v3.pdf}
    \caption{SciForge system framework. Desktop, group, and mobile surfaces expose six research capability patterns with explicit input--output boundaries. Every pattern follows the same evidence-aware control chain---task/object ingress, conditional fail-closed translation, agent/worker execution, provenance-attached evidence candidates, a thread-scoped Evidence Snapshot with advisory audit, and goal-scoped review and release through the Project DAG and human PI. The lower layers provide the shared execution, scientific translation, and local-first infrastructure.}
    \label{fig:sciforge_framework}
\end{figure*}

\subsection{User Interaction and Collaboration (Layer~1)}
The top layer provides interaction surfaces unified by the same runtime semantics. \textbf{Desktop execution} keeps the agent close to local files, servers, and HPC environments, enabling direct manipulation of research data without network latency or cloud upload. Researchers can edit code, inspect results, and iterate on experiments with the full power of local tooling. \textbf{Supported team-collaboration surfaces} provide \textbf{goal-oriented} shared review, multi-PI decision governance through the Project DAG, and IM-channel integration (Zulip, Discord, WeChat, Feishu) for remote task supervision. These extensions are designed to enable cross-session handoff, team discussion, and lightweight approval of long-running work while preserving the local-first, auditable evidence model that distinguishes SciForge from cloud-only research assistants.

\subsection{Research Capability Patterns (Layer~2)}
Research capability patterns expose six recurrent scientific operations with deliberately distinct boundaries. \textbf{Literature Synthesis} maps papers and source records to an evidence summary. \textbf{Data Analysis} maps datasets and analysis parameters to quantitative results and figures. \textbf{Hypothesis Design} maps existing evidence and a research goal to testable hypotheses; acceptance or rejection is not part of this operation and remains a Project DAG decision. \textbf{Reproduction} maps a prior paper, code, data, and protocol to an independently rerun result. \textbf{Research Writing} maps evidence and research artifacts to manuscript text. \textbf{Visual Review} maps figures, slides, or visual artifacts to annotations and candidate revisions. These are contextual patterns within one workbench, not independent applications with separate execution semantics. Regardless of entry point, each task follows the same evidence-aware control chain: (1) task and object ingress; (2) fail-closed translation when a native scientific object requires a domain expert; (3) delegated agent and worker execution through Layers~3 and~4; (4) provenance-attached evidence candidates with artifact and run lineage; (5) a thread-scoped Evidence Snapshot and advisory audit; and (6) goal-scoped review and candidate/certified release through the Project DAG and human PI.

Two complementary DAG substrates implement this shared pattern. The \textbf{Evidence DAG} is thread-scoped: each agent thread grows its own claim--source--reasoning graph that records the evidentiary structure of that conversation's reasoning. The \textbf{Project DAG} is goal-scoped: it aggregates auditable evidence across threads into a project-level graph, linking claims, audit findings, and decision records to the objectives they support. Audit runs remain advisory and fail open during exploration, while lightweight decision records capture explicit human judgments such as artifact review, acceptance or rejection, and release actions. Background work---search, PDF parsing, data exploration, plotting, protocol drafting, reproduction pipelines, manuscript and presentation generation, and workflow execution---is delegated to the Agent Runtime and workers, keeping the primary workbench surface available for human judgment.

\subsection{Core Engine (Layer~3)}
The Core Engine plans and executes research work through four coordinated subsystems. The \textbf{Agent Runtime} uniformly supports multiple agent backends---Codex, Claude Code, and custom runtimes---allowing research groups to select the execution environment that best matches their scenario: Codex for local-first interactive coding with full workspace access, Claude Code for extended autonomous task chains, and custom runtimes for specialized scientific computing or institutional deployment requirements. Across all backends, the Runtime coordinates the main agent, delegated sub-agents, tool execution, workspace operations, MCP worker invocation, and long-running task recovery through a consistent API surface. The \textbf{Automation Engine} formalizes reusable pipelines with code, agent, tool, and approval nodes; scheduled execution and loop-style fixed protocol automation are treated as run modes rather than separate architectural modules. \textbf{Scoped Research Memory} stores free-text records (scope, tags, confidence, timestamps, context) across sessions; typed goals and decision events belong to the Project DAG. \textbf{Evidence \& Release Governance} captures provenance and claim--evidence structures, creates reviewable snapshots and audit findings, and exposes goal-scoped release decisions without synchronously blocking exploratory execution by default. Multimodal translation caches store scientific and visual observations by content hash, enabling repeated requests to reuse expert evidence without recomputing final conclusions.
...
\textbf{Scientific Modality Routing.} For scientific files, the Router detects modality-specific extensions and applies one of two handling paths. \emph{Translate-then-reason} applies to four currently supported modalities: protein (.faa; .fasta/.fa with per-record conservative content classification) translated by Esm2Text, protein structures (.pdb, .cif, .mmcif) by Prot2Text, small molecules (.smi, .smiles) by BioT5, and single-cell transcriptomics via the C2S worker contract. Each translator output carries provenance metadata, allowing the main agent to reason over structured expert observations. \emph{Code-to-Science (C2S)} is a planned worker API contract that will allow code execution outputs to be routed through the same translate-then-reason pipeline, enabling computational results to participate in scientific reasoning chains. VCF, BED, GFF, and MGF are unsupported and fail closed; raw content is not routed to the general reasoner.


\subsection{Collaboration and Team Coordination (Supported)}
SciForge's current collaboration model centers on the desktop application with local-first workspace sharing through version-controlled project directories. Supported extensions provide real-time shared review surfaces for evidence inspection, multi-PI decision governance through the Project DAG, and IM-channel integration (Zulip, Discord, WeChat, Feishu) for remote task submission and status coordination. The \textbf{goal-oriented} research design means that team members operate within shared goal scopes: each scientific objective carries explicit review gates, evidence snapshots, and decision records that are visible to all collaborators, enabling transparent, auditable team science rather than isolated individual sessions.
...
Automated AI modeling & AI4AI-driven cell perturbation prediction with iterative algorithm-atom recombination & \textbf{[Validated Prototype]} \\
...
Molecular design and optimization & MolClaw agent-driven EGFR inhibitor design with 6-round docking optimization & \textbf{[Validated Prototype]} \\
Lab automation & Genome-to-BGC discovery and prioritization with multi-Agent scientific analysis & \textbf{[Validated Prototype]} \\
\bottomrule
\end{tabular}
\end{figure}
...
We exercised this paradigm on cell perturbation prediction---the task of predicting how a cell's transcriptome changes in response to a genetic or chemical perturbation---using a NeMo/BioNeMo training stack supporting PBMC, Replogle, and Tahoe benchmark datasets. The agent distilled algorithm atoms from the \texttt{stack} single-cell foundation model repository, producing structured design cards covering Cell-by-Gene Tabular Attention, Rectangular Gene Masking, Distribution Heads, Sliced Wasserstein Regularization, and In-Context Generation. These atoms were mapped to the project's NeMo model interface (MInterface, DInterface) and composed into new perturbation-prediction architectures under a frozen RJOB submission contract that forbids ad-hoc code changes during job submission.

Delta PCC---the improvement in Pearson correlation over a baseline JIT model---was tracked across design iterations. In the recorded pilot, Delta PCC improved from 0.30 (Iteration~1) to 0.48, 0.58, and finally 0.64 (Iteration~4), demonstrating that iterative atom-level design recombination can yield measurable performance gains (Fig.~\ref{fig:ai4ai-delta-pcc}). The complete agent logs, design cards, model code, configurations, and evaluation records are archived at \url{https://github.com/BruthYU/CELL_AI4AI}.

\textit{Limitations:} The Delta PCC curve represents a single pilot trajectory with four design iterations; statistical significance, cross-seed stability, and generalization to new perturbation tasks have not been established. The algorithm-atom distillation quality depends on the agent's reading fidelity and the target repository's code organization. The current pilot uses post-hoc metric reporting without pre-registered stopping criteria or blinded evaluation. Formal validation would require multi-seed replication, pre-registered success thresholds, and independent re-execution on held-out perturbation types.

\begin{figure}[ht!]
\centering
\includegraphics[width=0.95\textwidth]{figures/ai4ai-delta-pcc.png}
\caption{Delta PCC improvement across AI4AI design iterations for cell perturbation prediction. The curve tracks iterative algorithm-atom recombination on PBMC, Replogle, and Tahoe datasets under a NeMo/BioNeMo training stack.}
\label{fig:ai4ai-delta-pcc}
\end{figure}
...
\textit{[Validated Prototype]} Computational molecular design for drug discovery traditionally requires separate execution of docking, scoring, property filtering, and synthetic accessibility assessment---each demanding distinct expertise and manual data transfer between tools. SciForge's MolClaw workflow integrates these stages into a unified agent-driven optimization loop. A researcher provides a target protein structure (PDB~4HJO for EGFR T790M), a reference drug (Erlotinib), a 4-anilinoquinazoline scaffold with modifiable sites (R\textsubscript{1}--R\textsubscript{4}), and drug-like constraints; MolClaw then autonomously conducts multi-round \emph{design--dock--evaluate--refine} cycles.

We validated this workflow through a two-phase demonstration: (1)~a 26-candidate \emph{de novo} molecular design phase exploring substituent variations across four modification sites with SMILES validation, QED/Lipinski/PAINS filtering, and synthetic accessibility scoring; and (2)~a six-round iterative optimization phase (36~molecules across 6~rounds, plus baseline) using QuickVina2-GPU docking against the ATP-binding pocket. Each round followed a distinct strategy---exploration (gatekeeper and para-aniline scans), exploitation (best-substituent combinations), and pivot (new chemical classes)---with the agent interpreting docking scores, structural rationales, and property trade-offs before designing the next round.

The optimization trajectory demonstrated progressive score improvement: from the Erlotinib baseline ($-7.5$~kcal/mol) to the final best candidate EGFR\_R6\_02 ($-9.8$~kcal/mol, $\Delta = -2.3$~kcal/mol). Two molecules met the $\Delta\text{Score} \leq -2.0$ criterion. Key structure--activity relationships (SAR) emerged across rounds: R\textsubscript{2}=OH at C7 provided the best single improvement ($-8.3$~kcal/mol); R\textsubscript{1}=OMe~+~R\textsubscript{2}=OH established the optimal C6/C7 platform ($-8.8$~kcal/mol); R\textsubscript{4}=Cl at the para-aniline position contributed an additional $-0.3$~kcal/mol; a pivot round discovered R\textsubscript{4}=CF\textsubscript{3} surpassing Cl ($-9.2$~kcal/mol); and the final exploitation round identified synergistic R\textsubscript{2}=F~+~R\textsubscript{4}=CF\textsubscript{3} ($-9.8$~kcal/mol). All 37 dockings succeeded with no tool failures, and property tracking (MW, LogP, QED, TPSA, HBD/HBA) across rounds showed maintained drug-likeness.
...
\begin{enumerate}
    \item \textbf{Modality Routing Coverage.} Does the Scientific Model Router correctly detect modality, select the configured translator, and attach provenance-annotated observations? \textit{Current evidence:} four translator paths (Esm2Text, Prot2Text, BioT5, C2S) are implemented with unit-level integration in the codebase; C2S is a worker API contract, not a file ingress route; VCF/BED/GFF/MGF are unsupported and fail closed. \textit{Future:} systematic coverage benchmarks across standard bioinformatics file corpora, false-positive/negative routing rates, and comparison against manual modality annotation.
...
& Translate-then-reason with four modality experts (protein, structure, molecule, single-cell); C2S is a worker contract; unsupported formats fail closed \\
Agent Runtime
