...
let activeStart: Promise<LocalDrawioServerResult> | null = null
...
export async function startLocalDrawioServer(
  ...
  activeStart = startLocalDrawioServerOnce(options).finally(() => {
    activeStart = null
  })
  return activeStart
}

async function startLocalDrawioServerOnce(
  options: StartLocalDrawioServerOptions
): Promise<LocalDrawioServerResult> {
