...
const DEFAULT_REVIEW_DATA_PATH = 'outputs/sciforge-generated-review/sciforge_pdf_boxed_review_data.json'
const DEFAULT_GIF_PATH = 'case/visual-review/assets/sciforge_pdf_boxed_review.gif'
const REVIEW_AUTHOR_ID = 'sciforge-reviewer'
const REVIEW_ID_PREFIX = 'sciforge-review'
const DEFAULT_AUTO_REVIEW_COMMENT_COUNT = 12
...
async function callReviewModel(prompt: string, settings?: AppSettingsV1): Promise<string> {
  ...
  if (!response.ok) {
    throw new Error(`SciForge review model request failed (${response.status}): ${responseText.slice(0, 500)}`)
  }
...
async function requestReviewComments(
  ...
  const modelText = await callReviewModel(prompt, settings)
  try {
    return parseReviewCommentsJson(modelText)
  } catch {
    const repairedText = await callReviewModel(buildReviewJsonRepairPrompt(modelText, maxComments), settings)
...
function buildAutoReviewPrompt(extracted: ExtractedPdf, maxComments: number, selection?: PdfReviewSelection): string {
  const reviewScope = selection
    ...
    : 'Review scope: Review the full current PDF.'
  const countRule = selection
    ...
    : `Generate exactly ${maxComments} distinct comments. Do not stop after one comment. Cover different passages across motivation, claims, methods, evaluation, evidence, limitations, and writing clarity when possible.`
...
async function generateAutoReviewComments(
  ...
  const maxComments = clamp(Math.floor(payload.maxComments ?? DEFAULT_AUTO_REVIEW_COMMENT_COUNT), 1, 200)
  let comments = dedupeReviewComments(
    await requestReviewComments(buildAutoReviewPrompt(extracted, maxComments, payload.selection), maxComments, settings)
  )
...
export async function generatePdfReviewAnnotations(
  ...
  try {
    ...
    const comments = reviewDataPath
      ...
      : await generateAutoReviewComments(payload, settings)
    ...
    const gifCandidate = payload.gifPath ?? DEFAULT_GIF_PATH
    const gifPath = workspaceRoot ? join(workspaceRoot, gifCandidate) : gifCandidate
    return {
      ok: true,
      ...
      ...(await pathExists(gifPath) ? { gifPath } : {}),
...
export async function improvePdfReviewAnnotation(
  ...
  try {
    const thread = payload.sidecar.threads.find((item) => item.id === payload.threadId)
    ...
    const annotation = annotationId
      ? payload.sidecar.annotations.find((item) => item.id === annotationId)
      : undefined
    if (!annotation) throw new Error('PDF annotation was not found.')
    const anchor = payload.sidecar.anchors.find((item) => item.id === annotation.anchorId) ||
      payload.sidecar.anchors.find((item) => thread.anchorIds.includes(item.id))
    ...
    try {
      const modelText = await callReviewModel(buildImprovePrompt({ selectedText, userComment, page }), settings)
    ...
    const nextSidecar = stablePdfAnnotationSidecar({
      ...payload.sidecar,
      annotations: payload.sidecar.annotations.map((item) =>
      ...
      threads: payload.sidecar.threads.map((item) =>
        item.id === thread.id
          ? {
              ...item,
              status: 'open',
