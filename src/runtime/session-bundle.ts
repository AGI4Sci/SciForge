...
export async function ensureSessionBundle(workspace: string, bundleRel: string, metadata: SessionBundleMetadata) {
  ...
  await Promise.all([
    ...
    mkdir(join(root, 'ledger'), { recursive: true }),
    mkdir(join(root, 'projection'), { recursive: true }),
...
async function writeSessionBundleManifest(workspace: string, bundleRel: string, metadata: SessionBundleMetadata) {
  ...
  const manifest = {
    ...
    restore: {
      ...
      projectSessionLedgerRef: `${bundleRel}/ledger/events.jsonl`,
      currentProjectionRef: `${bundleRel}/projection/current.jsonl`,
      persistentProjectionRef: `${bundleRel}/projection/persistent.jsonl`,
      memoryCandidatesRef: `${bundleRel}/projection/memory-candidates.jsonl`,
...
function sessionBundleChecklistTemplate(bundleRel: string): SessionBundleChecklistItem[] {
  ...
  return [
    ...
    {
      id: 'pack.project-session-memory',
      phase: 'pack',
      label: 'Append-only project session ledger and disposable projection roots are present.',
      required: true,
      refs: [
        `${root}/ledger/`,
        `${root}/projection/`,
