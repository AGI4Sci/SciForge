...
import type { ValidationRepairAuditSinkRecord } from '../../packages/contracts/runtime/validation-repair-audit.js';
import {
  capabilityEvolutionRecordFromValidationRepairAuditSinkRecord,
  uniqueValidationRepairAuditLedgerSinkRecords,
  validationRepairAuditLedgerFactFromRecord,
} from './capability-evolution-ledger-validation-audit.js';
import type {
  ValidationRepairAuditLedgerFact,
  ValidationRepairAuditLedgerRecordOptions,
} from './capability-evolution-ledger-validation-audit.js';
...
export interface ValidationRepairAuditLedgerWriteOptions
  extends CapabilityEvolutionLedgerOptions, ValidationRepairAuditLedgerRecordOptions {}

export interface ValidationRepairAuditLedgerWriteResult extends CapabilityEvolutionLedgerAppendResult {
  compactRecord: CapabilityEvolutionCompactRecord;
  fact: ValidationRepairAuditLedgerFact;
}
...
export async function appendValidationRepairAuditSinkRecordToCapabilityEvolutionLedger(
  ...
  const record = capabilityEvolutionRecordFromValidationRepairAuditSinkRecord(sinkRecord, options);
  const appendResult = await appendCapabilityEvolutionRecord(options, record);
  const compactRecord = compactCapabilityEvolutionRecord(record, appendResult.ref);
  return {
    ...appendResult,
    compactRecord,
    fact: validationRepairAuditLedgerFactFromRecord(sinkRecord, appendResult.ref),
  };
}

export async function appendValidationRepairAuditSinkRecordsToCapabilityEvolutionLedger(
  options: ValidationRepairAuditLedgerWriteOptions,
  sinkRecords: ValidationRepairAuditSinkRecord[],
): Promise<ValidationRepairAuditLedgerWriteResult[]> {
  const uniqueLedgerRecords = uniqueValidationRepairAuditLedgerSinkRecords(sinkRecords);
  const results: ValidationRepairAuditLedgerWriteResult[] = [];
  for (const sinkRecord of uniqueLedgerRecords) {
    results.push(await appendValidationRepairAuditSinkRecordToCapabilityEvolutionLedger(options, sinkRecord));
  }
  return results;
}
