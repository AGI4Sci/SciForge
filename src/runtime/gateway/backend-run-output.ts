import type { BackendGenerationResponse, ToolPayload, WorkspaceTaskRunResult } from '../runtime-types.js';
import { cleanUrl, errorMessage, isRecord, uniqueStrings } from '../gateway-utils.js';
import { sha1 } from '../workspace-task-runner.js';
import { parseJsonErrorMessage, sanitizeBackendError } from './backend-failure-diagnostics.js';
import { coerceBackendToolPayload, extractJson, extractStandaloneJson } from './direct-answer-payload.js';
...
export function parseGenerationResponse(value: unknown): BackendGenerationResponse | undefined {
  const candidates = [
    value,
...
export function looksLikeTruncatedBackendResponseText(value: unknown) {
  const text = typeof value === 'string' ? value : '';
...
export function parseToolPayloadResponse(run: Record<string, unknown>): ToolPayload | undefined {
  ...
  for (const candidate of candidates) {
    ...
    const payload = coerceBackendToolPayload(parsed);
...
export function backendRunFailure(run: Record<string, unknown>) {
...
export function extractBackendFailureDetail(run: Record<string, unknown>) {
  const output = isRecord(run.output) ? run.output : {};
  const stages = Array.isArray(run.stages) ? run.stages.filter(isRecord) : [];
  const candidates = [
    output.error,
...
export function backendSessionRef(baseUrl: string, agentId: string) {
  return `${cleanUrl(baseUrl)}/api/agent-server/agents/${encodeURIComponent(agentId)}`;
}

export function backendRequestFailureMessage(operation: 'generation' | 'repair', error: unknown, timeoutMs: number) {
...
export function extractBackendOutputText(run: Record<string, unknown>) {
  const output = isRecord(run.output) ? run.output : {};
  const stages = Array.isArray(run.stages) ? run.stages.filter(isRecord) : [];
  const candidates = [
    output.result,
