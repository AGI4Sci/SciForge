...
import { looksLikeTruncatedBackendResponseText, looksLikeUnparsedGenerationResponseText, parseGenerationResponse } from './backend-run-output';
...
test('detects AgentServer transport-compacted output strings', () => {
  assert.equal(looksLikeTruncatedBackendResponseText('{"message":"ok"}'), false);
  assert.equal(
    looksLikeTruncatedBackendResponseText('{"message":"ok"}\n...[truncated 5799 chars for AgentServer HTTP response; full value remains in run store]'),
    true,
  );
  assert.equal(looksLikeTruncatedBackendResponseText('summary [truncated]'), true);
});
