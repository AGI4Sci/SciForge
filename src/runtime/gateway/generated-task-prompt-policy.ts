...
export function buildBackendGenerationPrompt(request: {
  prompt: string;
  ...
  const currentUserRequest = stringField(sessionFacts.currentUserRequest) ?? extractBackendCurrentUserRequest(request.prompt);
  ...
  const capabilityBrokerBrief = isRecord(scenarioFacts.capabilityBrokerBrief)
    ...
    : isRecord(request.availableRuntimeCapabilities) && (
      request.availableRuntimeCapabilities.schemaVersion === 'sciforge.backend.capability-broker-brief.v1'
      || request.availableRuntimeCapabilities.schemaVersion === 'sciforge.agentserver.capability-broker-brief.v1'
    )
  ...
  const promptRenderPlanSummary = promptRenderPlanSummaryForBackend(request, contextEnvelope, sessionFacts);
  const contextProjection = isRecord(sessionFacts.contextProjection)
    ...
    : undefined;
  const currentTurnSnapshot = backendCurrentTurnSnapshotFromHandoff({
    request,
    currentUserRequest,
  ...
  return [
    ...
    JSON.stringify(clipForBackendJson(currentTurnSnapshot), null, 2),
    '',
    ...
    '',
    JSON.stringify(clipForBackendJson({
      ...compactGenerationRequestForBackend(request, capabilityBrokerRouteSummary, promptRenderPlanSummary),
...
function backendCurrentTurnSnapshotFromHandoff(params: {
  request: Parameters<typeof buildBackendGenerationPrompt>[0];
  ...
  return {
    ...
    repairContinuation: params.request.repairContinuation ? {
      ...
      terminalPayloadContract: [
        ...
        'Allowed success shape: GeneratedTaskResponse containing a minimal provider-route adapter task for the existing failed unit.',
    ...
    outputContract: backendGenerationOutputContract(),
  };
}
...
function agentServerRepairContinuationHardStopPromptLines() {
  return [
    ...
    '- Success shape: a runnable GeneratedTaskResponse whose taskFiles implement a minimal provider-route adapter task for the existing failed execution unit; the adapter must use the supplied capability/provider route refs and must not rebuild the whole pipeline.',
...
function compactGenerationRequestForBackend(
  request: Parameters<typeof buildBackendGenerationPrompt>[0],
...
function backendBibliographicPolicyLinesForRequest(
  request: Parameters<typeof buildBackendGenerationPrompt>[0],
  scenarioFacts: Record<string, unknown>,
) {
  const include = backendShouldIncludeBibliographicVerificationPromptPolicy({
  ...
  return include ? backendBibliographicVerificationPromptPolicyLines() : [];
}
...
function compactCapabilityProviderRouteSummary(value: unknown) {
  ...
  const routes = toRecordList(value.routes).map((route) => ({
    ...
    reason: clipForBackendPrompt(route.reason, 220),
...
function compactCapabilityBriefForPrompt(brief: Record<string, unknown>) {
  ...
  return {
    ...
    brief: clipForBackendPrompt(brief.brief, 260),
    ...
    budget: {
      ...
      limits: clipForBackendPrompt(budget.limits, 120),
    },
    excluded: clipForBackendPrompt(brief.excluded, 180),
  };
}
  ...
  return {
    ...
    workspaceKernel: {
      ...
      eventIndex: toRecordList(workspaceKernel.eventIndex).slice(-16).map((entry) => ({
        ...
        summary: clipForBackendPrompt(entry.summary, 220),
...
function compactContextRefForPrompt(value: unknown) {
  ...
  return {
    ...
    preview: clipForBackendPrompt(value.preview, 160),
...
function compactContextEnvelopeForAgentServer(value: unknown) {
  ...
  for (const [key, entry] of Object.entries(value)) {
    ...
    if (key === 'agentServerCoreSnapshot') {
      const snapshot = compactBackendCoreSnapshotForPrompt(entry);
...
function compactBackendCoreSnapshotForPrompt(value: unknown) {
  if (!isRecord(value)) return undefined;
...
export function sanitizePromptHandoffValue(value: unknown, path = ''): unknown {
  ...
  if (typeof value === 'string') return clipForBackendPrompt(value, 1800);
...
function promptRenderPlanSummaryForBackend(
  request: Parameters<typeof buildBackendGenerationPrompt>[0],
  contextEnvelope: Record<string, unknown>,
