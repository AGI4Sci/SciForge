...
export interface BackendContextRequest {
...
export type AgentServerContextRequest = BackendContextRequest;

export interface BuildBackendContextRequestInput {
  sessionId: string;
  turnId: string;
  mode: ContextMode;
  currentTurnRef: ProjectMemoryRef;
...
/** @deprecated Use BuildBackendContextRequestInput. */
export type BuildAgentServerContextRequestInput = BuildBackendContextRequestInput;
...
export function buildBackendContextRequest(input: BuildBackendContextRequestInput): BackendContextRequest {
  ...
  const request: BackendContextRequest = {
  ...
  assertBackendContextRequest(request);
  return request;
}

export function canonicalSerializeBackendContextRequest(request: BackendContextRequest): string {
  assertBackendContextRequest(request);
...
export function validateBackendContextRequest(value: unknown): ContractValidationResult {
  const errors: string[] = [];
  const request = asRecord(value);
  if (!request) return invalid('request must be an object');
  collectForbiddenFieldErrors(request, REQUEST_FORBIDDEN_FIELDS, 'BackendContextRequest', errors);
...
export function assertBackendContextRequest(value: unknown): asserts value is BackendContextRequest {
  const result = validateBackendContextRequest(value);
  if (!result.ok) throw new ContractValidationError('Invalid BackendContextRequest', result.errors);
}

/** @deprecated Use buildBackendContextRequest. */
export const buildAgentServerContextRequest = buildBackendContextRequest;
/** @deprecated Use canonicalSerializeBackendContextRequest. */
export const canonicalSerializeAgentServerContextRequest = canonicalSerializeBackendContextRequest;
/** @deprecated Use validateBackendContextRequest. */
export const validateAgentServerContextRequest = validateBackendContextRequest;
/** @deprecated Use assertBackendContextRequest. */
export const assertAgentServerContextRequest = assertBackendContextRequest;
...
function normalizeRefSelectionPolicy(
  input: BuildBackendContextRequestInput['refSelectionPolicy'] | undefined,
  retrievalAvailable: boolean,
): RefSelectionPolicy {
  return {
...
function selectRefsForContextRequest(
  input: BuildBackendContextRequestInput,
...
function refCandidatesForMode(input: BuildBackendContextRequestInput): SelectedRefDescriptor[] {
