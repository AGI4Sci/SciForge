...
import { backendSessionRef } from './backend-run-output.js';
import {
...
export interface BackendGenerationRetryAudit {
  schemaVersion: typeof BACKEND_GENERATION_RETRY_SCHEMA_VERSION;
  attempt: 2;
  maxAttempts: 2;
  trigger: BackendFailureDiagnostic;
...
export type BackendGenerationFailureDiagnostics = {
  kind: 'contextWindowExceeded' | 'rateLimit' | 'agentserver';
  categories?: BackendFailureKind[];
  retryAfterMs?: number;
  resetAt?: string;
  retryAudit?: BackendGenerationRetryAudit;
  backend?: string;
  provider?: string;
  model?: string;
  agentId?: string;
  ...
  priorHandoff?: BackendGenerationRetryAudit['priorHandoff'];
...
export type BackendGenerationResult =
  ...
  | { ok: false; error: string; diagnostics?: BackendGenerationFailureDiagnostics };

export async function recoverOrReturnBackendGenerationFailure(params: {
  error: string;
  sanitizedError: string;
  dispatchAttempt: number;
  contextRecovery?: BackendGenerationFailureDiagnostics;
  ...
  | { retry: true; diagnostics: BackendGenerationFailureDiagnostics }
  | { retry: false; result: BackendGenerationResult }
> {
  const originalErrorSummary = sanitizeBackendError(params.error || params.sanitizedError);
  const contextSessionRef = backendSessionRef(params.baseUrl, params.agentId);
  ...
  if (isContextWindowExceededError(`${params.error}\n${params.sanitizedError}`)) {
    ...
    emitWorkspaceRuntimeEvent(params.callbacks, backendContextWindowRecoveryStartEvent({
    ...
    emitWorkspaceRuntimeEvent(params.callbacks, {
      ...
      message: compaction.ok
        ? 'Context compaction completed; retrying backend generation once.'
        : compactionStatus === 'skipped'
          ? 'Context compact API unsupported; retrying backend generation once with slim handoff diagnostics.'
          : 'Context compaction failed; retrying backend generation once with slim handoff diagnostics.',
    ...
    const diagnostics: BackendGenerationFailureDiagnostics = {
      kind: 'contextWindowExceeded',
      backend: params.adapter.backend,
      provider: diagnosticProvider,
      model: params.model,
  ...
  const diagnostic = classifyBackendFailure(params.error, {
  ...
  emitWorkspaceRuntimeEvent(params.callbacks, backendGenerationRecoveryStartEvent({
    categories: diagnostic.categories,
  ...
  if (compaction) {
    emitWorkspaceRuntimeEvent(params.callbacks, {
      ...
      message: 'Backend compact before provider/rate-limit retry',
      detail: compaction.message || compaction.reason,
      contextCompaction: contextCompactionMetadata(compaction),
      contextWindowState: compaction.after ? workspaceContextWindowStateFromBackend(compaction.after) : undefined,
      raw: compaction,
  ...
  const retryAudit: BackendGenerationRetryAudit = {
    schemaVersion: BACKEND_GENERATION_RETRY_SCHEMA_VERSION,
    attempt: 2,
    maxAttempts: 2,
    trigger: diagnostic,
...
export async function finalizeBackendGenerationSuccess<T extends Extract<BackendGenerationResult, { ok: true }>>(params: {
  result: T;
  contextRecovery?: BackendGenerationFailureDiagnostics;
  workspace: string;
  request: GatewayRequest;
  ...
  if (String(params.contextRecovery.kind) === 'contextWindowExceeded') {
    emitWorkspaceRuntimeEvent(params.callbacks, backendContextWindowRecoverySucceededEvent({
  ...
  emitWorkspaceRuntimeEvent(params.callbacks, backendGenerationRetrySucceededEvent({
...
async function appendContextRecoveryAuditAttempt(params: {
  ...
  request: GatewayRequest;
  skill: SkillAvailability;
  diagnostics: BackendGenerationFailureDiagnostics;
  status: 'repair-needed' | 'self-healed';
  failureReason: string;
}) {
  const id = `backend-context-recovery-${params.request.skillDomain}-${sha1(`${params.request.prompt}:${params.status}:${Date.now()}`).slice(0, 12)}`;
