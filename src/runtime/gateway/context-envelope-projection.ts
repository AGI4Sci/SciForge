...
export function contextProjectionForEnvelope(uiState: Record<string, unknown>) {
  ...
  const selectedMessageRefs = toRecordList(source.selectedMessageRefs ?? source.recentConversation)
    ...
    .map((entry) => ({
      ...
      summary: clipForBackendPrompt(entry.summary, 240),
  ...
  const backendContextRequest = isRecord(source.backendContextRequest)
    ? source.backendContextRequest
    : isRecord(source.agentServerContextRequest)
  ...
  return {
    ...
    authority: 'workspaceKernel refs are canonical truth; backend consumes contextRefs, capabilityBriefRef, and cachePlan',
    ...
    contextRefs: contextRefListForEnvelope(source.contextRefs ?? backendContextRequest?.contextRefs).slice(0, 64),
    capabilityBriefRef: memoryRefForEnvelope(source.capabilityBriefRef ?? backendContextRequest?.capabilityBriefRef),
    cachePlan: cachePlanForEnvelope(source.cachePlan ?? backendContextRequest?.cachePlan),
    ...
    pollutionGuard: isRecord(source.pollutionGuard) ? clipForBackendJson(source.pollutionGuard, 2) : undefined,
  };
}
  ...
  const kernel = isRecord(source.workspaceKernel)
    ...
    : isRecord(source.workspaceLedger)
      ...
      : isRecord(source.projectSessionMemory)
        ...
        : undefined;
  return kernel ? clipForBackendJson(kernel, 4) : undefined;
}
