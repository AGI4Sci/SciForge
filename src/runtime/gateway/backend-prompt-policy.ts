...
import { parseJsonErrorMessage, redactSecretText, sanitizeBackendError } from './backend-failure-diagnostics.js';
import {
  selectedAgentBackend,
  configuredBackendBaseUrl,
  requiresUserLlmEndpointForBackendBaseUrl,
} from './agent-backend-config.js';
import { ignoredLegacyRepairContextPolicyAuditForBackend, repairContextPolicySummaryForBackend } from './generated-task-repair-context-policy.js';
...
function backendRunFailure(run: Record<string, unknown>) {
  ...
  const detail = extractBackendFailureDetail(run);
  return `Agent backend failed: ${detail || 'run failed without a usable generation result.'}`;
}

function extractBackendFailureDetail(run: Record<string, unknown>) {
  const output = isRecord(run.output) ? run.output : {};
  ...
  for (const candidate of candidates) {
    ...
    return sanitizeBackendError(parsedMessage || text);
  }
  return undefined;
}

function backendRequestFailureMessage(operation: 'generation' | 'repair', error: unknown, timeoutMs: number) {
  const message = errorMessage(error);
  if (error instanceof Error && error.name === 'AbortError' || /abort|cancel|timeout/i.test(message)) {
    return `Backend ${operation} request timed out or was cancelled after ${timeoutMs}ms. Retry can resume with this repair-needed attempt in priorAttempts.`;
  }
  return `Backend ${operation} request failed: ${sanitizeBackendError(message)}`;
}

export async function requestBackendRepair(params: {
  baseUrl: string;
  ...
  try {
    ...
    const agentId = backendAgentId(params.request, 'runtime-repair');
    ...
    const agentServerSnapshot = await fetchBackendContextSnapshot(params.baseUrl, agentId);
    if (agentServerSnapshot) {
      repairContext.agentServerCoreSnapshot = agentServerSnapshot;
    }
    const repairPrompt = buildGeneratedTaskRepairPrompt({ ...params, repairContext });
    ...
    runPayload = {
      ...
      contextPolicy: backendContextPolicy(params.request),
      runtime: {
        backend,
    ...
    await writeBackendDebugArtifact(params.run.workspace, 'repair', runPayload, response.status, json, sessionBundleRelForRequest(params.request));
    if (!response.ok) {
      const detail = isRecord(json) ? String(json.error || json.message || '') : '';
      return { ok: false, error: sanitizeBackendError(detail || `Backend repair HTTP ${response.status}: ${String(text).slice(0, 500)}`) };
    }
    ...
    await writeBackendDebugArtifact(params.run.workspace, 'repair', runPayload, 0, { error: errorMessage(error) }, sessionBundleRelForRequest(params.request));
    return { ok: false, error: backendRequestFailureMessage('repair', error, timeoutMs) };
  } finally {
    clearTimeout(timeout);
  }
}

export async function readConfiguredBackendBaseUrl(workspace: string) {
  ...
  return configuredBackendBaseUrl({ workspaceConfigBaseUrl });
}

export async function writeBackendDebugArtifact(
  workspace: string,
  ...
  try {
    ...
    const rel = sessionBundleRel
      ? join(sessionBundleRel.replace(/\/+$/, ''), 'debug', 'backend', `${id}.json`)
      : join('.sciforge', 'debug', 'backend', `${id}.json`);
    await mkdir(dirname(join(workspace, rel)), { recursive: true });
    await writeFile(join(workspace, rel), JSON.stringify({
      createdAt: new Date().toISOString(),
      kind: 'backend-run-debug',
      task,
      responseStatus,
...
export function requiresUserLlmEndpoint(agentServerBaseUrl: string, request?: GatewayRequest) {
  return requiresUserLlmEndpointForBackendBaseUrl(agentServerBaseUrl, request);
}
