...
import { clipForBackendJson, clipForBackendPrompt, errorMessage, isRecord, toRecordList } from '../gateway-utils.js';
...
export async function preflightBackendContextWindow(params: {
  ...
  if (state) {
    emitWorkspaceRuntimeEvent(params.callbacks, {
      type: BACKEND_CONTEXT_WINDOW_STATE_EVENT_TYPE,
      source: WORKSPACE_RUNTIME_SOURCE,
      status: state.status,
      message: `Backend context window ${state.status}`,
...
export async function readBackendContextWindowState(
  ...
  try {
    ...
    const snapshot = compactBackendCoreSnapshot(data);
...
export function backendAgentId(request: GatewayRequest, _purpose: string) {
  ...
  const continuityScope = requestNeedsBackendContinuity(request)
...
export function backendContextPolicy(request: GatewayRequest) {
  const hasSession = typeof request.uiState?.sessionId === 'string' && request.uiState.sessionId.trim().length > 0;
  const policyMode = backendContextPolicyMode(request);
  ...
  const useContinuity = requestNeedsBackendContinuity(request);
  return {
...
function backendContextPolicyMode(request: GatewayRequest) {
  const uiState = isRecord(request.uiState) ? request.uiState : {};
  const policy = isRecord(uiState.contextReusePolicy) ? uiState.contextReusePolicy : {};
  return stringField(policy.mode) ?? '';
}

export function requestNeedsBackendContinuity(request: GatewayRequest) {
  return agentHarnessContinuityDecision(request).useContinuity;
}

/** @deprecated Use preflightBackendContextWindow. */
export const preflightAgentServerContextWindow = preflightBackendContextWindow;
/** @deprecated Use backendAgentId. */
export const agentServerAgentId = backendAgentId;
/** @deprecated Use backendContextPolicy. */
export const agentServerContextPolicy = backendContextPolicy;
/** @deprecated Use requestNeedsBackendContinuity. */
export const requestNeedsAgentServerContinuity = requestNeedsBackendContinuity;
...
export async function fetchBackendContextSnapshot(baseUrl: string, agentId: string) {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 5000);
  try {
    ...
    return compactBackendCoreSnapshot(data);
  } catch {
    return undefined;
  } finally {
    clearTimeout(timeout);
...
export function compactBackendCoreSnapshot(snapshot: Record<string, unknown>) {
  ...
  return {
    ...
    session: isRecord(snapshot.session) ? {
      ...
      recovery: isRecord(snapshot.session.recovery) ? clipForBackendJson(snapshot.session.recovery, 2) : undefined,
    } : undefined,
    operationalGuidance: clipForBackendJson(snapshot.operationalGuidance, 3),
    workLayout: clipForBackendJson(snapshot.workLayout, 3),
    workBudget: clipForBackendJson(snapshot.workBudget, 2),
    persistentBudget: clipForBackendJson(snapshot.persistentBudget, 2),
    memoryBudget: clipForBackendJson(snapshot.memoryBudget, 2),
    recentTurnRefs: boundedTurnRefs.slice(-6).map(compactBackendCoreRecentTurnRef),
    currentWork: {
      ...
      compactionTags: currentWorkEntries
        ...
        .map((entry) => ({
          ...
          summary: Array.isArray(entry.summary) ? entry.summary.slice(0, 4).map((item) => clipForBackendPrompt(item, 400)) : undefined,
        })),
    },
  };
}

/** @deprecated Use fetchBackendContextSnapshot. */
export const fetchAgentServerContextSnapshot = fetchBackendContextSnapshot;
/** @deprecated Use compactBackendCoreSnapshot. */
export const compactAgentServerCoreSnapshot = compactBackendCoreSnapshot;

function compactBackendCoreRecentTurnRef(turn: Record<string, unknown>) {
