...
test('backend generation prompt projects core snapshot as turn refs without recentTurns or raw bodies', () => {
...
test('AgentServer prompt gateway config resolves AgentServer base URL through centralized env and workspace policy', async () => {
  ...
  try {
    ...
    assert.equal(await readConfiguredBackendBaseUrl(workspace), 'http://workspace-agent.example.test');
...
test('backend generation prompt renderer consumes BackendHandoffPacket and bounded render plan only', () => {
...
test('backend generation prompt requires provider-first code when ready web routes exist', () => {
  const prompt = buildBackendGenerationPrompt({
...
test('backend generation prompt carries tiny capability discovery brief and keeps execution gated', () => {
  const prompt = buildBackendGenerationPrompt({
...
test('backend generation prompt applies provider-first helper contract to any ready capability route', () => {
  const prompt = buildBackendGenerationPrompt({
