...
function providerFirstRecoveryAdapterSource(initialReason: string, retryReason: string) {
  return [
    ...
    '        model = _first(record, ["model", "system", "organism", "disease", "population"]) or "pancreatic cancer / spatial transcriptomics context inferred from query"',
    '        rows.append({',
    ...
    '        ready_ids = []',
    '    capability_id = "web_search" if "web_search" in ready_ids else (ready_ids[0] if ready_ids else "web_search")',
    ...
    '    report = "# Evidence Matrix\\n\\n" + matrix_markdown + "\\n\\n## Recovery Notes\\n\\nGenerated by SciForge provider-first recovery adapter after AgentServer task code twice bypassed ready provider routes. Citations are provider metadata and should be verified before high-stakes use."',
    '    claims = [{"statement": row["claim"], "confidence": row["confidence"], "evidenceRefs": [row["citation/ref"]]} for row in rows[:8]]',
    '    message = f"Recovered through the SciForge {capability_id} provider route and produced an evidence matrix with {len(rows[:8])} candidate evidence items. Treat rows as provider-grounded metadata until full-text verification."',
    '    payload = {',
    '        "message": message,',
    '        "confidence": 0.68,',
    '        "claimType": "evidence-matrix",',
    ...
    '        ],',
    '        "executionUnits": [{"id": "provider-first-recovery", "status": "done", "tool": "invoke_capability", "summary": f"Called {capability_id} via SciForge provider route."}],',
    '        "artifacts": [',
