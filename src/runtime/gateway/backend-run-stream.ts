...
import {
  ...
  type BackendCapabilityDiscoveryToolTransportOptions,
} from './capability-discovery-tool-transport.js';

export interface BackendRunStreamResult {
  json: unknown;
...
export async function readBackendRunStream(
  response: Response,
  ...
  options: {
    ...
    maxSilentMs?: number;
    silencePolicy?: BackendStreamSilencePolicy;
    silentRetryCount?: number;
    silentRunId?: string;
    silentStreamDecision?: SilentStreamDecisionRecord;
    onSilentTimeout?: (message: string, audit: BackendSilentStreamGuardAudit) => void;
    capabilityDiscoveryToolTransport?: BackendCapabilityDiscoveryToolTransportOptions;
  } = {},
): Promise<BackendRunStreamResult> {
  if (!response.body) {
  ...
  async function consumeLine(rawLine: string) {
    ...
    for (const event of streamEventsFromEnvelope(envelope)) {
      ...
      const totalUsage = backendEventTotalUsage(event);
      if (
        ...
        if (options.convergenceGuardMode === 'repair-continuation') {
          throw new BackendRepairContinuationBoundedStopError(message, totalUsage, options.maxTotalUsage);
        }
        throw new BackendGenerationConvergenceGuardError(message, totalUsage, options.maxTotalUsage);
      }
    }
  ...
  for (;;) {
    ...
    if ('silentTimeout' in readResult) {
      ...
      const audit = backendSilentStreamGuardAudit(silencePolicy, {
        elapsedMs: silentMs,
...
export class BackendRepairContinuationBoundedStopError extends Error {
  readonly code = 'BACKEND_REPAIR_CONTINUATION_BOUNDED_STOP';
  readonly totalUsage: number;
  readonly limit: number;

  constructor(message: string, totalUsage: number, limit: number) {
    super(message);
    this.name = 'BackendRepairContinuationBoundedStopError';
    this.totalUsage = totalUsage;
    this.limit = limit;
  }
}

export class BackendGenerationConvergenceGuardError extends Error {
  readonly code = 'BACKEND_GENERATION_CONVERGENCE_GUARD';
  readonly totalUsage: number;
  readonly limit: number;

  constructor(message: string, totalUsage: number, limit: number) {
    super(message);
    this.name = 'BackendGenerationConvergenceGuardError';
    this.totalUsage = totalUsage;
    this.limit = limit;
  }
}

export function isBackendRepairContinuationBoundedStopError(error: unknown): error is BackendRepairContinuationBoundedStopError {
  if (error instanceof BackendRepairContinuationBoundedStopError) return true;
  if (!isRecord(error)) return false;
  return error.code === 'BACKEND_REPAIR_CONTINUATION_BOUNDED_STOP'
    || String(error.message || '').includes('Backend repair generation bounded-stop');
}
...
function convergenceGuardMessage(totalUsage: number, limit: number, mode: 'generation' | 'repair-continuation' = 'generation') {
  if (mode === 'repair-continuation') {
    return `Backend repair generation bounded-stop after ${totalUsage} total tokens (limit ${limit}); return a minimal single-stage repair/continue response, or a failed-with-reason ToolPayload with recoverActions requesting refs/digests-only follow-up instead of an unbounded repair loop.`;
  }
  return `backend generation stopped by convergence guard after ${totalUsage} total tokens (limit ${limit}); use bounded session refs, current-reference digests, or a smaller task plan instead of an unbounded generation loop.`;
}
...
export type BackendStreamSilenceDecision = 'visible-status' | 'retry' | 'abort' | 'background';

export interface BackendStreamSilencePolicy {
  schemaVersion: typeof AGENTSERVER_SILENT_STREAM_POLICY_SCHEMA_VERSION;
  source: string;
  timeoutMs: number;
  decision: BackendStreamSilenceDecision;
  status?: string;
  maxRetries?: number;
...
export interface BackendSilentStreamGuardAudit {
...
function streamEventsFromEnvelope(envelope: Record<string, unknown>) {
  ...
  if (looksLikeBackendStreamEvent(envelope)) return [normalizeTopLevelStreamEvent(envelope)];
  return [];
}

function looksLikeBackendStreamEvent(value: Record<string, unknown>) {
...
export function backendGenerationTokenGuardLimit(request: GatewayRequest, options: { repairContinuation?: boolean } = {}) {
...
export function currentReferenceDigestSilentGuardPolicy(request: GatewayRequest): BackendStreamSilencePolicy {
  const digests = Array.isArray(request.uiState?.currentReferenceDigests)
    ? request.uiState.currentReferenceDigests
    : [];
...
export function backendSilentStreamGuardAudit(
  policy: BackendStreamSilencePolicy | undefined,
  ...
  const message = `backend generation stopped by silent stream guard after ${elapsedMs}ms without stream events; silencePolicy decision=${fallback.decision}, timeoutMs=${fallback.timeoutMs}, retry=${retryCount}/${fallback.maxRetries ?? 0}.`;
  const silentStreamDecision = buildSilentStreamDecisionRecord({
    existing: input.existingDecision,
    runId: input.runId,
    source: 'backend.stream.silentGuard',
...
function silentPolicyFromTimeout(timeoutMs: number | undefined): BackendStreamSilencePolicy | undefined {
...
function recoveryActionForSilenceDecision(decision: BackendStreamSilenceDecision, retryable: boolean) {
...
function silenceDecisionField(value: unknown): BackendStreamSilenceDecision | undefined {
...
function backendEventTotalUsage(event: unknown) {
