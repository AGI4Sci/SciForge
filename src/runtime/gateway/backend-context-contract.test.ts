...
test('BackendContextRequest canonical serialization is deterministic and refs-first', () => {
  const request = buildContextRequest();
  const serialized = canonicalSerializeBackendContextRequest(request);
  ...
  assert.equal(canonicalSerializeBackendContextRequest(JSON.parse(serialized) as BackendContextRequest), serialized);
});

test('BackendContextRequest rejects raw history, raw body, full ref list, compaction state, recent turns, and untagged selected refs', () => {
  const polluted = {
    ...buildContextRequest(),
    currentTask: {
  ...
  const result = validateBackendContextRequest(polluted);
  ...
  const legacyCompat = validateBackendContextRequest({
    ...buildContextRequest(),
...
test('buildBackendContextRequest applies drift guards and deterministic byte budgets', () => {
  const huge = { ref: 'artifact:huge-old-result', kind: 'artifact', digest: 'sha256:huge', sizeBytes: 9999 };
  const fresh = buildBackendContextRequest({
  ...
  assert.equal(validateBackendContextRequest(fresh).ok, true);

  const continued = buildBackendContextRequest({
    sessionId: 'session-builder',
...
test('buildBackendContextRequest retrieval-unavailable path uses deterministic fallback refs only', () => {
  const request = buildBackendContextRequest({
    sessionId: 'session-builder',
...
test('SA-CONF-03 golden fixtures cover context request and degraded handoff boundaries', () => {
  ...
  for (const [name, fixture] of Object.entries(requestFixtures)) {
    assert.equal(validateBackendContextRequest(fixture).ok, true, name);
    const serialized = canonicalSerializeBackendContextRequest(fixture);
    assert.equal(canonicalSerializeBackendContextRequest(JSON.parse(serialized) as BackendContextRequest), serialized);
  ...
  for (const [name, kind, fixture] of failClosedCases) {
    const result = kind === 'request'
      ? validateBackendContextRequest(fixture)
      : validateDegradedHandoffPacket(fixture);
    assert.equal(result.ok, false, name);
  }
});

function buildContextRequest(): BackendContextRequest {
  return {
...
function buildSaConf03RequestFixtures(): Record<string, BackendContextRequest> {
  const freshExplicitRefs: BackendContextRequest = {
    _contractVersion: AGENTSERVER_CONTEXT_REQUEST_VERSION,
  ...
  const continueNoExplicitRefs: BackendContextRequest = {
    _contractVersion: AGENTSERVER_CONTEXT_REQUEST_VERSION,
    sessionId: 'sa-conf-03-session',
    turnId: 'turn-continue',
    cachePlan: {
  ...
  const repairExplicitRefs: BackendContextRequest = {
    _contractVersion: AGENTSERVER_CONTEXT_REQUEST_VERSION,
    sessionId: 'sa-conf-03-session',
    turnId: 'turn-repair',
    cachePlan: {
  ...
  const retrievalUnavailable: BackendContextRequest = {
    ...continueNoExplicitRefs,
