...
export function buildContextEnvelope(
  ...
  const harnessContextGovernance = harnessContextGovernanceForRequest(request);
  const governedCurrentReferences = applyHarnessRecordContextGovernance(
    'sessionFacts.currentReferences',
    currentReferences,
    harnessContextGovernance,
  );
  const governedCurrentReferenceDigests = applyHarnessRecordContextGovernance(
    'sessionFacts.currentReferenceDigests',
    currentReferenceDigests,
    harnessContextGovernance,
    {
      maxCount: harnessContextGovernance?.contextBudget.maxReferenceDigests,
  ...
  return {
    ...
    contextGovernanceAudit: harnessContextGovernance ? harnessContextGovernanceAudit(harnessContextGovernance) : undefined,
...
type HarnessContextGovernance = {
  schemaVersion: 'sciforge.context-envelope.harness-governance.v1';
  source: string;
  contractRef?: string;
  traceRef?: string;
  shadowMode?: boolean;
  contextBudget: HarnessContextBudget;
  contextRefs: {
    allowed: string[];
    blocked: string[];
    required: string[];
  };
  decisions: HarnessContextDecision[];
};

function harnessContextGovernanceForRequest(request: GatewayRequest): HarnessContextGovernance | undefined {
  const uiState = isRecord(request.uiState) ? request.uiState : {};
  if (!harnessContextGovernanceEnabled(uiState)) return undefined;
  const source = harnessContextSource(uiState);
  if (!source) return undefined;
  const contextBudget = harnessContextBudgetFromRecord(source.contract);
  const contextRefs = harnessContextRefsFromRecord(source.contract);
...
function harnessContextSource(uiState: Record<string, unknown>):
  ...
  if (handoff) {
    return {
      ...
      contract: harnessContractFromHandoff(handoff),
...
function harnessContractFromHandoff(handoff: Record<string, unknown>) {
...
function harnessContextGovernanceEnabled(uiState: Record<string, unknown>) {
...
function harnessContextBudgetFromRecord(contract: Record<string, unknown>): HarnessContextBudget {
...
function harnessContextRefsFromRecord(contract: Record<string, unknown>) {
  return {
...
function applyHarnessRecordContextGovernance(
  target: string,
  records: Array<Record<string, unknown>>,
  governance: HarnessContextGovernance | undefined,
  budget?: { maxCount?: number; budgetField?: keyof HarnessContextBudget },
) {
  if (!governance) return records;
  const beforeRefs = records.map((record, index) => harnessRecordAuditRef(record, index));
  const filtered = records.filter((record) => harnessRecordAllowed(record, governance));
  const filteredRefs = filtered.map((record, index) => harnessRecordAuditRef(record, index));
  ...
  const slimmed = selectHarnessBudgetedRecords(filtered, maxCount, governance.contextRefs.required);
  governance.decisions.push({
    ...
    omittedRefs: filtered
      .map((record, index) => harnessRecordAuditRef(record, index))
      .filter((ref) => !slimmed.map((record, index) => harnessRecordAuditRef(record, index)).includes(ref))
      .slice(0, 16),
...
function selectHarnessBudgetedRecords(
  records: Array<Record<string, unknown>>,
  maxCount: number,
  requiredRefs: string[],
) {
...
function harnessRecordAllowed(record: Record<string, unknown>, governance: HarnessContextGovernance) {
...
function recordMatchesAnyRef(record: Record<string, unknown> | undefined, refs: string[]) {
  ...
  const recordRefs = harnessRecordRefs(record);
  return refs.some((ref) => recordRefs.includes(ref));
}

function harnessRecordRefs(record: Record<string, unknown>) {
...
function harnessRecordAuditRef(record: Record<string, unknown>, index: number) {
  return harnessRecordRefs(record)[0] ?? `record:${index}`;
}

function harnessContextGovernanceAudit(governance: HarnessContextGovernance) {
  return {
