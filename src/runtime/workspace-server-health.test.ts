...
test('workspace writer health helper preserves the public health response shape', () => {
  ...
  assert.equal((health.capabilities as readonly string[]).includes('browser-host-search'), false);
  assert.equal((health.capabilities as readonly string[]).includes('browser-host-open-read'), false);
