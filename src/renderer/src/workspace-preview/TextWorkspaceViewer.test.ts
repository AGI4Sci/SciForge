...
describe('TextWorkspaceViewer', () => {
  ...
  it('keeps Markdown drafts attached to the shared content identity during annotation refreshes', () => {
    const observation = createTextObservation()
    const model = buildTextWorkspaceViewerModel(observation, true)

    expect(textWorkspaceViewerDraftSourceKey(
      {
        ...observation,
        documentAnnotations: {
          threadCount: 1,
    ...
    )).toBe('markdown-content-revision')
  })
