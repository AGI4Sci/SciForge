...
type Props = {
  ...
  onAccepted?: (document: VisualDocument) => void
}
...
async function workspaceImage(
  ...
  const result = await window.sciforge.readWorkspaceImage({ workspaceRoot, path })
...
export function VisualReviewPanel({
  ...
  const load = useCallback(async (quiet = false): Promise<void> => {
    ...
    try {
      const opened = await window.sciforge.openVisualDocument({ workspaceRoot, documentId })
  ...
  const requestRevision = useCallback(async (nextAnnotations: VisualReviewAnnotation[]) => {
    ...
    setMessage(null)
    try {
      await window.sciforge.saveVisualDocumentAnnotations({
        workspaceRoot,
        documentId,
        annotations: persistable.map(uiAnnotationToStored)
      })
      const exported = await window.sciforge.exportVisualReviewPacket({ workspaceRoot, documentId })
      onSendReviewRequest(buildVisualRevisionRequest({
        workspaceRoot,
        documentId,
  ...
  }, [documentId, onSendReviewRequest, workspaceRoot])

  const reject = useCallback(async () => {
    if (!activeCandidate) return
    setBusy(true)
    try {
      await window.sciforge.rejectVisualCandidateRevision({
        workspaceRoot,
        documentId,
  ...
  }, [activeCandidate, documentId, load, workspaceRoot])

  const accept = useCallback(async () => {
    if (!activeCandidate) return
    setBusy(true)
    try {
      const result = await window.sciforge.acceptVisualCandidateRevision({
        workspaceRoot,
        documentId,
        revisionId: activeCandidate.id
      })
  ...
  }, [activeCandidate, documentId, load, onAccepted, workspaceRoot])
    ...
    <aside className={`ds-no-drag flex h-full min-h-0 flex-col bg-white dark:bg-ds-canvas ${className}`}>
      <div className="flex min-h-12 items-center gap-2 border-b border-ds-border-muted px-3 py-2">
        ...
        <VisualStyleControl
          ...
          onApplied={(styleProfileRef) => {
            void window.sciforge.updateVisualDocumentContext({
              workspaceRoot,
              documentId,
              styleProfileRef
            }).then((result) => setDocument(result.document)).catch((error) => {
              setMessage(error instanceof Error ? error.message : String(error))
            })
          }}
        />
