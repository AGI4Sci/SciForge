...
test('right pane command action does not synthesize VirtualAppScreen attach refs from GUI state', () => {
  const action = createRightPaneCommandTextAction({
    session,
    ...
    commandText: '/computer-use screen attach --source right-pane-screen --profile "vscode-editor" --target-app-ref "app:profile/vscode-editor"',
  });

  assert.equal(
    action?.commandText,
    '/computer-use screen attach --source right-pane-screen --profile "vscode-editor" --target-app-ref "app:profile/vscode-editor"',
  );
  assert.doesNotMatch(action?.commandText ?? '', /virtual-app-screen|computer-use:screen-activation|gui\.present:.*screen-pane/);
});
