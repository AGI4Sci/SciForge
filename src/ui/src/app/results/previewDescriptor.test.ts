...
test('normalizes artifact preview descriptors from file metadata in UI results', () => {
  ...
  assert.equal(descriptor ? shouldHydratePreviewDescriptor(descriptor, artifact.path ?? '') : false, true);
});
...
test('merges hydrated descriptor derivatives and diagnostics in UI results', () => {
